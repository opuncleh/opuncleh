...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".opuncleh");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.OPUNCLEH_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$OPUNCLEH_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via OPUNCLEH_STATE_DIR.
