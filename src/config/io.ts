...
import type { OpunclehConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareOpunclehVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  opuncleh config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  opuncleh config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: OpunclehConfig,
  pathSegments: string[],
): { changed: boolean; next: OpunclehConfig } {
...
function coerceConfig(value: unknown): OpunclehConfig {
...
function stampConfigVersion(cfg: OpunclehConfig): OpunclehConfig {
...
function warnIfConfigFromFuture(cfg: OpunclehConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareOpunclehVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Opuncleh (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as OpunclehConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): OpunclehConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as OpunclehConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: OpunclehConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as OpunclehConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.OPUNCLEH_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.OPUNCLEH_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.OPUNCLEH_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.OPUNCLEH_WATCH_SESSION === "string" &&
        deps.env.OPUNCLEH_WATCH_SESSION.trim().length > 0
          ? deps.env.OPUNCLEH_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.OPUNCLEH_WATCH_COMMAND === "string" &&
        deps.env.OPUNCLEH_WATCH_COMMAND.trim().length > 0
          ? deps.env.OPUNCLEH_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `OPUNCLEH_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: OpunclehConfig | null = null;
let runtimeConfigSourceSnapshot: OpunclehConfig | null = null;
let runtimeConfigSnapshotRefreshHandler: RuntimeConfigSnapshotRefreshHandler | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.OPUNCLEH_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.OPUNCLEH_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): OpunclehConfig | null {
  return runtimeConfigSnapshot;
}

export function getRuntimeConfigSourceSnapshot(): OpunclehConfig | null {
  return runtimeConfigSourceSnapshot;
}

function isCompatibleTopLevelRuntimeProjectionShape(params: {
  runtimeSnapshot: OpunclehConfig;
  candidate: OpunclehConfig;
}): boolean {
...
export function projectConfigOntoRuntimeSourceSnapshot(config: OpunclehConfig): OpunclehConfig {
...
export function loadConfig(): OpunclehConfig {
...
export async function readBestEffortConfig(): Promise<OpunclehConfig> {
...
export async function writeConfigFile(
  ...
  // edits to opuncleh.json remain visible (no stale snapshot).
}
