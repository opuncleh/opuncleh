...
import type { OpunclehConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: OpunclehConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: OpunclehConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: OpunclehConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = OpunclehSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as OpunclehConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as OpunclehConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as OpunclehConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as OpunclehConfig,
  };
}
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native Opuncleh config schema.
