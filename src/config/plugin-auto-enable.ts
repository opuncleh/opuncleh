...
function isWhatsAppConfigured(cfg: OpunclehConfig): boolean {
...
function isGenericChannelConfigured(cfg: OpunclehConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: OpunclehConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: OpunclehConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: OpunclehConfig, env: NodeJS.ProcessEnv): string[] {
...
function isPluginExplicitlyDisabled(cfg: OpunclehConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: OpunclehConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: OpunclehConfig, pluginId: string): OpunclehConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
