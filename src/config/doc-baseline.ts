...
function resolveRepoRoot(): string {
  const fromPackage = resolveOpunclehPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    OPUNCLEH_STATE_DIR: path.join(os.tmpdir(), "opuncleh-config-doc-baseline-state"),
    OPUNCLEH_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
  };
