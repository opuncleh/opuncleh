...
import type { ConfigFileSnapshot } from "./types.opuncleh.js";
import { OpunclehSchema } from "./zod-schema.js";

const { mapSensitivePaths } = __test__;
const mainSchemaHints = mapSensitivePaths(OpunclehSchema, "", {});
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.opuncleh/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/opuncleh/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/opuncleh/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/opuncleh/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/opuncleh/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = OpunclehSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "OpunclehConfig";
    const hints = mainSchemaHints;
