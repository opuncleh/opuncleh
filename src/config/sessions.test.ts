...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "opuncleh-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ OPUNCLEH_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-opuncleh",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-opuncleh#general");
  });
  ...
  it("derives session transcripts dir from OPUNCLEH_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { OPUNCLEH_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when OPUNCLEH_STATE_DIR differs from stored paths", () => {
