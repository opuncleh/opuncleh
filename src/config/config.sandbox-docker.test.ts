...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("opuncleh-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "opuncleh-sandbox-browser-global" },
      agentBrowser: { network: "opuncleh-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("opuncleh-sandbox-browser-agent");
  });
