...
import { buildWebSearchProviderConfig, withTempHome, writeOpunclehConfig } from "./test-helpers.js";
import { OpunclehSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = OpunclehSchema.safeParse({
      $schema: "https://opuncleh.io/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://opuncleh.io/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = OpunclehSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = OpunclehSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("flags legacy config entries without auto-migrating", async () => {
    await withTempHome(async (home) => {
      await writeOpunclehConfig(home, {
        agents: { list: [{ id: "pi" }] },
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeOpunclehConfig(home, {
        gateway: { bind: "${OPUNCLEH_BIND}" },
      });

      const prev = process.env.OPUNCLEH_BIND;
      process.env.OPUNCLEH_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.OPUNCLEH_BIND;
        } else {
          process.env.OPUNCLEH_BIND = prev;
        }
      }
    });
  });
