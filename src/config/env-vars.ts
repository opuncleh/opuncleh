...
function collectConfigEnvVarsByTarget(cfg?: OpunclehConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: OpunclehConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: OpunclehConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: OpunclehConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.OPUNCLEH_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
