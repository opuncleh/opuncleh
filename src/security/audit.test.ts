...
`,
  ...
  it("flags non-loopback bind without auth as critical", async () => {
    ...
    const prevToken = process.env.OPUNCLEH_GATEWAY_TOKEN;
    const prevPassword = process.env.OPUNCLEH_GATEWAY_PASSWORD;
    delete process.env.OPUNCLEH_GATEWAY_TOKEN;
    delete process.env.OPUNCLEH_GATEWAY_PASSWORD;
    ...
    try {
      ...
      if (prevToken === undefined) {
        ...
        process.env.OPUNCLEH_GATEWAY_TOKEN = prevToken;
      }
      if (prevPassword === undefined) {
        delete process.env.OPUNCLEH_GATEWAY_PASSWORD;
      } else {
        process.env.OPUNCLEH_GATEWAY_PASSWORD = prevPassword;
      }
    }
  });
  ...
  it("warns when sandbox browser containers have missing or stale hash labels", async () => {
    ...
    const execDockerRawFn = (async (args: string[]) => {
      if (args[0] === "ps") {
        return {
          stdout: Buffer.from("opuncleh-sbx-browser-old\nopuncleh-sbx-browser-missing-hash\n"),
  ...
  it("flags wildcard Control UI origins by exposure level", async () => {
    const loopbackCfg: OpunclehConfig = {
      gateway: {
        bind: "loopback",
        controlUi: { allowedOrigins: ["*"] },
      },
    };
    const exposedCfg: OpunclehConfig = {
      gateway: {
        bind: "lan",
        auth: { mode: "token", token: "very-long-browser-token-0123456789" },
        controlUi: { allowedOrigins: ["*"] },
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: OpunclehConfig = { session: { dmScope: "main" } };
  ...
  it("flags hooks token reuse of the gateway env token as critical", async () => {
    const prevToken = process.env.OPUNCLEH_GATEWAY_TOKEN;
    process.env.OPUNCLEH_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: OpunclehConfig = {
  ...
  it("warns on unpinned npm install specs and missing integrity metadata", async () => {
    ...
    const res = await runSecurityAudit({
      ...
      configPath: path.join(sharedInstallMetadataStateDir, "opuncleh.json"),
      execDockerRawFn: execDockerRawUnavailable,
    });
