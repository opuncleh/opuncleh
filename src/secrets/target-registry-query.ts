...
const OPUNCLEH_COMPILED_SECRET_TARGETS = COMPILED_SECRET_TARGET_REGISTRY.filter(
...
function buildConfigTargetIdIndex(): Map<string, CompiledTargetRegistryEntry[]> {
  ...
  for (const entry of OPUNCLEH_COMPILED_SECRET_TARGETS) {
...
const OPUNCLEH_TARGETS_BY_ID = buildConfigTargetIdIndex();
...
export function discoverConfigSecretTargetsByIds(
  ...
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: OPUNCLEH_COMPILED_SECRET_TARGETS,
    entriesById: OPUNCLEH_TARGETS_BY_ID,
  });
