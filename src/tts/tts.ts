...
export function resolveTtsConfig(cfg: OpunclehConfig): ResolvedTtsConfig {
...
export function resolveTtsPrefsPath(config: ResolvedTtsConfig): string {
  ...
  const envPath = process.env.OPUNCLEH_TTS_PREFS?.trim();
  if (envPath) {
...
export function buildTtsSystemPromptHint(cfg: OpunclehConfig): string | undefined {
...
export function resolveTtsProviderOrder(primary: TtsProvider, cfg?: OpunclehConfig): TtsProvider[] {
...
export async function textToSpeech(params: {
  ...
  for (const provider of providers) {
    ...
    try {
      ...
      const tempRoot = resolvePreferredOpunclehTmpDir();
