...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set OPUNCLEH_GATEWAY_TOKEN/OPUNCLEH_GATEWAY_PASSWORD, pass --token/--password,",
...
export class GatewayChatClient {
  ...
  constructor(connection: ResolvedGatewayConnection) {
    ...
    this.client = new GatewayClient({
      ...
      clientDisplayName: "opuncleh-tui",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.OPUNCLEH_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.OPUNCLEH_GATEWAY_PASSWORD);
