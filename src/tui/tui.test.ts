...
describe("resolveInitialTuiAgentId", () => {
  const cfg: OpunclehConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/opuncleh" },
        { id: "ops", workspace: "/tmp/opuncleh/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/opuncleh/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run opuncleh devices list");
    expect(state.pairingHint).toContain("opuncleh devices list");
  });
