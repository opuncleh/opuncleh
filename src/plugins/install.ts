...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing opuncleh.extensions; update the plugin package to include opuncleh.extensions (for example ["./dist/index.js"]). See https://www.opuncleh.com/docs.html/help/troubleshooting#plugin-install-fails-with-missing-opuncleh-extensions';

export const PLUGIN_INSTALL_ERROR_CODE = {
  INVALID_NPM_SPEC: "invalid_npm_spec",
  MISSING_OPUNCLEH_EXTENSIONS: "missing_opuncleh_extensions",
  EMPTY_OPUNCLEH_EXTENSIONS: "empty_opuncleh_extensions",
...
function ensureOpunclehExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_OPUNCLEH_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json opuncleh.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_OPUNCLEH_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function installBundleFromSourceDir(
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Bundle "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "opuncleh security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Bundle "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "opuncleh security audit --deep" after install.`,
    );
  }
...
async function detectNativePackageInstallSource(packageDir: string): Promise<boolean> {
  ...
  try {
    ...
    return ensureOpunclehExtensions({ manifest }).ok;
  } catch {
    return false;
  }
}
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureOpunclehExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from opuncleh.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-opuncleh"), the plugin registry
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Plugin "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "opuncleh security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "opuncleh security audit --deep" after install.`,
    );
  }
...
export async function installPluginFromNpmSpec(params: {
  ...
  const flowResult = await installFromNpmSpecArchiveWithInstaller({
    tempDirPrefix: "opuncleh-npm-pack-",
    spec,
    timeoutMs,
