...
import { resolveOpunclehPackageRootSync } from "../infra/opuncleh-root.js";
...
import { discoverOpunclehPlugins } from "./discovery.js";
...
function listPluginSdkExportedSubpaths(params: { modulePath?: string } = {}): string[] {
  ...
  const packageRoot = resolveOpunclehPackageRootSync({
...
const resolvePluginSdkScopedAliasMap = (): Record<string, string> => {
  ...
  for (const subpath of listPluginSdkExportedSubpaths()) {
    ...
    if (resolved) {
      aliasMap[`opuncleh/plugin-sdk/${subpath}`] = resolved;
    }
  }
  return aliasMap;
};
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: OpunclehPluginDefinition;
  register?: OpunclehPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as OpunclehPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as OpunclehPluginDefinition;
...
function resolveCandidateDuplicateRank(params: {
  candidate: ReturnType<typeof discoverOpunclehPlugins>["candidates"][number];
...
function compareDuplicateCandidateOrder(params: {
  left: ReturnType<typeof discoverOpunclehPlugins>["candidates"][number];
  right: ReturnType<typeof discoverOpunclehPlugins>["candidates"][number];
  manifestByRoot: Map<string, ReturnType<typeof loadPluginManifestRegistry>["plugins"][number]>;
  provenance: PluginProvenanceIndex;
  env: NodeJS.ProcessEnv;
}): number {
...
export function loadOpunclehPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  if (options.activate === false && options.cache !== false) {
    throw new Error(
      "loadOpunclehPlugins: activate:false requires cache:false to prevent command registry divergence",
    );
  }
  ...
  const getJiti = () => {
    ...
    const aliasMap = {
      ...(pluginSdkAlias ? { "opuncleh/plugin-sdk": pluginSdkAlias } : {}),
  ...
  const discovery = discoverOpunclehPlugins({
    workspaceDir: options.workspaceDir,
  ...
  for (const candidate of orderedCandidates) {
    ...
    if (record.format === "bundle") {
      ...
      for (const capability of unsupportedCapabilities) {
        registry.diagnostics.push({
          ...
          pluginId: record.id,
          source: record.source,
          message: `bundle capability detected but not wired into Opuncleh yet: ${capability}`,
        });
      }
      if (
        enableState.enabled &&
    ...
    let mod: OpunclehPluginModule | null = null;
    try {
      mod = getJiti()(safeSource) as OpunclehPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
