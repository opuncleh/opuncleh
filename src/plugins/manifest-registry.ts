...
import { discoverOpunclehPlugins, type PluginCandidate } from "./discovery.js";
...
function resolveManifestCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.OPUNCLEH_PLUGIN_MANIFEST_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseManifestCache(env: NodeJS.ProcessEnv): boolean {
  const disabled = env.OPUNCLEH_DISABLE_PLUGIN_MANIFEST_CACHE?.trim();
  if (disabled) {
    return false;
  }
...
function buildRecord(params: {
  ...
  return {
    ...
    format: params.candidate.format ?? "opuncleh",
...
export function loadPluginManifestRegistry(params: {
  ...
  for (const candidate of candidates) {
    ...
    const isBundleRecord = (candidate.format ?? "opuncleh") === "bundle";
    const manifestRes =
