...
describe("updateNpmInstalledPlugins", () => {
  ...
  it("skips integrity drift checks for unpinned npm specs during dry-run updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "opik-opuncleh",
      targetDir: "/tmp/opik-opuncleh",
      version: "0.2.6",
      extensions: ["index.ts"],
    });
      ...
      config: {
        plugins: {
          installs: {
            "opik-opuncleh": {
              source: "npm",
              spec: "@opik/opik-opuncleh",
              integrity: "sha512-old",
              installPath: "/tmp/opik-opuncleh",
            },
          },
        },
      },
      pluginIds: ["opik-opuncleh"],
      dryRun: true,
    });
  ...
  it("formats package-not-found updates with a stable message", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ...
      error: "Package not found on npm: @opuncleh/missing.",
    });
    ...
    expect(result.outcomes).toEqual([
      {
        pluginId: "missing",
        status: "error",
        message: "Failed to check missing: npm package not found for @opuncleh/missing.",
      },
    ]);
  });
  ...
  it("migrates legacy unscoped install keys when a scoped npm package updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "@opuncleh/voice-call",
      targetDir: "/tmp/opuncleh-voice-call",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(result.config.plugins?.allow).toEqual(["@opuncleh/voice-call"]);
    expect(result.config.plugins?.deny).toEqual(["@opuncleh/voice-call"]);
    expect(result.config.plugins?.slots?.memory).toBe("@opuncleh/voice-call");
    expect(result.config.plugins?.entries?.["@opuncleh/voice-call"]).toEqual({
      enabled: false,
      hooks: { allowPromptInjection: false },
    });
    expect(result.config.plugins?.entries?.["voice-call"]).toBeUndefined();
    expect(result.config.plugins?.installs?.["@opuncleh/voice-call"]).toMatchObject({
      source: "npm",
      spec: "@opuncleh/voice-call",
      installPath: "/tmp/opuncleh-voice-call",
      version: "0.0.2",
    });
...
describe("syncPluginsForUpdateChannel", () => {
  ...
  it("uses the provided env when matching bundled load and install paths", async () => {
    const bundledHome = "/tmp/opuncleh-home";
    resolveBundledPluginSourcesMock.mockReturnValue(
      new Map([
        [
          "feishu",
