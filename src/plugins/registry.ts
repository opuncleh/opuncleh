...
import type {
  OpunclehPluginApi,
  OpunclehPluginChannelRegistration,
  OpunclehPluginCliRegistrar,
  OpunclehPluginCommandDefinition,
  OpunclehPluginHttpRouteAuth,
  OpunclehPluginHttpRouteMatch,
  OpunclehPluginHttpRouteHandler,
  OpunclehPluginHttpRouteParams,
  OpunclehPluginHookOptions,
  ProviderPlugin,
  OpunclehPluginService,
  OpunclehPluginToolContext,
...
export type PluginToolRegistration = {
  pluginId: string;
  factory: OpunclehPluginToolFactory;
  names: string[];
  optional: boolean;
  source: string;
};
...
export type PluginCliRegistration = {
  pluginId: string;
  register: OpunclehPluginCliRegistrar;
  commands: string[];
  source: string;
};
...
export type PluginServiceRegistration = {
  pluginId: string;
  service: OpunclehPluginService;
  source: string;
};

export type PluginCommandRegistration = {
  pluginId: string;
  command: OpunclehPluginCommandDefinition;
  source: string;
};
...
export function createPluginRegistry(registryParams: PluginRegistryParams) {
  ...
  const registerTool = (
    ...
    const factory: OpunclehPluginToolFactory =
      typeof tool === "function" ? tool : (_ctx: OpunclehPluginToolContext) => tool;
  ...
  const registerHook = (
    ...
    opts: OpunclehPluginHookOptions | undefined,
    config: OpunclehPluginApi["config"],
  ) => {
  ...
  const registerHttpRoute = (record: PluginRecord, params: OpunclehPluginHttpRouteParams) => {
  ...
  const registerChannel = (
    record: PluginRecord,
    registration: OpunclehPluginChannelRegistration | ChannelPlugin,
  ) => {
    const normalized =
      typeof (registration as OpunclehPluginChannelRegistration).plugin === "object"
        ? (registration as OpunclehPluginChannelRegistration)
  ...
  const registerCli = (
    record: PluginRecord,
    registrar: OpunclehPluginCliRegistrar,
  ...
  const registerService = (record: PluginRecord, service: OpunclehPluginService) => {
  ...
  const registerCommand = (record: PluginRecord, command: OpunclehPluginCommandDefinition) => {
