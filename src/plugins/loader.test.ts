...
const fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "opuncleh-plugin-"));
let tempDirIndex = 0;
...
describe("loadOpunclehPlugins", () => {
  ...
  it("does not reuse cached bundled plugin registries across env changes", () => {
    ...
    const first = loadOpunclehPlugins({
      ...options,
      env: {
        ...process.env,
    ...
    const second = loadOpunclehPlugins({
      ...options,
      env: {
        ...process.env,
  ...
  it("does not reuse cached registries when env-resolved install paths change", () => {
    ...
    const second = loadOpunclehPlugins(secondOptions);
    const third = loadOpunclehPlugins(secondOptions);
  ...
  it("prefers bundled plugin over auto-discovered global duplicate ids", () => {
    ...
    const stateDir = makeTempDir();
    withEnv({ OPUNCLEH_STATE_DIR: stateDir, CLAWDBOT_STATE_DIR: undefined }, () => {
  ...
  it("does not auto-load workspace-discovered plugins unless explicitly trusted", () => {
    ...
    const workspaceExtDir = path.join(workspaceDir, ".opuncleh", "extensions", "workspace-helper");
  ...
  it("does not warn about missing provenance for env-resolved load paths", () => {
    ...
    const ignoredHome = makeTempDir();
    const stateDir = makeTempDir();
    const pluginDir = path.join(opunclehHome, "plugins", "tracked-load-path");
    fs.mkdirSync(pluginDir, { recursive: true });
    const plugin = writePlugin({
  ...
  it("does not warn about missing provenance for env-resolved install paths", () => {
    ...
    const ignoredHome = makeTempDir();
    const stateDir = makeTempDir();
    const pluginDir = path.join(opunclehHome, "plugins", "tracked-install-path");
    fs.mkdirSync(pluginDir, { recursive: true });
    const plugin = writePlugin({
