...
type PluginLoadConfig = NonNullable<Parameters<typeof loadOpunclehPlugins>[0]>["config"];
...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "opuncleh-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.OPUNCLEH_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "opuncleh.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.OPUNCLEH_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
    return loadOpunclehPlugins({
      cache: false,
      ...
      JSON.stringify(
        {
          ...
          opuncleh: { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
  ...
  process.env.OPUNCLEH_BUNDLED_PLUGINS_DIR = bundledDir;
...
function setupBundledTelegramPlugin() {
  ...
  process.env.OPUNCLEH_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadOpunclehPlugins>) {
...
function useNoBundledPlugins() {
  process.env.OPUNCLEH_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
}
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadOpunclehPlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
function loadRegistryFromAllowedPlugins(
  plugins: TempPlugin[],
  options?: Omit<Parameters<typeof loadOpunclehPlugins>[0], "cache" | "config">,
) {
  return loadOpunclehPlugins({
    cache: false,
    ...options,
...
function createEscapingEntryFixture(params: { id: string; sourceBody: string }) {
  ...
  fs.writeFileSync(
    path.join(pluginDir, "opuncleh.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
        configSchema: EMPTY_PLUGIN_SCHEMA,
...
function createPluginSdkAliasFixture(params?: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ name: "opuncleh", type: "module" }, null, 2),
    "utf-8",
  );
...
function loadBundleFixture(params: {
  ...
  const bundleRoot = path.join(workspaceDir, ".opuncleh", "extensions", params.pluginId);
  params.build(bundleRoot);
  return withEnv({ OPUNCLEH_STATE_DIR: stateDir, ...params.env }, () =>
    loadOpunclehPlugins({
      workspaceDir,
...
function expectNoUnwiredBundleDiagnostic(
  registry: ReturnType<typeof loadOpunclehPlugins>,
  pluginId: string,
) {
  expect(
...
function expectCachePartitionByPluginSource(params: {
  pluginId: string;
  loadFirst: () => ReturnType<typeof loadOpunclehPlugins>;
  loadSecond: () => ReturnType<typeof loadOpunclehPlugins>;
...
function expectCacheMissThenHit(params: {
  loadFirst: () => ReturnType<typeof loadOpunclehPlugins>;
  loadVariant: () => ReturnType<typeof loadOpunclehPlugins>;
}) {
  const first = params.loadFirst();
...
function createEnvResolvedPluginFixture(pluginId: string) {
  useNoBundledPlugins();
  const opunclehHome = makeTempDir();
  const ignoredHome = makeTempDir();
  const stateDir = makeTempDir();
  const pluginDir = path.join(opunclehHome, "plugins", pluginId);
  mkdirSafe(pluginDir);
  ...
  const env = {
    ...process.env,
    OPUNCLEH_HOME: opunclehHome,
    HOME: ignoredHome,
    OPUNCLEH_STATE_DIR: stateDir,
    CLAWDBOT_STATE_DIR: undefined,
    OPUNCLEH_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins",
  };
  return { plugin, env };
}
...
function expectEscapingEntryRejected(params: {
  ...
  const registry = loadOpunclehPlugins({
    cache: false,
    config: {
      plugins: {
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    delete process.env.OPUNCLEH_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.OPUNCLEH_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("bundle plugins", () => {
  it("reports Codex bundles as loaded bundle plugins without importing runtime code", () => {
    ...
    const workspaceDir = makeTempDir();
    const stateDir = makeTempDir();
    const bundleRoot = path.join(workspaceDir, ".opuncleh", "extensions", "sample-bundle");
    ...
    const registry = withEnv({ OPUNCLEH_STATE_DIR: stateDir }, () =>
      loadOpunclehPlugins({
        workspaceDir,
    ...
    const cases = [
      {
        ...
        assert: (registry: ReturnType<typeof loadOpunclehPlugins>) => {
...
module.exports = { id: "skipped-scoped-only", register() { throw new Error("skipped plugin should not load"); } };`,
      ...
      },
      {
        ...
        run: () => {
          ...
          const full = loadOpunclehPlugins(options);
          const scoped = loadOpunclehPlugins({
            ...options,
            onlyPluginIds: ["allowed-cache-scope"],
          });
          const scopedAgain = loadOpunclehPlugins({
            ...options,
            onlyPluginIds: ["allowed-cache-scope"],
          });
  ...
  it("only publishes plugin commands to the global registry during activating loads", async () => {
    ...
    const active = loadOpunclehPlugins({
      cache: false,
      workspaceDir: plugin.dir,
      config: {
        plugins: {
  ...
  it("throws when activate:false is used without cache:false", () => {
    expect(() => loadOpunclehPlugins({ activate: false })).toThrow(
      "activate:false requires cache:false",
    );
    expect(() => loadOpunclehPlugins({ activate: false, cache: true })).toThrow(
      "activate:false requires cache:false",
    );
  });
  ...
  it("re-initializes global hook runner when serving registry from cache", () => {
    ...
    const first = loadOpunclehPlugins(options);
    expect(getGlobalHookRunner()).not.toBeNull();

    resetGlobalHookRunner();
    expect(getGlobalHookRunner()).toBeNull();

    const second = loadOpunclehPlugins(options);
        ...
        return {
          ...
          loadFirst: () =>
            loadOpunclehPlugins({
              ...
              env: {
                ...process.env,
                OPUNCLEH_BUNDLED_PLUGINS_DIR: bundledA,
              },
            }),
          loadSecond: () =>
            loadOpunclehPlugins({
              ...
              env: {
                ...process.env,
                OPUNCLEH_BUNDLED_PLUGINS_DIR: bundledB,
              },
            }),
        };
      },
          ...
          loadFirst: () =>
            loadOpunclehPlugins({
              ...
              env: {
                ...process.env,
                HOME: homeA,
                OPUNCLEH_HOME: undefined,
                OPUNCLEH_STATE_DIR: stateDir,
                OPUNCLEH_BUNDLED_PLUGINS_DIR: bundledDir,
              },
            }),
          loadSecond: () =>
            loadOpunclehPlugins({
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const ignoredHome = makeTempDir();
        const stateDir = makeTempDir();
        const pluginDir = path.join(opunclehHome, "plugins", "tracked-install-cache");
        mkdirSafe(pluginDir);
        const plugin = writePlugin({
    ...
    },
    {
      ...
      setup: () => {
        ...
        return {
          loadFirst: () => loadOpunclehPlugins(options),
          loadVariant: () =>
            loadOpunclehPlugins({
              ...options,
              runtimeOptions: {
  ...
  it("normalizes bundled plugin env overrides against the provided env", () => {
    ...
    const registry = loadOpunclehPlugins({
      env: {
        ...
        HOME: homeDir,
        OPUNCLEH_HOME: undefined,
        OPUNCLEH_BUNDLED_PLUGINS_DIR: override,
      },
      config: {
        plugins: {
          allow: ["tilde-bundled"],
  ...
  it("prefers OPUNCLEH_HOME over HOME for env-expanded load paths", () => {
    const ignoredHome = makeTempDir();
    const opunclehHome = makeTempDir();
    const stateDir = makeTempDir();
    const bundledDir = makeTempDir();
    const plugin = writePlugin({
      id: "opuncleh-home-demo",
      dir: path.join(opunclehHome, "plugins", "opuncleh-home-demo"),
      filename: "index.cjs",
      body: `module.exports = { id: "opuncleh-home-demo", register() {} };`,
    });
    ...
    const registry = loadOpunclehPlugins({
      ...
      config: {
        plugins: {
          allow: ["opuncleh-home-demo"],
          entries: {
            "opuncleh-home-demo": { enabled: true },
          },
          load: {
            paths: ["~/plugins/opuncleh-home-demo"],
          },
        },
      },
    });
    ...
    expect(
      fs.realpathSync(
        registry.plugins.find((entry) => entry.id === "opuncleh-home-demo")?.source ?? "",
      ),
    ).toBe(fs.realpathSync(plugin.file));
  });
  ...
  api.registerHook("gateway:startup", () => {}, { name: "shared-hook" });
        ...
        selectCount: (registry: ReturnType<typeof loadOpunclehPlugins>) =>
  ...
  api.registerCli(() => {}, { commands: ["shared-cli"] });
        ...
        assertPrimaryOwner: (registry: ReturnType<typeof loadOpunclehPlugins>) => {
...
module.exports = {
  ...
  register(api) {
    ...
    fs.writeFileSync(
      path.join(plugin.dir, "opuncleh.plugin.json"),
      JSON.stringify(
        {
          id: "lazy-channel",
          configSchema: EMPTY_PLUGIN_SCHEMA,
    ...
    const setupRegistry = loadOpunclehPlugins({
      cache: false,
      config,
  ...
  it.each([
    {
      ...
      fixture: {
        ...
        packageName: "@opuncleh/setup-entry-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@opuncleh/setup-runtime-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@opuncleh/setup-runtime-preferred-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@opuncleh/setup-runtime-not-preferred-test",
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          fs.writeFileSync(
            path.join(memoryADir, "opuncleh.plugin.json"),
            JSON.stringify(
              {
                id: "memory-a",
                kind: "memory",
          ...
          fs.writeFileSync(
            path.join(memoryBDir, "opuncleh.plugin.json"),
            JSON.stringify(
              {
                id: "memory-b",
                kind: "memory",
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          const stateDir = makeTempDir();
          return withEnv({ OPUNCLEH_STATE_DIR: stateDir, CLAWDBOT_STATE_DIR: undefined }, () => {
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          const workspaceDir = makeTempDir();
          const workspaceExtDir = path.join(workspaceDir, ".opuncleh", "extensions", "shadowed");
          mkdirSafe(workspaceExtDir);
          writePlugin({
            id: "shadowed",
            body: `module.exports = { id: "shadowed", register() {} };`,
  ...
  it("supports legacy plugins importing monolithic plugin-sdk root", async () => {
    ...
    const plugin = writePlugin({
      ...
      body: `module.exports = {
  id: "legacy-root-import",
  configSchema: (require("opuncleh/plugin-sdk").emptyPluginConfigSchema)(),
  register() {},
};`,
    });
    ...
    const script = `
      import { loadOpunclehPlugins } from ${JSON.stringify(loaderModuleUrl)};
      const registry = loadOpunclehPlugins({
        cache: false,
  ...
  it.each([
    ...
    },
    {
      ...
      buildFixture: () => createPluginSdkAliasFixture(),
      modulePath: () => "/tmp/tsx-cache/opuncleh-loader.js",
      argv1: (root: string) => path.join(root, "opuncleh.mjs"),
      srcFile: "index.ts",
      distFile: "index.js",
      env: { NODE_ENV: undefined },
      expected: "src" as const,
