...
export function resolveBundledPluginsDir(env: NodeJS.ProcessEnv = process.env): string | undefined {
  const override = env.OPUNCLEH_BUNDLED_PLUGINS_DIR?.trim();
  if (override) {
  ...
  try {
    const packageRoots = [
      resolveOpunclehPackageRootSync({ cwd: process.cwd() }),
      resolveOpunclehPackageRootSync({ moduleUrl: import.meta.url }),
    ].filter(
