...
import { clearPluginDiscoveryCache, discoverOpunclehPlugins } from "./discovery.js";
import {
...
function makeTempDir() {
  return makeTrackedTempDir("opuncleh-plugins", tempDirs);
}
...
async function discoverWithStateDir(
  stateDir: string,
  params: Parameters<typeof discoverOpunclehPlugins>[0],
) {
  return discoverOpunclehPlugins({ ...params, env: buildDiscoveryEnv(stateDir) });
}
...
function writePluginPackageManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      opuncleh: { extensions: params.extensions },
    }),
    "utf-8",
  );
}
...
describe("discoverOpunclehPlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const workspaceExt = path.join(workspaceDir, ".opuncleh", "extensions");
  ...
  it("resolves tilde workspace dirs against the provided env", () => {
    ...
    const workspaceExt = path.join(workspaceRoot, ".opuncleh", "extensions");
    mkdirSafe(workspaceExt);
    fs.writeFileSync(path.join(workspaceExt, "tilde-workspace.ts"), "export default {}", "utf-8");

    const result = discoverOpunclehPlugins({
  ...
  it("derives unscoped ids for scoped packages", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@opuncleh/voice-call",
  ...
  it("normalizes bundled provider package ids to canonical plugin ids", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@opuncleh/ollama-provider",
      extensions: ["./src/index.ts"],
    });
    fs.writeFileSync(
      path.join(globalExt, "src", "index.ts"),
  ...
  it("normalizes bundled speech package ids to canonical plugin ids", async () => {
    ...
    writePluginPackageManifest({
      ...
      packageName: "@opuncleh/elevenlabs-speech",
      extensions: ["./src/index.ts"],
    });
    writePluginPackageManifest({
      packageDir: microsoftDir,
      packageName: "@opuncleh/microsoft-speech",
      extensions: ["./src/index.ts"],
    });
  ...
  it("treats configured directory paths as plugin packages", async () => {
    ...
    writePluginPackageManifest({
      packageDir: packDir,
      packageName: "@opuncleh/demo-plugin-dir",
      extensions: ["./index.js"],
    });
  ...
  it("falls back to legacy index discovery when a scanned bundle sidecar is malformed", async () => {
    ...
    expect(legacy?.format).toBe("opuncleh");
  ...
  it("blocks extension entries that escape package directory", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@opuncleh/escape-pack",
  ...
  it("reuses discovery results from cache until cleared", async () => {
    ...
    const first = discoverOpunclehPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        OPUNCLEH_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
    });
    ...
    const second = discoverOpunclehPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        OPUNCLEH_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
    ...
    const third = discoverOpunclehPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        OPUNCLEH_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
