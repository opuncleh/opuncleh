...
function makeTempDir() {
  return makeTrackedTempDir("opuncleh-manifest-registry", tempDirs);
}

function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "opuncleh.plugin.json"), JSON.stringify(manifest), "utf-8");
}
...
function prepareLinkedManifestFixture(params: { id: string; mode: "symlink" | "hardlink" }): {
  ...
  const outsideManifest = path.join(outsideDir, "opuncleh.plugin.json");
  const linkedManifest = path.join(rootDir, "opuncleh.plugin.json");
  ...
  it("does not reuse cached load-path manifests across env home changes", () => {
    ...
    const first = loadPluginManifestRegistry({
      ...
      env: {
        ...
        OPUNCLEH_STATE_DIR: path.join(homeA, ".state"),
      },
    });
    const second = loadPluginManifestRegistry({
      cache: true,
      ...
      env: {
        ...
        HOME: homeB,
        OPUNCLEH_HOME: undefined,
        OPUNCLEH_STATE_DIR: path.join(homeB, ".state"),
      },
    });
