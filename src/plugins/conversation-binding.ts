...
const APPROVALS_PATH = "~/.opuncleh/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "opuncleh-app-server:thread:",
  "opuncleh-codex-app-server:thread:",
] as const;
...
const pluginBindingGlobalStateKey = Symbol.for("opuncleh.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to Opuncleh instead.${buildDetachHintSuffix(binding.detachHint)}`;
}
