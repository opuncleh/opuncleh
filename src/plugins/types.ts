...
export type OpunclehPluginConfigSchema = {
...
export type OpunclehPluginToolContext = {
...
export type OpunclehPluginToolFactory = (
  ctx: OpunclehPluginToolContext,
) => AnyAgentTool | AnyAgentTool[] | null | undefined;

export type OpunclehPluginToolOptions = {
  name?: string;
  names?: string[];
  optional?: boolean;
};

export type OpunclehPluginHookOptions = {
  entry?: HookEntry;
  name?: string;
  description?: string;
  register?: boolean;
...
export type ProviderAuthResult = {
   ...
   */
  configPatch?: Partial<OpunclehConfig>;
  defaultModel?: string;
  notes?: string[];
};
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). Opuncleh uses this to expose
 ...
 *
 * Runs after Opuncleh resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common Opuncleh auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and Opuncleh wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before Opuncleh builds its generic
...
/**
 * Provider-owned stream wrapper hook after Opuncleh applies its generic
 ...
 *
 * Return `true` or `false` to override Opuncleh's built-in provider cache TTL
 ...
 *
 * Runs only after Opuncleh exhausts normal env/profile/config auth resolution
 ...
 *
 * Runs after Opuncleh loads the discovered model catalog and merges configured
...
export type ProviderPlugin = {
   ...
   *
   * Opuncleh calls this only from async model resolution paths. After it
  ...
  /**
   * Provider-owned stream wrapper applied after generic Opuncleh wrappers.
   *
   ...
   *
   * Called after Opuncleh resolves the raw configured credential but before the
   ...
   * hint than Opuncleh's generic auth-store guidance.
   */
   ...
   * `errorMessage` when Opuncleh should surface a provider-specific hint for
   ...
   * merging. Opuncleh deduplicates by `provider/id`, so plugins only need to
   ...
   *
   * Opuncleh uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `opuncleh doctor`.
   *
   ...
   *
   * Opuncleh calls this before falling back to the shared `pi-ai` OAuth
   ...
   * path. Return nothing to keep Opuncleh's generic error text.
   */
  buildAuthDoctorHint?: (
...
export type OpunclehPluginGatewayMethod = {
  method: string;
...
export type PluginCommandContext = {
  ...
  commandBody: string;
  /** Current Opuncleh configuration */
  config: OpunclehConfig;
 ...
 */
export type OpunclehPluginCommandDefinition = {
...
export type OpunclehPluginHttpRouteAuth = "gateway" | "plugin";
export type OpunclehPluginHttpRouteMatch = "exact" | "prefix";

export type OpunclehPluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type OpunclehPluginHttpRouteParams = {
  path: string;
  handler: OpunclehPluginHttpRouteHandler;
  auth: OpunclehPluginHttpRouteAuth;
  match?: OpunclehPluginHttpRouteMatch;
  replaceExisting?: boolean;
};

export type OpunclehPluginCliContext = {
  program: Command;
  config: OpunclehConfig;
  workspaceDir?: string;
  logger: PluginLogger;
...
export type OpunclehPluginCliRegistrar = (ctx: OpunclehPluginCliContext) => void | Promise<void>;

export type OpunclehPluginServiceContext = {
  config: OpunclehConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
...
export type OpunclehPluginService = {
  id: string;
  start: (ctx: OpunclehPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: OpunclehPluginServiceContext) => void | Promise<void>;
};

export type OpunclehPluginChannelRegistration = {
  plugin: ChannelPlugin;
};

export type OpunclehPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  ...
  register?: (api: OpunclehPluginApi) => void | Promise<void>;
  activate?: (api: OpunclehPluginApi) => void | Promise<void>;
};

export type OpunclehPluginModule =
  | OpunclehPluginDefinition
  | ((api: OpunclehPluginApi) => void | Promise<void>);

export type PluginRegistrationMode = "full" | "setup-only" | "setup-runtime";

export type OpunclehPluginApi = {
  id: string;
  name: string;
  version?: string;
  description?: string;
  ...
  logger: PluginLogger;
  registerTool: (
    tool: AnyAgentTool | OpunclehPluginToolFactory,
    opts?: OpunclehPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: OpunclehPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: OpunclehPluginHttpRouteParams) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: OpunclehPluginChannelRegistration | ChannelPlugin) => void;
  registerGatewayMethod: (method: string, handler: GatewayRequestHandler) => void;
  registerCli: (registrar: OpunclehPluginCliRegistrar, opts?: { commands?: string[] }) => void;
  registerService: (service: OpunclehPluginService) => void;
   ...
   */
  registerCommand: (command: OpunclehPluginCommandDefinition) => void;
...
export type PluginFormat = "opuncleh" | "bundle";
