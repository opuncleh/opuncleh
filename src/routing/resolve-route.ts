...
function listAgents(cfg: OpunclehConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: OpunclehConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<OpunclehConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: OpunclehConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: OpunclehConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: OpunclehConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<OpunclehConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  OpunclehConfig,
  {
    bindingsRef: OpunclehConfig["bindings"];
    agentsRef: OpunclehConfig["agents"];
    sessionRef: OpunclehConfig["session"];
...
function resolveRouteCacheForConfig(cfg: OpunclehConfig): Map<string, ResolvedAgentRoute> {
