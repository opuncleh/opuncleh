...
describe("resolveAgentRoute", () => {
  const resolveDiscordGuildRoute = (cfg: OpunclehConfig) =>
    resolveAgentRoute({
      cfg,
      channel: "discord",
      accountId: "default",
  ...
  test("defaultAgentId is used when no binding matches", () => {
    const cfg: OpunclehConfig = {
      agents: {
        list: [{ id: "home", default: true, workspace: "~/opuncleh-home" }],
      },
    };
    const route = resolveAgentRoute({
      cfg,
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<OpunclehConfig["bindings"]>[number];
