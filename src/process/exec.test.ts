...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        OPUNCLEH_BASE_ENV: "base",
        OPUNCLEH_TO_REMOVE: undefined,
      },
      env: {
        OPUNCLEH_TEST_ENV: "ok",
      },
    });

    expect(resolved.OPUNCLEH_BASE_ENV).toBe("base");
    expect(resolved.OPUNCLEH_TEST_ENV).toBe("ok");
    expect(resolved.OPUNCLEH_TO_REMOVE).toBeUndefined();
    expect(resolved.OPUNCLEH_CLI).toBe(OPUNCLEH_CLI_ENV_VALUE);
  });
