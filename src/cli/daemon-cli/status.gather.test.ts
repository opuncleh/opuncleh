...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    OPUNCLEH_STATE_DIR: "/tmp/opuncleh-daemon",
    OPUNCLEH_CONFIG_PATH: "/tmp/opuncleh-daemon/opuncleh.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.OPUNCLEH_STATE_DIR ?? "/tmp/opuncleh-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.OPUNCLEH_CONFIG_PATH ?? `${stateDir}/opuncleh.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/opuncleh-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.OPUNCLEH_STATE_DIR = "/tmp/opuncleh-cli";
    process.env.OPUNCLEH_CONFIG_PATH = "/tmp/opuncleh-cli/opuncleh.json";
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.OPUNCLEH_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.OPUNCLEH_GATEWAY_PORT ?? "missing-port",
    }));
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.OPUNCLEH_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
  ...
  it("surfaces stale gateway listener pids from restart health inspection", async () => {
    inspectGatewayRestart.mockResolvedValueOnce({
      ...
      portUsage: {
        port: 19001,
        status: "busy",
        listeners: [{ pid: 9000, ppid: 8999, commandLine: "opuncleh-gateway" }],
        hints: [],
      },
      healthy: false,
