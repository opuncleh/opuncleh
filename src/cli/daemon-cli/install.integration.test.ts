...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("opuncleh-daemon-install-int-");
    configPath = path.join(tempHome, "opuncleh.json");
    process.env.HOME = tempHome;
    process.env.OPUNCLEH_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.OPUNCLEH_GATEWAY_TOKEN).toBeUndefined();
  });
});
