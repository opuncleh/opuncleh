...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "opuncleh-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "opuncleh-bundled-skills-test-"));
    process.env.OPUNCLEH_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
