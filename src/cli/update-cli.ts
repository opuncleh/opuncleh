...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Opuncleh and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["opuncleh update", "Update a source checkout (git)"],
        ["opuncleh update --channel beta", "Switch to beta channel (git + npm)"],
        ["opuncleh update --channel dev", "Switch to dev channel (git + npm)"],
        ["opuncleh update --tag beta", "One-off update to a dist-tag or version"],
        ["opuncleh update --tag main", "One-off package install from GitHub main"],
        ["opuncleh update --dry-run", "Preview actions without changing anything"],
        ["opuncleh update --no-restart", "Update without restarting the service"],
        ["opuncleh update --json", "Output result as JSON"],
        ["opuncleh update --yes", "Non-interactive (accept downgrade prompts)"],
        ["opuncleh update wizard", "Interactive update wizard"],
        ["opuncleh --update", "Shorthand for opuncleh update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run opuncleh update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.opuncleh.com/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["opuncleh update status", "Show channel + version status."],
          ["opuncleh update status --json", "JSON output."],
          ["opuncleh update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.opuncleh.com/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
