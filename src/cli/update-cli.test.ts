...
import type { OpunclehConfig, ConfigFileSnapshot } from "../config/types.opuncleh.js";
...
vi.mock("../infra/opuncleh-root.js", () => ({
  resolveOpunclehPackageRoot: vi.fn(),
}));
...
const { resolveOpunclehPackageRoot } = await import("../infra/opuncleh-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/opuncleh-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as OpunclehConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/opuncleh-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveOpunclehPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("opuncleh-update");
    setTty(false);
  ...
  const setupUpdatedRootRefresh = (params?: {
    ...
    const root = createCaseDir("opuncleh-updated-root");
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(resolveOpunclehPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("opuncleh");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/opuncleh-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "opuncleh-gateway" }],
      hints: [],
    });
    ...
    const cases = [
      {
        ...
        assert: () => {
          ...
          expect(logs.join("\n")).toContain("Opuncleh update status");
        },
      },
      {
        name: "json output",
  ...
  it.each([
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as OpunclehConfig,
        });
      },
    ...
    async ({ mode, options, prepare, expectedChannel, expectedTag, expectedPersistedChannel }) => {
      ...
      if (expectedChannel !== undefined) {
        ...
        expect(runGatewayUpdate).not.toHaveBeenCalled();
        expect(runCommandWithTimeout).toHaveBeenCalledWith(
          ["npm", "i", "-g", "opuncleh@latest", "--no-fund", "--no-audit", "--loglevel=error"],
          expect.any(Object),
        );
      }
    ...
    for (const scenario of [
      {
        ...
        run: async () => {
          mockPackageInstallStatus(createCaseDir("opuncleh-update"));
          await updateCommand({ tag: "next" });
        },
        expectedSpec: "opuncleh@next",
      },
      {
        name: "main shorthand",
        run: async () => {
        ...
        expectedSpec: "github:opuncleh/opuncleh#main",
      },
      {
        name: "explicit git package spec",
        run: async () => {
          mockPackageInstallStatus(createCaseDir("opuncleh-update"));
          await updateCommand({ yes: true, tag: "github:opuncleh/opuncleh#main" });
        },
        expectedSpec: "github:opuncleh/opuncleh#main",
      },
      {
        name: "OPUNCLEH_UPDATE_PACKAGE_SPEC override",
        run: async () => {
          mockPackageInstallStatus(createCaseDir("opuncleh-update"));
          await withEnvAsync(
        ...
        expectedSpec: "http://10.211.55.2:8138/opuncleh-next.tgz",
      },
    ]) {
      vi.clearAllMocks();
      readPackageName.mockResolvedValue("opuncleh");
  ...
  it("prepends portable Git PATH for package updates on Windows", async () => {
    ...
    const tempDir = createCaseDir("opuncleh-update");
    const localAppData = createCaseDir("opuncleh-localappdata");
        ...
        await withEnvAsync(
          {
            ...
            OPUNCLEH_CONFIG_PATH: "./config/opuncleh.json",
          },
          async () => {
            await updateCommand({});
          },
      ...
      expectedOptions: (root: string) =>
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            OPUNCLEH_STATE_DIR: path.resolve("./state"),
            OPUNCLEH_CONFIG_PATH: path.resolve("./config/opuncleh.json"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
    ...
    },
    {
      ...
      expectedOptions: (_root: string, context?: { originalCwd: string }) =>
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            OPUNCLEH_STATE_DIR: path.resolve(context?.originalCwd ?? process.cwd(), "./state"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ OPUNCLEH_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.OPUNCLEH_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("opuncleh-update-wizard");
    await withEnvAsync({ OPUNCLEH_GIT_DIR: tempDir }, async () => {
      setTty(true);
