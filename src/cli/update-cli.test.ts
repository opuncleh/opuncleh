...
describe("update-cli", () => {
  ...
  it("honors --tag override", async () => {
    ...
    expect(runGatewayUpdate).not.toHaveBeenCalled();
    expect(runCommandWithTimeout).toHaveBeenCalledWith(
      ["npm", "i", "-g", "opuncleh@next", "--no-fund", "--no-audit", "--loglevel=error"],
      expect.any(Object),
    );
  });
  ...
  it("uses OPUNCLEH_UPDATE_PACKAGE_SPEC for package updates", async () => {
    const tempDir = createCaseDir("opuncleh-update");
    mockPackageInstallStatus(tempDir);
    ...
    expect(runCommandWithTimeout).toHaveBeenCalledWith(
      [
        ...
        "i",
        "-g",
        "http://10.211.55.2:8138/opuncleh-next.tgz",
        "--no-fund",
        "--no-audit",
        "--loglevel=error",
      ],
