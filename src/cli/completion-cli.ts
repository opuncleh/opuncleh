...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# Opuncleh Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(opuncleh completion --shell zsh)
  return (
...
function updateCompletionProfile(
  ...
  const block = `# Opuncleh Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(opuncleh completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/completion", "docs.opuncleh.io/cli/completion")}\n`,
    )
    .addOption(
    ...
    .option(
      "--write-state",
      "Write completion scripts to $OPUNCLEH_STATE_DIR/completions (no stdout)",
    )
...
export async function installCompletion(shell: string, yes: boolean, binName = "opuncleh") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c opuncleh -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'opuncleh gateway' and not 'opuncleh other gateway', we need to check the sequence.
