...
const CONFIG_SET_EXAMPLE_PROVIDER = formatCliCommand(
  "opuncleh config set secrets.providers.vault --provider-source file --provider-path /etc/opuncleh/secrets.json --provider-mode json",
);
...
function formatDoctorHint(message: string): string {
  return `Run \`${formatCliCommand("opuncleh doctor")}\` ${message}`;
}
...
function collectDryRunSchemaErrors(config: OpunclehConfig): ConfigSetDryRunError[] {
...
export async function runConfigSet(opts: {
  ...
  try {
    ...
    const nextConfig = next as OpunclehConfig;
...
export async function runConfigValidate(opts: { json?: boolean; runtime?: RuntimeEnv } = {}) {
  const runtime = opts.runtime ?? defaultRuntime;
  let outputPath = CONFIG_PATH ?? "opuncleh.json";
...
export function registerConfigCli(program: Command) {
  const cmd = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.opuncleh.com/cli/config")}\n`,
    )
    .option(
      "--section <section>",
  ...
  cmd
    ...
    .option(
      "--dry-run",
      "Validate changes without writing opuncleh.json (checks run in builder/json/batch modes; exec SecretRefs are skipped unless --allow-exec is set)",
      false,
    )
    .option(
      "--allow-exec",
