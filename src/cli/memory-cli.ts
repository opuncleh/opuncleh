    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["opuncleh memory status", "Show index and provider status."],
          ["opuncleh memory status --deep", "Probe embedding provider readiness."],
          ["opuncleh memory index --force", "Force a full reindex."],
          ['opuncleh memory search "meeting notes"', "Quick search using positional query."],
          [
            'opuncleh memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          ["opuncleh memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.opuncleh.com/cli/memory")}\n`,
    );
