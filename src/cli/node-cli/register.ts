    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "opuncleh node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["opuncleh node status", "Check node host service status."],
          ["opuncleh node install", "Install the node host service."],
          ["opuncleh node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.opuncleh.com/cli/node")}\n`,
    );
