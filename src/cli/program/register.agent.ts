...
${formatHelpExamples([
  ['opuncleh agent --to +15555550123 --message "status update"', "Start a new session."],
  ['opuncleh agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'opuncleh agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'opuncleh agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['opuncleh agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'opuncleh agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.opuncleh.com/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.opuncleh.com/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['opuncleh agents set-identity --agent main --name "Opuncleh" --emoji "🦞"', "Set name + emoji."],
  ["opuncleh agents set-identity --agent main --avatar avatars/opuncleh.png", "Set avatar path."],
  [
    "opuncleh agents set-identity --workspace ~/.opuncleh/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "opuncleh agents set-identity --identity-file ~/.opuncleh/workspace/IDENTITY.md --agent main",
