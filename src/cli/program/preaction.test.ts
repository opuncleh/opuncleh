...
beforeEach(() => {
  ...
  originalHideBanner = process.env.OPUNCLEH_HIDE_BANNER;
  delete process.env.NODE_NO_WARNINGS;
  delete process.env.OPUNCLEH_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.OPUNCLEH_HIDE_BANNER = originalHideBanner;
  }
});
...
describe("registerPreActionHooks", () => {
  ...
  function buildProgram() {
    const program = new Command().name("opuncleh");
  ...
  it("handles debug mode and plugin-required command preaction", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "opuncleh", "status", "--debug"],
    });
    ...
    expect(process.title).toBe("opuncleh-status");

    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["message", "send"],
      processArgv: ["node", "opuncleh", "message", "send"],
    });
  ...
  it("keeps setup alias and channels add manifest-first", async () => {
    await runPreAction({
      parseArgv: ["onboard"],
      processArgv: ["node", "opuncleh", "onboard"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "opuncleh", "channels", "add"],
    });
  ...
  it("skips help/version preaction and respects banner opt-out", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "opuncleh", "--version"],
    });
    ...
    vi.clearAllMocks();
    process.env.OPUNCLEH_HIDE_BANNER = "1";

    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "opuncleh", "status"],
    });
  ...
  it("applies --json stdout suppression only for explicit JSON output commands", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "opuncleh", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "opuncleh", "update", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "opuncleh", "config", "set", "gateway.auth.mode", "{bad", "--json"],
    });
  ...
  it("bypasses config guard for config validate", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "opuncleh", "config", "validate"],
    });
  ...
  it("bypasses config guard for config validate when root option values are present", async () => {
    await runPreAction({
      parseArgv: ["config", "validate"],
      processArgv: ["node", "opuncleh", "--profile", "work", "config", "validate"],
    });
  ...
  it("bypasses config guard for backup create", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "opuncleh", "backup", "create", "--json"],
    });
