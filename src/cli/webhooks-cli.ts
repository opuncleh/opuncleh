...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.opuncleh.com/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + Opuncleh hooks")
    ...
    .option("--hook-url <url>", "Opuncleh hook URL")
    .option("--hook-token <token>", "Opuncleh hook token")
