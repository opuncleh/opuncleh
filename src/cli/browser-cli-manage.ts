...
async function runBrowserToggle(
  ...
  const name = status.profile ?? "opuncleh";
...
export function registerBrowserManageCommands(
  ...
  browser
    ...
    .action(async (_opts, cmd) => {
      ...
      await runBrowserCommand(async () => {
        ...
        defaultRuntime.log(
          [
            `profile: ${status.profile ?? "opuncleh"}`,
        ...
        defaultRuntime.log(
          profiles
            .map((p) => {
              ...
              const driver = p.driver !== "opuncleh" ? ` [${p.driver}]` : "";
  ...
  browser
    ...
    .option("--driver <driver>", "Profile driver (opuncleh|existing-session). Default: opuncleh")
    .action(
      async (
        opts: {
          name: string;
