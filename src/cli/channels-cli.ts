    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["opuncleh channels list", "List configured channels and auth profiles."],
          ["opuncleh channels status --probe", "Run channel status checks and probes."],
          [
            "opuncleh channels add --channel telegram --token <token>",
            "Add or update a channel account non-interactively.",
          ],
          ["opuncleh channels login --channel whatsapp", "Link a WhatsApp Web account."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink(
          "/cli/channels",
          "docs.opuncleh.io/cli/channels",
        )}\n`,
    );
