...
const buildGatewayInstallPlan = vi.fn(
  async (params: { port: number; token?: string; env?: NodeJS.ProcessEnv }) => ({
    ...
    environment: {
      OPUNCLEH_GATEWAY_PORT: String(params.port),
      ...(params.token ? { OPUNCLEH_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    process.env.OPUNCLEH_STATE_DIR = "/tmp/opuncleh-cli-state";
    process.env.OPUNCLEH_CONFIG_PATH = "/tmp/opuncleh-cli-state/opuncleh.json";
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        ...
        OPUNCLEH_STATE_DIR: "/tmp/opuncleh-daemon-state",
        OPUNCLEH_CONFIG_PATH: "/tmp/opuncleh-daemon-state/opuncleh.json",
        OPUNCLEH_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.opuncleh.gateway.plist",
    });
