...
const TEMP_MEDIA_PREFIX = "opuncleh-media-";
...
function createGroqAudioConfig(): OpunclehConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): OpunclehConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): OpunclehConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: OpunclehConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredOpunclehTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        OPUNCLEH_AGENT_DIR: isolatedAgentDir,
