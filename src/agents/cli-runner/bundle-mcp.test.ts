...
describe("prepareCliBundleMcpConfig", () => {
  it("injects a merged --mcp-config overlay for claude-cli", async () => {
    ...
    try {
      const homeDir = await tempHarness.createTempDir("opuncleh-cli-bundle-mcp-home-");
      const workspaceDir = await tempHarness.createTempDir("opuncleh-cli-bundle-mcp-workspace-");
