...
import { resolvePreferredOpunclehTmpDir } from "../infra/tmp-opuncleh-dir.js";
...
async function withOutsideHardlinkInOpunclehTmp<T>(
  params: {
...
describe("resolveSandboxedMediaSource", () => {
  ...
  it.each([
    ...
    {
      name: "absolute paths under host tmp outside opuncleh tmp root",
      media: path.join(os.tmpdir(), "outside-opuncleh", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked Opuncleh tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked Opuncleh tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInOpunclehTmp(
      {
        openClawTmpDir,
  ...
  it("rejects symlinked Opuncleh tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInOpunclehTmp(
