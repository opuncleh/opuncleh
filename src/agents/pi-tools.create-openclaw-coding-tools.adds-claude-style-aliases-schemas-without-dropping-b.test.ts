...
const defaultTools = createOpunclehCodingTools({ senderIsOwner: true });
...
describe("createOpunclehCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: OpunclehConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createOpunclehCodingTools({
    ...
    const anthropicTools = createOpunclehCodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: OpunclehConfig = {
      tools: {
        exec: {
    ...
    const allowed = createOpunclehCodingTools({
    ...
    const denied = createOpunclehCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createOpunclehCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
