 ...
 *
 * @see https://github.com/opuncleh/opuncleh/issues/47711
 */
...
export async function recoverOrphanedSubagentSessions(params: {
  ...
  const configChangePattern = /opuncleh\.json|opuncleh gateway restart|config\.patch/i;
      ...
      try {
        ...
        const resumed = await resumeOrphanedSession({
          ...
          configChangeHint: configChangeDetected
            ? "\n\n[config changes from your previous run were already applied — do not re-modify opuncleh.json or restart the gateway]"
            : undefined,
          originalRunId: runId,
