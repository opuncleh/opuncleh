...
const GOOGLE_LIVE_SINGLE_KEY = "OPUNCLEH_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "OPUNCLEH_LIVE_ANTHROPIC_KEY",
    listVar: "OPUNCLEH_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "OPUNCLEH_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `OPUNCLEH_LIVE_${base}_KEY`;
