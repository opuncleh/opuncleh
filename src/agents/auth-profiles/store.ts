 ...
 * A common mistake — caused by the similarity with the `opuncleh.json`
...
function normalizeRawCredentialEntry(raw: Record<string, unknown>): Partial<AuthProfileCredential> {
  ...
  // mode → type alias (opuncleh.json uses "mode"; auth-profiles.json uses "type")
...
function loadAuthProfileStoreForAgent(
  ...
  const forceReadOnly = process.env.OPUNCLEH_AUTH_STORE_READONLY === "1";
