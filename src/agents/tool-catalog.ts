...
type CoreToolDefinition = {
  ...
  includeInOpunclehGroup?: boolean;
};
...
function buildCoreToolGroupMap() {
  ...
  const opunclehTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInOpunclehGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:opuncleh": opunclehTools,
