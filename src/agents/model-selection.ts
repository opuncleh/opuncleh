...
export function isCliProvider(provider: string, cfg?: OpunclehConfig): boolean {
  const normalized = normalizeProviderId(provider);
...
function resolveAllowedFallbacks(params: { cfg: OpunclehConfig; agentId?: string }): string[] {
  if (params.agentId) {
