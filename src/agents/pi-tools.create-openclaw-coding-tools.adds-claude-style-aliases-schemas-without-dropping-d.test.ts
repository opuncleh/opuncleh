...
describe("createOpunclehCodingTools", () => {
  it("returns image metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "opuncleh-read-"));
    try {
      ...
      const contents = "Hello from opuncleh read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    ...
    const tools = createOpunclehCodingTools({ sandbox });
