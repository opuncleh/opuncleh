...
function resolveAttachmentLimits(config: OpunclehConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".opuncleh", "attachments");
  const relDir = path.posix.join(".opuncleh", "attachments", attachmentId);
