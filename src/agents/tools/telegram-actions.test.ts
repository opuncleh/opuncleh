...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): OpunclehConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as OpunclehConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): OpunclehConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as OpunclehConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: OpunclehConfig, accountId = "media") {
    await handleTelegramAction(
