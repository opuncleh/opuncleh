...
function findTool(name: string, config: OpunclehConfig) {
  const allTools = createOpunclehTools({ config, sandboxed: true });
...
async function prepareAndActivate(params: { config: OpunclehConfig; env?: NodeJS.ProcessEnv }) {
...
describe("opuncleh tools runtime web metadata wiring", () => {
