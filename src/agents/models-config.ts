...
import { planOpunclehModelsJson } from "./models-config.plan.js";
...
function resolveModelsConfigInput(config?: OpunclehConfig): {
  config: OpunclehConfig;
  sourceConfigForSecrets: OpunclehConfig;
} {
...
export async function ensureOpunclehModelsJson(
  config?: OpunclehConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const resolved = resolveModelsConfigInput(config);
  const cfg = resolved.config;
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveOpunclehAgentDir();
  ...
  return await withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planOpunclehModelsJson({
      cfg,
