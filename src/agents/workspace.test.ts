...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses OPUNCLEH_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      OPUNCLEH_HOME: "/srv/opuncleh-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/opuncleh-home"), ".opuncleh", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".opuncleh", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("opuncleh-workspace-");
  ...
  it("migrates legacy onboardingCompletedAt markers to setupCompletedAt", async () => {
    const tempDir = await makeTempWorkspace("opuncleh-workspace-");
    await fs.mkdir(path.join(tempDir, ".opuncleh"), { recursive: true });
    await fs.writeFile(
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "opuncleh-workspace-hardlink-"));
    try {
