...
function mockContextDeps(params: {
  ...
  vi.doMock("./agent-paths.js", () => ({
    resolveOpunclehAgentDir: () => "/tmp/opuncleh-agent",
  }));
...
describe("lookupContextTokens", () => {
  ...
  it("only warms eagerly for startup commands that need model metadata", async () => {
    ...
    try {
      for (const scenario of [
        {
          argv: ["node", "opuncleh", "--profile", "--", "config", "validate"],
          expectedCalls: 1,
        },
        {
          argv: ["node", "opuncleh", "logs", "--limit", "5"],
          expectedCalls: 0,
        },
        {
        ...
        {
          argv: ["node", "opuncleh", "gateway", "status", "--json"],
          expectedCalls: 0,
        },
      ]) {
        vi.resetModules();
  ...
  it("retries config loading after backoff when an initial load fails", async () => {
    ...
    const argvSnapshot = process.argv;
    process.argv = ["node", "opuncleh", "config", "validate"];
    try {
      const { lookupContextTokens } = await import("./context.js");
