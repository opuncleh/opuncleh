...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["OPUNCLEH_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "opuncleh-subagent-"));
  ...
  it("uses isolated temp state when OPUNCLEH_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "opuncleh-test-state"));
  });
});
