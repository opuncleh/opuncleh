...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseOpunclehManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
  const spec = applyOpunclehManifestInstallCommonFields<SkillInstallSpec>(
    {
...
): OpunclehSkillMetadata | undefined {
