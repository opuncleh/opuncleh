    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "opuncleh.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "opuncleh.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "opuncleh.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("opuncleh-");
  const acpxRoot = await tempDirs.make("opuncleh-acpx-plugin-");
  const helperRoot = await tempDirs.make("opuncleh-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("opuncleh-");
  const pluginRoot = await tempDirs.make("opuncleh-plugin-");
  const outsideDir = await tempDirs.make("opuncleh-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("opuncleh-");
    const pluginRoot = await tempDirs.make("opuncleh-claude-bundle-");
