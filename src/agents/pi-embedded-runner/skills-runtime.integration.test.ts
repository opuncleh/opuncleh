...
const originalBundledDir = process.env.OPUNCLEH_BUNDLED_PLUGINS_DIR;
...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("opuncleh-bundled-");
  const workspaceDir = await createTempDir("opuncleh-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: OpunclehConfig) {
  const { bundledPluginsDir, workspaceDir } = await setupBundledDiffsPlugin();
  process.env.OPUNCLEH_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
...
afterEach(async () => {
  process.env.OPUNCLEH_BUNDLED_PLUGINS_DIR = originalBundledDir;
  clearPluginManifestRegistryCache();
  await Promise.all(
