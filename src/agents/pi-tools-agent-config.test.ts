...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "opuncleh-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: OpunclehConfig) {
    return createOpunclehCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<OpunclehConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<OpunclehConfig["agents"]>["list"]>[number]["tools"];
  }): OpunclehConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createOpunclehCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createOpunclehCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createOpunclehCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createOpunclehCodingTools({
      config: cfg,
    ...
    const bobTools = createOpunclehCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createOpunclehCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createOpunclehCodingTools({
      config: cfg,
