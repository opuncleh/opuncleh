...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.OPUNCLEH_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".opuncleh", `workspace-${profile}`);
  }
  return path.join(home, ".opuncleh", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".opuncleh";
