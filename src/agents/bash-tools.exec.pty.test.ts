...
test("exec sets OPUNCLEH_SHELL in pty mode", async () => {
  const tool = createExecTool({ allowBackground: false, security: "full", ask: "off" });
  const result = await tool.execute("toolcall-opuncleh-shell", {
    command: "node -e \"process.stdout.write(process.env.OPUNCLEH_SHELL || '')\"",
    pty: true,
  });
