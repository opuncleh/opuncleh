...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("opuncleh-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("opuncleh-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("opuncleh-sbx-");
