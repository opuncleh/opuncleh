...
import { markOpunclehExecEnv } from "../../infra/opuncleh-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "opuncleh.sandbox=1");
  args.push("--label", `opuncleh.sessionKey=${params.scopeKey}`);
  args.push("--label", `opuncleh.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `opuncleh.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markOpunclehExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "opuncleh.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`opuncleh sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`opuncleh sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("opuncleh sandbox recreate --all");
}
