...
describe("provider attribution", () => {
  it("resolves the canonical Opuncleh product and runtime version", () => {
    const identity = resolveProviderAttributionIdentity({
      OPUNCLEH_VERSION: "2026.3.99",
    });
  ...
  it("returns a hidden-spec OpenAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("openai", { OPUNCLEH_VERSION: "2026.3.14" })).toEqual({
      provider: "openai",
      enabledByDefault: true,
      ...
      headers: {
        originator: "opuncleh",
        "User-Agent": "opuncleh/2026.3.14",
      },
    });
    expect(resolveProviderAttributionHeaders("openai", { OPUNCLEH_VERSION: "2026.3.14" })).toEqual({
      originator: "opuncleh",
      "User-Agent": "opuncleh/2026.3.14",
    });
  });
  ...
  it("returns a hidden-spec OpenAI Codex attribution policy", () => {
    expect(
      resolveProviderAttributionPolicy("openai-codex", { OPUNCLEH_VERSION: "2026.3.14" }),
    ).toEqual({
  ...
  it("lists the current attribution support matrix", () => {
    expect(
      listProviderAttributionPolicies({ OPUNCLEH_VERSION: "2026.3.14" }).map((policy) => [
        policy.provider,
