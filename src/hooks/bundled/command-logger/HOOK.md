...
homepage: https://docs.opuncleh.io/automation/hooks#command-logger
metadata:
  {
    "opuncleh":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.opuncleh/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.opuncleh/logs/commands.log ~/.opuncleh/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/opuncleh`:
   ```
   /home/username/.opuncleh/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
