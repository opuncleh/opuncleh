...
This directory contains hooks that ship with Opuncleh. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.opuncleh/opuncleh.json`:
...
2. Restart gateway: `pkill -9 -f 'opuncleh.*gateway' && pnpm opuncleh gateway`
3. Enable the hook: `opuncleh hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.opuncleh.io/automation/hooks
