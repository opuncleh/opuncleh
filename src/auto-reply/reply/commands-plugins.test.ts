...
const workspaceHarness = createCommandWorkspaceHarness("opuncleh-command-plugins-");

async function createClaudeBundlePlugin(params: { workspaceDir: string; pluginId: string }) {
  const pluginDir = path.join(params.workspaceDir, ".opuncleh", "extensions", params.pluginId);
...
function buildCfg(): OpunclehConfig {
  return {
    commands: {
      text: true,
      plugins: true,
...
describe("handleCommands /plugins", () => {
  ...
  it("lists discovered plugins and inspects plugin details", async () => {
    await withTempHome("opuncleh-command-plugins-home-", async () => {
