...
const { createOpunclehCodingToolsMock } = vi.hoisted(() => ({
  createOpunclehCodingToolsMock: vi.fn(() => []),
}));
...
describe("resolveCommandsSystemPromptBundle", () => {
  beforeEach(() => {
    createOpunclehCodingToolsMock.mockClear();
  ...
  it("opts command tool builds into gateway subagent binding", async () => {
    ...
    expect(createOpunclehCodingToolsMock).toHaveBeenCalledWith(
