...
import { scheduleGatewaySigusr1Restart, triggerOpunclehRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Opuncleh in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerOpunclehRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Opuncleh via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
