...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "opuncleh-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop opuncleh!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@opuncleh_bot", { botUsername: "opuncleh_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@opuncleh_bot", { botUsername: "opuncleh_bot" })).toBe(true);
