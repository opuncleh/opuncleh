...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    opunclehHome: process.env.OPUNCLEH_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("OPUNCLEH_HOME", snapshot.opunclehHome);
  restoreKey("OPUNCLEH_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.OPUNCLEH_STATE_DIR = join(home, ".opuncleh");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "opuncleh-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".opuncleh", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): OpunclehConfig {
  return {
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-5" },
        workspace: join(home, "opuncleh"),
