...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "opuncleh-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): OpunclehConfig {
    const defaults: NonNullable<NonNullable<OpunclehConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: OpunclehConfig) {
