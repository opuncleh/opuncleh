...
async function requireRiskAcknowledgement(params: {
  ...
  await params.prompter.note(
    [
      ...
      "",
      "Opuncleh is a hobby project and still in beta. Expect sharp edges.",
      "By default, Opuncleh is a personal agent: one trusted operator boundary.",
      "This bot can read files and run actions if tools are enabled.",
      "A bad prompt can trick it into doing unsafe things.",
      "",
      "Opuncleh is not a hostile multi-tenant boundary by default.",
      "If multiple users can message one tool-enabled agent, they share that delegated tool authority.",
      "",
      "If you’re not comfortable with security hardening and access control, don’t run Opuncleh.",
      ...
      "",
      "Must read: https://docs.opuncleh.io/gateway/security",
    ].join("\n"),
    "Security",
  );
...
export async function runOnboardingWizard(
  ...
  await prompter.intro("Opuncleh onboarding");
    ...
    if (snapshot.issues.length > 0) {
      await prompter.note(
        [
          ...
          "",
          "Docs: https://docs.opuncleh.io/gateway/configuration",
        ].join("\n"),
        "Config issues",
      );
    }
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("opuncleh doctor")}\` to repair it, then re-run onboarding.`,
    );
    runtime.exit(1);
    return;
  }
  ...
  let nextConfig: OpunclehConfig = applyOnboardingLocalWorkspaceConfig(baseConfig, workspaceDir);
