...
function withFakeCli(versionOutput: string): { root: string; cliPath: string } {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "opuncleh-install-sh-"));
  const cliPath = path.join(root, "opuncleh");
...
OPUNCLEH_BIN="$FAKE_OPUNCLEH_BIN"
resolve_opuncleh_version`,
    ],
    {
      ...
      env: {
        ...process.env,
        FAKE_OPUNCLEH_BIN: cliPath,
        OPUNCLEH_INSTALL_SH_NO_RUN: "1",
      },
    },
  );
  return output.trim();
...
describe("install.sh version resolution", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("Opuncleh 2026.3.10 (abcdef0)");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("Opuncleh dev's build");
      tempRoots.push(fixture.root);

      expect(resolveVersionFromInstaller(fixture.cliPath)).toBe("Opuncleh dev's build");
    },
  );
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      const hostileCwd = fs.mkdtempSync(path.join(os.tmpdir(), "opuncleh-install-stdin-"));
