  ...
  return {
    agents: {
      defaults: {
        ...
        workspace: "/tmp/opuncleh",
      },
    },
    session: { store: "/tmp/opuncleh-sessions.json" },
  ...
  it(
    ...
    async () => {
      const cfg = createMentionRequiredGuildConfig({
        messages: {
          responsePrefix: "PFX",
          groupChat: { mentionPatterns: ["\\bopuncleh\\b"] },
        },
      });
      ...
      await handler(
        createGuildMessageEvent({ messageId: "m2", content: "opuncleh: hello" }),
        client,
      );
      ...
      createGuildMessageEvent({
        ...
        messagePatch: {
          referencedMessage: {
            ...
            author: { id: "bot-id", bot: true, username: "Opuncleh" },
          },
        },
        eventPatch: {
