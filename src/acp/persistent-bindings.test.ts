...
type ConfiguredBinding = NonNullable<OpunclehConfig["bindings"]>[number];
...
function createCfgWithBindings(
  ...
  overrides?: Partial<OpunclehConfig>,
): OpunclehConfig {
  return {
    ...baseCfg,
    ...overrides,
...
function resolveBindingRecord(cfg: OpunclehConfig, overrides: Partial<BindingRecordInput> = {}) {
...
describe("resolveConfiguredAcpBindingRecord", () => {
  ...
  it("derives configured binding cwd from an explicit agent workspace", () => {
    const cfg = createCfgWithBindings(
      ...
      {
        agents: {
          list: [{ id: "codex", workspace: "/workspace/opuncleh" }, { id: "claude" }],
        },
      },
    );
    const resolved = resolveBindingRecord(cfg);
