...
const createTempDir = () => tempDirs.make("opuncleh-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets OPUNCLEH_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "opuncleh",
    });

    expect(env.OPUNCLEH_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("opuncleh");
  });

  it("overrides pre-existing OPUNCLEH_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      OPUNCLEH_SHELL: "wrong",
    });
    expect(env.OPUNCLEH_SHELL).toBe("acp-client");
  });
  ...
  it("preserves OPUNCLEH_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        OPUNCLEH_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["OPUNCLEH_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default Opuncleh bridge", () => {
    ...
    expect(env.OPUNCLEH_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.OPUNCLEH_SHELL).toBe("acp-client");
  });
...
describe("shouldStripProviderAuthEnvVarsForAcpServer", () => {
  it("strips provider auth env vars for the default bridge", () => {
    ...
    expect(
      shouldStripProviderAuthEnvVarsForAcpServer({
        serverCommand: "opuncleh",
        serverArgs: ["acp"],
        defaultServerCommand: "opuncleh",
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("OPUNCLEH_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "opuncleh", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "opuncleh", "dist", "entry.js");
    const shimPath = path.join(dir, "opuncleh.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\opuncleh\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/opuncleh-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/opuncleh-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/opuncleh-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
