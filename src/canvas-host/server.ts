...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.opunclehCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.opunclehCanvasA2UIAction &&
        typeof window.opunclehCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.opunclehSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("opuncleh:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Opuncleh node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.opunclehSendUserAction === "function"
        ? window.opunclehSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.OPUNCLEH_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.OPUNCLEH_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Opuncleh Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
