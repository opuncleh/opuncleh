...
describe("canvas host state dir defaults", () => {
  it("uses OPUNCLEH_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("opuncleh-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Opuncleh Canvas");
      } finally {
        await handler.close();
      }
    });
