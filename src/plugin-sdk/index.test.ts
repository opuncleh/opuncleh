...
describe("plugin-sdk exports", () => {
  ...
  it("emits importable bundled subpath entries", { timeout: 240_000 }, async () => {
    const outDir = await fs.mkdtemp(path.join(os.tmpdir(), "opuncleh-plugin-sdk-build-"));
    const fixtureDir = await fs.mkdtemp(path.join(os.tmpdir(), "opuncleh-plugin-sdk-consumer-"));
...
`,
      ...
      const packageDir = path.join(fixtureDir, "opuncleh");
      ...
      await fs.symlink(packageDir, path.join(consumerDir, "node_modules", "opuncleh"), "dir");
      await fs.writeFile(
        consumerEntry,
        [
