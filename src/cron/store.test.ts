...
const { makeStorePath } = createCronStoreHarness({ prefix: "opuncleh-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses OPUNCLEH_HOME for tilde expansion", () => {
    vi.stubEnv("OPUNCLEH_HOME", "/srv/opuncleh-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/opuncleh-home", "cron", "jobs.json"));
  });
});
