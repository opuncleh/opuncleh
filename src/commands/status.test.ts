...
beforeAll(() => {
  envSnapshot = captureEnv(["OPUNCLEH_PROFILE"]);
  process.env.OPUNCLEH_PROFILE = "isolated";
});
...
vi.mock("../infra/opuncleh-root.js", () => ({
  resolveOpunclehPackageRoot: vi.fn().mockResolvedValue("/tmp/opuncleh"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/opuncleh/pnpm-lock.yaml",
      markerPath: "/tmp/opuncleh/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.opuncleh.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.opuncleh.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("opuncleh status --all") ||
          line.includes("opuncleh --profile isolated status --all"),
      ),
    ).toBe(true);
  });

  it("shows gateway auth when reachable", async () => {
    await withEnvVar("OPUNCLEH_GATEWAY_TOKEN", "abcd1234", async () => {
