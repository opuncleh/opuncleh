...
type PromptDefaultModelResult = { model?: string; config?: OpunclehConfig };
...
function resolveConfiguredModelRaw(cfg: OpunclehConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: OpunclehConfig): string[] {
...
export function applyPrimaryModel(cfg: OpunclehConfig, model: string): OpunclehConfig {
...
export function applyModelAllowlist(cfg: OpunclehConfig, models: string[]): OpunclehConfig {
  const defaults = cfg.agents?.defaults;
