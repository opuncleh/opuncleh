...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "OPUNCLEH_GATEWAY_TOKEN",
          placeholder: "OPUNCLEH_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like OPUNCLEH_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. Opuncleh will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: Opuncleh trusts user identity from a reverse proxy.",
        ...
        "Docs: https://www.opuncleh.com/docs.html/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
