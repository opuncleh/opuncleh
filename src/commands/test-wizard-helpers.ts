...
export async function setupAuthTestEnv(
  ...
  process.env.OPUNCLEH_AGENT_DIR = agentDir;
...
export function requireOpunclehAgentDir(): string {
  const agentDir = process.env.OPUNCLEH_AGENT_DIR;
  if (!agentDir) {
    throw new Error("OPUNCLEH_AGENT_DIR not set");
  }
  return agentDir;
}
