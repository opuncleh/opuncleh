...
import { OpunclehSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: OpunclehConfig): {
  config: OpunclehConfig;
  removed: string[];
} {
  const parsed = OpunclehSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: OpunclehConfig): void {
