...
import type { OpunclehConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: OpunclehConfig): string[] {
...
export async function noteSecurityWarnings(cfg: OpunclehConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("opuncleh security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.opuncleh/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("opuncleh approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.opuncleh.io/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("opuncleh configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("opuncleh config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("opuncleh doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("opuncleh config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('opuncleh config set session.dmScope "per-channel-peer"') +
