...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: OpunclehConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: OpunclehConfig) => { changed: boolean; next: OpunclehConfig };
  defaultModel: string;
  overrideConfig: OpunclehConfig;
  alreadyDefaultConfig: OpunclehConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: OpunclehConfig) => config,
      applyDefaultConfig: (config: OpunclehConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: OpunclehConfig = { agents: { defaults: {} } };
