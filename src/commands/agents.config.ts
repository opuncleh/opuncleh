...
function resolveAgentName(cfg: OpunclehConfig, agentId: string) {
...
function resolveAgentModel(cfg: OpunclehConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: OpunclehConfig): AgentSummary[] {
