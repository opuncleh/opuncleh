...
const ensureOpunclehModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveOpunclehAgentDir = vi.fn().mockReturnValue("/tmp/opuncleh-agent");
...
vi.mock("../agents/models-config.js", () => ({
  ensureOpunclehModelsJson,
}));

vi.mock("../agents/agent-paths.js", () => ({
  resolveOpunclehAgentDir,
}));
...
beforeEach(() => {
  ...
  ensureOpunclehModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureOpunclehModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the write snapshot config when provided", async () => {
    ...
    expect(ensureOpunclehModelsJson).toHaveBeenCalled();
    expect(ensureOpunclehModelsJson.mock.calls[0]?.[0]).toEqual(resolvedConfig);
  });
