...
import { resolveOpunclehPackageRoot } from "../infra/opuncleh-root.js";
...
function resolveMode(cfg: OpunclehConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("Opuncleh doctor");

  const root = await resolveOpunclehPackageRoot({
  ...
  let cfg: OpunclehConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("opuncleh configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("opuncleh config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("opuncleh setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  if (resolveMode(cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("opuncleh config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("opuncleh config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("opuncleh doctor --fix")}" to apply changes.`);
  }
