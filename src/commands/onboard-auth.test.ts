...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under OPUNCLEH_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("opuncleh-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "opuncleh-oauth-sync-"));
    ...
    process.env.OPUNCLEH_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "opuncleh-oauth-nosync-"));
    process.env.OPUNCLEH_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside OPUNCLEH_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "opuncleh-oauth-external-"));
    process.env.OPUNCLEH_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* OPUNCLEH_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to OPUNCLEH_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("opuncleh-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
