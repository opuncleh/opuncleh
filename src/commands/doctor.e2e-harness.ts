...
export const resolveOpunclehPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadOpunclehPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
beforeEach(() => {
  ...
  resolveOpunclehPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.OPUNCLEH_STATE_DIR;
  originalUpdateInProgress = process.env.OPUNCLEH_UPDATE_IN_PROGRESS;
  process.env.OPUNCLEH_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "opuncleh-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.OPUNCLEH_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.OPUNCLEH_UPDATE_IN_PROGRESS;
  } else {
    process.env.OPUNCLEH_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
