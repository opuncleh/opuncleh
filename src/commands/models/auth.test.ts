...
describe("modelsAuthLoginCommand", () => {
  ...
  let currentConfig: OpunclehConfig;
  let lastUpdatedConfig: OpunclehConfig | null;
  ...
  beforeEach(() => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/opuncleh/agents/main");
    mocks.resolveAgentWorkspaceDir.mockReturnValue("/tmp/opuncleh/workspace");
    mocks.resolveDefaultAgentWorkspaceDir.mockReturnValue("/tmp/opuncleh/workspace");
    mocks.loadValidConfigOrThrow.mockImplementation(async () => currentConfig);
    mocks.updateConfig.mockImplementation(
      async (mutator: (cfg: OpunclehConfig) => OpunclehConfig) => {
  ...
  it("runs plugin-owned openai-codex login", async () => {
    ...
    expect(mocks.upsertAuthProfile).toHaveBeenCalledWith({
      ...
      agentDir: "/tmp/opuncleh/agents/main",
    });
  ...
  it("loads lockout state from the agent-scoped store", async () => {
    ...
    expect(mocks.loadAuthProfileStoreForRuntime).toHaveBeenCalledWith("/tmp/opuncleh/agents/main");
  });
  ...
  it("reports loaded plugin providers when requested provider is unavailable", async () => {
    ...
    await expect(modelsAuthLoginCommand({ provider: "anthropic" }, runtime)).rejects.toThrow(
      'Unknown provider "anthropic". Loaded providers: openai-codex. Verify plugins via `opuncleh plugins list --json`.',
    );
  });
