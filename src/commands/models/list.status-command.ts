...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveOpunclehAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("opuncleh models auth setup-token")}\` or \`${formatCliCommand("opuncleh configure")}\`.`
          : `Run \`${formatCliCommand("opuncleh configure")}\` or set an API key env var.`;
