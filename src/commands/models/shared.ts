...
export async function loadValidConfigOrThrow(): Promise<OpunclehConfig> {
...
export async function updateConfig(
  mutator: (cfg: OpunclehConfig) => OpunclehConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: OpunclehConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: OpunclehConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("opuncleh agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
