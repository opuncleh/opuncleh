...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveOpunclehAgentDir: vi.fn().mockReturnValue("/tmp/opuncleh-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/opuncleh-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/opuncleh-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/opuncleh-dev/opuncleh.json",
    }),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveOpunclehAgentDir: mocks.resolveOpunclehAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/opuncleh-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveOpunclehAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.configPath).toBe("/tmp/opuncleh-dev/opuncleh.json");
    expect(payload.auth.storePath).toBe("/tmp/opuncleh-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/opuncleh-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/opuncleh-agent-custom");
