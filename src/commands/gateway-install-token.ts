...
import { readConfigFileSnapshot, writeConfigFile, type OpunclehConfig } from "../config/config.js";
...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("opuncleh config set gateway.auth.mode token")} or ${formatCliCommand("opuncleh config set gateway.auth.mode password")}.`,
  ].join(" ");
}
