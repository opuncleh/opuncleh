...
export function normalizeCompatibilityConfigValues(cfg: OpunclehConfig): {
  config: OpunclehConfig;
  changes: string[];
} {
  const changes: string[] = [];
  let next: OpunclehConfig = cfg;
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as OpunclehConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as OpunclehConfig["browser"],
    };
    changes.push(
