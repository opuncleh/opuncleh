...
export async function statusCommand(
  ...
  const daemonValue = (() => {
    ...
    const installedPrefix = daemon.managedByOpuncleh ? "installed · " : "";
  ...
  const nodeDaemonValue = (() => {
    ...
    const installedPrefix = nodeDaemon.managedByOpuncleh ? "installed · " : "";
  ...
  runtime.log(theme.heading("Opuncleh status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`opuncleh devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("opuncleh devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("opuncleh devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("opuncleh security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("opuncleh security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://www.opuncleh.com/docs.html/faq");
  runtime.log("Troubleshooting: https://www.opuncleh.com/docs.html/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("opuncleh status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("opuncleh logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("opuncleh status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("opuncleh gateway probe")}`);
  }
}
