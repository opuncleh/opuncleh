...
export type SearchProvider = NonNullable<
  NonNullable<NonNullable<NonNullable<OpunclehConfig["tools"]>["web"]>["search"]>["provider"]
>;
...
function rawKeyValue(config: OpunclehConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: OpunclehConfig, provider: SearchProvider): boolean {
...
function applyProviderOnly(config: OpunclehConfig, provider: SearchProvider): OpunclehConfig {
  const next = {
    ...config,
    tools: {
      ...config.tools,
...
function preserveDisabledState(original: OpunclehConfig, result: OpunclehConfig): OpunclehConfig {
...
export async function setupSearch(
  ...
  const choice = await prompter.select<PickerValue>({
    ...
    options: [
      ...options,
      {
        ...
        label: "Skip for now",
        hint: "Configure later with opuncleh configure --section web",
      },
    ],
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — Opuncleh will store a reference instead of the API key.",
