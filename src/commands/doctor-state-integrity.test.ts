...
type EnvSnapshot = {
  HOME?: string;
  OPUNCLEH_HOME?: string;
  OPUNCLEH_STATE_DIR?: string;
  OPUNCLEH_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    OPUNCLEH_HOME: process.env.OPUNCLEH_HOME,
    OPUNCLEH_STATE_DIR: process.env.OPUNCLEH_STATE_DIR,
    OPUNCLEH_OAUTH_DIR: process.env.OPUNCLEH_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: OpunclehConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: OpunclehConfig) {
...
async function runStateIntegrityText(cfg: OpunclehConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "opuncleh-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.OPUNCLEH_HOME = tempHome;
    process.env.OPUNCLEH_STATE_DIR = path.join(tempHome, ".opuncleh");
    delete process.env.OPUNCLEH_OAUTH_DIR;
    fs.mkdirSync(process.env.OPUNCLEH_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when OPUNCLEH_OAUTH_DIR is explicitly configured", async () => {
    process.env.OPUNCLEH_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: OpunclehConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints opuncleh-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: OpunclehConfig = {};
    ...
    expect(text).toMatch(/opuncleh sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/opuncleh sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /opuncleh sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
