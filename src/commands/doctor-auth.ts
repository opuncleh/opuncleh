...
function pruneAuthProfiles(
  cfg: OpunclehConfig,
  profileIds: Set<string>,
): { next: OpunclehConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("opuncleh models auth setup-token")}`,
    );
  }
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  ...
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("opuncleh models auth setup-token")} or ${formatCliCommand(
      "opuncleh configure",
    )}.`;
  }
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "opuncleh models auth login --provider openai-codex",
    )} or ${formatCliCommand("opuncleh configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("opuncleh configure")}\` or \`${formatCliCommand("opuncleh onboard")}\`.`;
}
