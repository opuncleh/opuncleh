...
export function hasExplicitMemorySearchConfig(cfg: OpunclehConfig, agentId: string): boolean {
  if (
    cfg.agents?.defaults &&
...
export function resolveMemoryPluginStatus(cfg: OpunclehConfig): MemoryPluginStatus {
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  resolveMemoryConfig: (cfg: OpunclehConfig, agentId: string) => { store: { path: string } } | null;
