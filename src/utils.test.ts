...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDir("opuncleh-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", async () => {
    await withTempDir("opuncleh-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", async () => {
    await withTempDir("opuncleh-lid-a-", async (first) => {
      await withTempDir("opuncleh-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.opuncleh when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "opuncleh-config-dir-"));
    try {
  ...
  it("expands OPUNCLEH_STATE_DIR using the provided env", () => {
    const env = {
      HOME: "/tmp/opuncleh-home",
      OPUNCLEH_STATE_DIR: "~/state",
    } as NodeJS.ProcessEnv;

    expect(resolveConfigDir(env)).toBe(path.resolve("/tmp/opuncleh-home", "state"));
  });
});

describe("resolveHomeDir", () => {
  it("prefers OPUNCLEH_HOME over HOME", () => {
    vi.stubEnv("OPUNCLEH_HOME", "/srv/opuncleh-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/opuncleh-home"));
...
describe("shortenHomePath", () => {
  it("uses $OPUNCLEH_HOME prefix when OPUNCLEH_HOME is set", () => {
    vi.stubEnv("OPUNCLEH_HOME", "/srv/opuncleh-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/opuncleh-home")}/.opuncleh/opuncleh.json`)).toBe(
      "$OPUNCLEH_HOME/.opuncleh/opuncleh.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $OPUNCLEH_HOME replacement when OPUNCLEH_HOME is set", () => {
    vi.stubEnv("OPUNCLEH_HOME", "/srv/opuncleh-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/opuncleh-home")}/.opuncleh/opuncleh.json`),
    ).toBe("config: $OPUNCLEH_HOME/.opuncleh/opuncleh.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/opuncleh", {}, () => "/Users/thoffman")).toBe(
      path.resolve("/Users/thoffman", "opuncleh"),
    );
  });
  ...
  it("prefers OPUNCLEH_HOME for tilde expansion", () => {
    vi.stubEnv("OPUNCLEH_HOME", "/srv/opuncleh-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/opuncleh")).toBe(path.resolve("/srv/opuncleh-home", "opuncleh"));
  ...
  it("uses the provided env for tilde expansion", () => {
    ...
    expect(resolveUserPath("~/opuncleh", env)).toBe(path.resolve("/srv/opuncleh-home", "opuncleh"));
  });
