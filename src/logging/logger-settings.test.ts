...
let originalOpunclehLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.OPUNCLEH_TEST_FILE_LOG;
  originalOpunclehLogLevel = process.env.OPUNCLEH_LOG_LEVEL;
  delete process.env.OPUNCLEH_TEST_FILE_LOG;
  delete process.env.OPUNCLEH_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.OPUNCLEH_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalOpunclehLogLevel === undefined) {
    delete process.env.OPUNCLEH_LOG_LEVEL;
  } else {
    process.env.OPUNCLEH_LOG_LEVEL = originalOpunclehLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
