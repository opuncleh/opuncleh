...
import {
  probeAuthenticatedOpunclehRelay,
...
async function probeRelay(baseUrl: string, relayAuthToken: string): Promise<boolean> {
  return await probeAuthenticatedOpunclehRelay({
    baseUrl,
    relayAuthHeader: "x-opuncleh-relay-token",
    relayAuthToken,
  });
}
...
describe("extension-relay-auth", () => {
  ...
  it("accepts authenticated opuncleh relay probe responses", async () => {
