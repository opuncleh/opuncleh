...
function localOpunclehProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "opuncleh",
...
function createLocalOpunclehResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localOpunclehProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveOpunclehUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "opuncleh-reset-"));
    const profileDir = path.join(tempRoot, "opuncleh");
    ...
    const ops = createLocalOpunclehResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveOpunclehUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "opuncleh-reset-no-own-"));
    const profileDir = path.join(tempRoot, "opuncleh");
    fs.mkdirSync(profileDir, { recursive: true });
