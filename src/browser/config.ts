import type { BrowserConfig, BrowserProfileConfig, OpunclehConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_OPUNCLEH_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_OPUNCLEH_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "opuncleh" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_OPUNCLEH_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_OPUNCLEH_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Opuncleh browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for opuncleh/opuncleh CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: OpunclehConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_OPUNCLEH_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_OPUNCLEH_BROWSER_PROFILE_NAME]
        ? DEFAULT_OPUNCLEH_BROWSER_PROFILE_NAME
        : "chrome");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "opuncleh";
