...
export function resolveOpunclehUserDataDir(profileName = DEFAULT_OPUNCLEH_BROWSER_PROFILE_NAME) {
...
export async function launchOpunclehChrome(
  ...
  const userDataDir = resolveOpunclehUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_OPUNCLEH_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateOpunclehProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🦞 opuncleh browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`opuncleh browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`opuncleh browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 opuncleh browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopOpunclehChrome(
