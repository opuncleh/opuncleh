...
const { stopOpunclehChromeMock, stopChromeExtensionRelayServerMock } = vi.hoisted(() => ({
  stopOpunclehChromeMock: vi.fn(async () => {}),
...
vi.mock("./chrome.js", () => ({
  stopOpunclehChrome: stopOpunclehChromeMock,
}));
...
describe("ensureExtensionRelayForProfiles", () => {
  ...
  it("starts relay only for extension profiles", async () => {
    resolveProfileMock.mockImplementation((_resolved: unknown, name: string) => {
      ...
      return { driver: "opuncleh", cdpUrl: "http://127.0.0.1:18889" };
    });
...
describe("stopKnownBrowserProfiles", () => {
  beforeEach(() => {
    ...
    stopOpunclehChromeMock.mockClear();
    stopChromeExtensionRelayServerMock.mockClear();
  });

  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["opuncleh", "chrome"]);
    const stopMap: Record<string, ReturnType<typeof vi.fn>> = {
      opuncleh: vi.fn(async () => {}),
      chrome: vi.fn(async () => {
    ...
    expect(stopMap.opuncleh).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopOpunclehChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("opuncleh browser stop failed: Error: oops");
  });
});
