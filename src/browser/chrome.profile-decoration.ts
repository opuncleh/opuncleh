...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".opuncleh-profile-decorated");
}
 ...
 */
export function decorateOpunclehProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_OPUNCLEH_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_OPUNCLEH_BROWSER_COLOR).toUpperCase();
