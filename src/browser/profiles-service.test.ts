...
vi.mock("./chrome.js", () => ({
  resolveOpunclehUserDataDir: vi.fn(() => "/tmp/opuncleh-test/opuncleh/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveOpunclehUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "opuncleh-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveOpunclehUserDataDir).mockReturnValue(userDataDir);
