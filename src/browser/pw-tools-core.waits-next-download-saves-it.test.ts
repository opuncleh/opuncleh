...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredOpunclehTmpDir: vi.fn(() => "/tmp/opuncleh"),
}));
vi.mock("../infra/tmp-opuncleh-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredOpunclehTmpDir.mockReturnValue("/tmp/opuncleh");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "opuncleh-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".opuncleh-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredOpunclehTmpDir.mockReturnValue("/tmp/opuncleh-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "opuncleh-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "opuncleh-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredOpunclehTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "opuncleh-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "opuncleh-preferred", "downloads")}${path.sep}`),
    );
  });
