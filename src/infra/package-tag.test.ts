...
describe("normalizePackageTagInput", () => {
  const packageNames = ["opuncleh", "@opuncleh/plugin"] as const;
  ...
  it("strips known package-name prefixes before returning the tag", () => {
    expect(normalizePackageTagInput("opuncleh@beta", packageNames)).toBe("beta");
    expect(normalizePackageTagInput("@opuncleh/plugin@2026.2.24", packageNames)).toBe("2026.2.24");
    expect(normalizePackageTagInput("opuncleh@   ", packageNames)).toBeNull();
  });

  it("treats exact known package names as an empty tag", () => {
    expect(normalizePackageTagInput("opuncleh", packageNames)).toBeNull();
    expect(normalizePackageTagInput(" @opuncleh/plugin ", packageNames)).toBeNull();
  });
  ...
  it("returns trimmed raw values when no package prefix matches", () => {
    ...
    expect(normalizePackageTagInput("opuncleher@beta", packageNames)).toBe("opuncleher@beta");
  });
});
