...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "opuncleh-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["OPUNCLEH_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.OPUNCLEH_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveOpunclehPackageRoot } = await import("./opuncleh-root.js"));
    ...
    vi.mocked(resolveOpunclehPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveOpunclehPackageRoot).mockResolvedValue("/opt/opuncleh");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/opuncleh/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
