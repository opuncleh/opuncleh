...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __opunclehDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__opunclehDiagnosticEventsState) {
    globalStore.__opunclehDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__opunclehDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: OpunclehConfig): boolean {
