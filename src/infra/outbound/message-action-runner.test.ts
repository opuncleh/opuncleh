...
describe("runMessageAction sandboxed media validation", () => {
  ...
  it("allows media paths under preferred Opuncleh tmp root", async () => {
    const tmpRoot = resolvePreferredOpunclehTmpDir();
    ...
    try {
      ...
      const hostTmpOutsideOpuncleh = path.join(os.tmpdir(), "outside-opuncleh", "test-media.png");
      await expect(
        runMessageAction({
          cfg: slackConfig,
          action: "send",
