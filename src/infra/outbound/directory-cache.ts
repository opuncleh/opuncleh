...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: OpunclehConfig | null = null;
  ...
  get(key: string, cfg: OpunclehConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: OpunclehConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: OpunclehConfig): void {
