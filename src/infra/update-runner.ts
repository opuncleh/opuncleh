...
const DEFAULT_PACKAGE_NAME = "opuncleh";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["opuncleh", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "opuncleh-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "opuncleh.mjs");
    ...
    const doctorStep = await runStep(
      step("opuncleh doctor", doctorArgv, gitRoot, { OPUNCLEH_UPDATE_IN_PROGRESS: "1" }),
    );
