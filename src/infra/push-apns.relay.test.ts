  ...
  describe("resolveApnsRelayConfigFromEnv", () => {
    ...
    it("lets env overrides win and clamps tiny timeout values", () => {
      const resolved = resolveApnsRelayConfigFromEnv(
        {
          OPUNCLEH_APNS_RELAY_BASE_URL: " https://relay-override.example.com/base/ ",
          OPUNCLEH_APNS_RELAY_TIMEOUT_MS: "999",
    ...
    it("allows loopback http URLs for alternate truthy env values", () => {
      const resolved = resolveApnsRelayConfigFromEnv({
        OPUNCLEH_APNS_RELAY_BASE_URL: "http://[::1]:8787",
        OPUNCLEH_APNS_RELAY_ALLOW_HTTP: "yes",
        OPUNCLEH_APNS_RELAY_TIMEOUT_MS: "nope",
      } as NodeJS.ProcessEnv);
    ...
    it.each([
      {
        ...
        env: { OPUNCLEH_APNS_RELAY_BASE_URL: "ftp://relay.example.com" },
      ...
      {
        name: "query string",
        env: { OPUNCLEH_APNS_RELAY_BASE_URL: "https://relay.example.com/path?debug=1" },
        expected: "query and fragment are not allowed",
      },
      {
        name: "userinfo",
        env: { OPUNCLEH_APNS_RELAY_BASE_URL: "https://user:pass@relay.example.com/path" },
