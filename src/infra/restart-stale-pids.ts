 ...
 * returning control to the caller (typically `triggerOpunclehRestart` →
...
/**
 * Parse opuncleh gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("opuncleh")) {
 ...
 * Returns only PIDs that belong to opuncleh gateway processes (not the current process).
 */
