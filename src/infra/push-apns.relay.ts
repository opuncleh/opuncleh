...
const GATEWAY_DEVICE_ID_HEADER = "x-opuncleh-gateway-device-id";
const GATEWAY_SIGNATURE_HEADER = "x-opuncleh-gateway-signature";
const GATEWAY_SIGNED_AT_HEADER = "x-opuncleh-gateway-signed-at-ms";
...
export function resolveApnsRelayConfigFromEnv(
  ...
  const envBaseUrl = normalizeNonEmptyString(env.OPUNCLEH_APNS_RELAY_BASE_URL);
  ...
  const baseUrlSource = envBaseUrl
    ? "OPUNCLEH_APNS_RELAY_BASE_URL"
    : "gateway.push.apns.relay.baseUrl";
  if (!baseUrl) {
    return {
      ok: false,
      error:
        "APNs relay config missing: set gateway.push.apns.relay.baseUrl or OPUNCLEH_APNS_RELAY_BASE_URL",
    };
  }
  ...
  try {
    ...
    if (parsed.protocol === "http:" && !readAllowHttp(env.OPUNCLEH_APNS_RELAY_ALLOW_HTTP)) {
      throw new Error(
        "http relay URLs require OPUNCLEH_APNS_RELAY_ALLOW_HTTP=true (development only)",
      );
    }
    ...
    return {
      ...
      value: {
        ...
        timeoutMs: normalizeTimeoutMs(
          env.OPUNCLEH_APNS_RELAY_TIMEOUT_MS ?? configuredRelay?.timeoutMs,
        ),
      },
    };
  } catch (err) {
