...
const originalOpunclehHome = process.env.OPUNCLEH_HOME;
...
afterEach(() => {
  vi.restoreAllMocks();
  if (originalOpunclehHome === undefined) {
    delete process.env.OPUNCLEH_HOME;
  } else {
    process.env.OPUNCLEH_HOME = originalOpunclehHome;
  }
...
function createHomeDir(): string {
  ...
  tempDirs.push(dir);
  process.env.OPUNCLEH_HOME = dir;
  return dir;
}

function approvalsFilePath(homeDir: string): string {
  return path.join(homeDir, ".opuncleh", "exec-approvals.json");
}
...
describe("exec approvals store helpers", () => {
  it("expands home-prefixed default file and socket paths", () => {
    ...
    expect(path.normalize(resolveExecApprovalsPath())).toBe(
      path.normalize(path.join(dir, ".opuncleh", "exec-approvals.json")),
    );
    expect(path.normalize(resolveExecApprovalsSocketPath())).toBe(
      path.normalize(path.join(dir, ".opuncleh", "exec-approvals.sock")),
    );
  });
