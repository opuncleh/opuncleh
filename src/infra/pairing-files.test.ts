...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/opuncleh-state", "devices")).toEqual({
      dir: path.join("/tmp/opuncleh-state", "devices"),
      pendingPath: path.join("/tmp/opuncleh-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/opuncleh-state", "devices", "paired.json"),
    });
  });
