...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.OPUNCLEH_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*opuncleh-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "opuncleh";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "opuncleh");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "opuncleh-gw");
  ...
  records.push(`_opuncleh-gw._tcp IN PTR ${instanceLabel}._opuncleh-gw._tcp`);
  records.push(`${instanceLabel}._opuncleh-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._opuncleh-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; opuncleh-content-hash: ${contentHash}\n${contentBody}`;
}
