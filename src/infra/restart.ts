...
export function triggerOpunclehRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.OPUNCLEH_SYSTEMD_UNIT,
      process.env.OPUNCLEH_PROFILE,
    );
  ...
  const label =
    process.env.OPUNCLEH_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.OPUNCLEH_PROFILE);
