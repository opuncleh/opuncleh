...
function candidateBinDirs(opts: EnsureOpunclehPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `opuncleh` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "opuncleh");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.OPUNCLEH_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "opuncleh"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `opuncleh` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureOpunclehCliOnPath(opts: EnsureOpunclehPathOpts = {}) {
  if (isTruthyEnvValue(process.env.OPUNCLEH_PATH_BOOTSTRAPPED)) {
    return;
  }
