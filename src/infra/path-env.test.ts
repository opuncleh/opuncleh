...
describe("ensureOpunclehCliOnPath", () => {
  ...
  function setupAppCliRoot(name: string) {
    const tmp = abs(`/tmp/opuncleh-path/${name}`);
  ...
  function bootstrapPath(params: {
    ...
    ensureOpunclehCliOnPath(params);
  ...
  it.each([
    ...
    ({ envValue, allowProjectLocalBin }) => {
      ...
      process.env.PATH = "/usr/bin";
      delete process.env.OPUNCLEH_PATH_BOOTSTRAPPED;
      delete process.env.OPUNCLEH_ALLOW_PROJECT_LOCAL_BIN;
      ...
      if (envValue === undefined) {
        ...
        process.env.OPUNCLEH_ALLOW_PROJECT_LOCAL_BIN = envValue;
      }
