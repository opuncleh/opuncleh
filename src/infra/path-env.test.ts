...
let ensureOpunclehCliOnPath: typeof import("./path-env.js").ensureOpunclehCliOnPath;

describe("ensureOpunclehCliOnPath", () => {
  const envKeys = [
    "PATH",
    "OPUNCLEH_PATH_BOOTSTRAPPED",
    "OPUNCLEH_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureOpunclehCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling opuncleh exists", () => {
    const tmp = abs("/tmp/opuncleh-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "opuncleh");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.OPUNCLEH_PATH_BOOTSTRAPPED;

    ensureOpunclehCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.OPUNCLEH_PATH_BOOTSTRAPPED = "1";
    ensureOpunclehCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/opuncleh-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "opuncleh");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/opuncleh-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "opuncleh");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "opuncleh");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/opuncleh-path/case-linuxbrew");
