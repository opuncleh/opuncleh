...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: OpunclehConfig, policy: ChannelAccessPolicy) => OpunclehConfig;
  resolveAllowlist: (params: { cfg: OpunclehConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: OpunclehConfig; resolved: TResolved }) => OpunclehConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: OpunclehConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: OpunclehConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: OpunclehConfig, policy: ChannelAccessPolicy): OpunclehConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: OpunclehConfig, policy: ChannelAccessPolicy): OpunclehConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: OpunclehConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: OpunclehConfig; resolved: string[] }) => {
      calls.push("apply");
