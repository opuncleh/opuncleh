...
export function setTopLevelChannelDmPolicyWithAllowFrom(params: {
  ...
  channel: string;
  dmPolicy: DmPolicy;
  getAllowFrom?: (cfg: OpunclehConfig) => Array<string | number> | undefined;
}): OpunclehConfig {
  const channelConfig =
    (params.cfg.channels?.[params.channel] as Record<string, unknown> | undefined) ?? {};
...
export async function runSingleChannelSecretStep(params: {
  ...
  applyUseEnv?: (cfg: OpunclehConfig) => OpunclehConfig | Promise<OpunclehConfig>;
  applySet?: (
    cfg: OpunclehConfig,
    value: SecretInput,
...
export async function promptSingleChannelSecretInput(params: {
  ...
  const selectedMode = await resolveSecretInputModeForEnvSelection({
    ...
    copy: {
      ...
      plaintextHint: "Stores the credential directly in Opuncleh config",
  ...
  const resolved = await promptSecretRefForOnboarding({
    ...
    copy: {
      ...
      envVarPlaceholder: params.preferredEnvVar ?? "OPUNCLEH_SECRET",
      envVarFormatError:
        'Use an env var name like "OPUNCLEH_SECRET" (uppercase letters, numbers, underscores).',
      noProvidersMessage:
