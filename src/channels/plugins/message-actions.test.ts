...
describe("message action capability checks", () => {
  ...
  it("aggregates capabilities across plugins", () => {
    ...
    expect(listChannelMessageCapabilities({} as OpunclehConfig).toSorted()).toEqual([
      "buttons",
      "cards",
      "interactive",
    ]);
    expect(channelSupportsMessageCapability({} as OpunclehConfig, "interactive")).toBe(true);
    expect(channelSupportsMessageCapability({} as OpunclehConfig, "buttons")).toBe(true);
    expect(channelSupportsMessageCapability({} as OpunclehConfig, "cards")).toBe(true);
  });
  ...
  it("checks per-channel capabilities", () => {
    ...
    expect(
      channelSupportsMessageCapabilityForChannel(
        { cfg: {} as OpunclehConfig, channel: "discord" },
        "interactive",
      ),
    ).toBe(true);
    expect(
      channelSupportsMessageCapabilityForChannel(
        { cfg: {} as OpunclehConfig, channel: "telegram" },
        "interactive",
      ),
    ).toBe(false);
    expect(
    ...
    expect(channelSupportsMessageCapabilityForChannel({ cfg: {} as OpunclehConfig }, "cards")).toBe(
      false,
    );
  });
  ...
  it("prefers unified message tool discovery over legacy discovery methods", () => {
    ...
    expect(listChannelMessageActions({} as OpunclehConfig)).toEqual(["send", "broadcast", "react"]);
    expect(listChannelMessageCapabilities({} as OpunclehConfig)).toEqual(["interactive"]);
    expect(
      resolveChannelMessageToolSchemaProperties({
        cfg: {} as OpunclehConfig,
        channel: "discord",
  ...
  it("skips crashing action/capability discovery paths and logs once", () => {
    ...
    expect(listChannelMessageActions({} as OpunclehConfig)).toEqual(["send", "broadcast"]);
    expect(listChannelMessageCapabilities({} as OpunclehConfig)).toEqual([]);
