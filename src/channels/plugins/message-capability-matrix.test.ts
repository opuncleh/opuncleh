...
describe("channel action capability matrix", () => {
  ...
  it("forwards Telegram action capabilities through the channel wrapper", () => {
    ...
    const result = telegramPlugin.actions?.getCapabilities?.({ cfg: {} as OpunclehConfig });
    ...
    const discordResult = discordPlugin.actions?.getCapabilities?.({ cfg: {} as OpunclehConfig });
