...
function telegramCfg(): OpunclehConfig {
  return { channels: { telegram: { botToken: "tok" } } } as OpunclehConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: OpunclehConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as OpunclehConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as OpunclehConfig;
...
function createSignalAccountOverrideCfg(): OpunclehConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as OpunclehConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as OpunclehConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as OpunclehConfig,
        accountId: undefined,
        params: {
