...
declare const __OPUNCLEH_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "opuncleh";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["OPUNCLEH_VERSION"],
      runtimeVersion,
      env["OPUNCLEH_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current Opuncleh version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __OPUNCLEH_VERSION__ === "string" ? __OPUNCLEH_VERSION__ : undefined,
  bundledVersion: process.env.OPUNCLEH_BUNDLED_VERSION,
});
