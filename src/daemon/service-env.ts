...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.OPUNCLEH_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    OPUNCLEH_PROFILE: profile,
    OPUNCLEH_GATEWAY_PORT: String(port),
    OPUNCLEH_LAUNCHD_LABEL: resolvedLaunchdLabel,
    OPUNCLEH_SYSTEMD_UNIT: systemdUnit,
    OPUNCLEH_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    OPUNCLEH_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    OPUNCLEH_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  const sharedEnv = resolveSharedServiceEnvironmentFields(env, platform);
  const gatewayToken =
    env.OPUNCLEH_GATEWAY_TOKEN?.trim() || env.CLAWDBOT_GATEWAY_TOKEN?.trim() || undefined;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    OPUNCLEH_GATEWAY_TOKEN: gatewayToken,
    OPUNCLEH_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    OPUNCLEH_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    OPUNCLEH_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    OPUNCLEH_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    OPUNCLEH_LOG_PREFIX: "node",
    OPUNCLEH_SERVICE_MARKER: NODE_SERVICE_MARKER,
    OPUNCLEH_SERVICE_KIND: NODE_SERVICE_KIND,
    OPUNCLEH_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    OPUNCLEH_STATE_DIR: sharedEnv.stateDir,
    OPUNCLEH_CONFIG_PATH: sharedEnv.configPath,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.OPUNCLEH_STATE_DIR;
  const configPath = env.OPUNCLEH_CONFIG_PATH;
