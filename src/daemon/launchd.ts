...
async function resolveLaunchAgentGatewayPort(env: GatewayServiceEnv): Promise<number | null> {
  ...
  const fromEnv = parseStrictPositiveInteger(env.OPUNCLEH_GATEWAY_PORT ?? "");
  return fromEnv ?? null;
}
...
function throwBootstrapGuiSessionError(params: {
  ...
  throw new Error(
    [
      ...
      "Headless deployments should use a dedicated logged-in user session or a custom LaunchDaemon (not shipped): https://www.opuncleh.com/docs.html/gateway",
    ].join("\n"),
  );
}
...
export async function installLaunchAgent({
  ...
  await bootstrapLaunchAgentOrThrow({
    ...
    plistPath,
    actionHint: "opuncleh gateway install --force",
  });
...
export async function restartLaunchAgent({
  ...
  await bootstrapLaunchAgentOrThrow({
    ...
    serviceTarget,
    plistPath,
    actionHint: "opuncleh gateway restart",
  });
