...
const TEST_MANAGED_HOME = "/tmp/opuncleh-test-home";
const GATEWAY_SERVICE = "opuncleh-gateway.service";
...
async function expectExecStartWithoutEnvironment(envFileLine: string) {
  mockReadGatewayServiceFile(["[Service]", "ExecStart=/usr/bin/opuncleh gateway run", envFileLine]);
...
describe("readSystemdServiceExecStart", () => {
  ...
  it("loads OPUNCLEH_GATEWAY_TOKEN from EnvironmentFile", async () => {
    const readFileSpy = mockReadGatewayServiceFile(
      ["[Service]", "ExecStart=/usr/bin/opuncleh gateway run", "EnvironmentFile=%h/.opuncleh/.env"],
      { [`${TEST_SERVICE_HOME}/.opuncleh/.env`]: "OPUNCLEH_GATEWAY_TOKEN=env-file-token\n" },
    );
  ...
  it("ignores missing optional EnvironmentFile entries", async () => {
    await expectExecStartWithoutEnvironment("EnvironmentFile=-%h/.opuncleh/missing.env");
  });
...
describe("systemd service control", () => {
  ...
  it("restarts a profile-specific user unit", async () => {
    execFileMock
      .mockImplementationOnce((_cmd, _args, _opts, cb) => cb(null, "", ""))
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        assertUserSystemctlArgs(args, "restart", "opuncleh-gateway-work.service");
        cb(null, "", "");
      });
