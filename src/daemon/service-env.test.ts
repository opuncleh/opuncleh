...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.OPUNCLEH_GATEWAY_PORT).toBe("18789");
    expect(env.OPUNCLEH_GATEWAY_TOKEN).toBeUndefined();
    expect(env.OPUNCLEH_SERVICE_MARKER).toBe("opuncleh");
    expect(env.OPUNCLEH_SERVICE_KIND).toBe("gateway");
    expect(typeof env.OPUNCLEH_SERVICE_VERSION).toBe("string");
    expect(env.OPUNCLEH_SYSTEMD_UNIT).toBe("opuncleh-gateway.service");
    expect(env.OPUNCLEH_WINDOWS_TASK_NAME).toBe("Opuncleh Gateway");
    if (process.platform === "darwin") {
      expect(env.OPUNCLEH_LAUNCHD_LABEL).toBe("ai.opuncleh.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", OPUNCLEH_PROFILE: "work" },
      port: 18789,
    });
    expect(env.OPUNCLEH_SYSTEMD_UNIT).toBe("opuncleh-gateway-work.service");
    expect(env.OPUNCLEH_WINDOWS_TASK_NAME).toBe("Opuncleh Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.OPUNCLEH_LAUNCHD_LABEL).toBe("ai.opuncleh.work");
    }
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through OPUNCLEH_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", OPUNCLEH_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.OPUNCLEH_GATEWAY_TOKEN).toBe("node-token");
  });

  it("maps legacy CLAWDBOT_GATEWAY_TOKEN to OPUNCLEH_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", CLAWDBOT_GATEWAY_TOKEN: " legacy-token " },
    });
    expect(env.OPUNCLEH_GATEWAY_TOKEN).toBe("legacy-token");
  });

  it("prefers OPUNCLEH_GATEWAY_TOKEN over legacy CLAWDBOT_GATEWAY_TOKEN", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
        OPUNCLEH_GATEWAY_TOKEN: "opuncleh-token",
        CLAWDBOT_GATEWAY_TOKEN: "legacy-token",
      },
    });
    expect(env.OPUNCLEH_GATEWAY_TOKEN).toBe("opuncleh-token");
  });

  it("omits OPUNCLEH_GATEWAY_TOKEN when both token env vars are empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".opuncleh"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", OPUNCLEH_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".opuncleh-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", OPUNCLEH_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".opuncleh"));
  });

  it("uses OPUNCLEH_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", OPUNCLEH_STATE_DIR: "/var/lib/opuncleh" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/opuncleh"));
  });

  it("expands ~ in OPUNCLEH_STATE_DIR", () => {
    const env = { HOME: "/Users/test", OPUNCLEH_STATE_DIR: "~/opuncleh-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/opuncleh-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { OPUNCLEH_STATE_DIR: "C:\\State\\opuncleh" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\opuncleh");
  });
});
