...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `opuncleh-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      OPUNCLEH_LAUNCHD_LABEL: `ai.opuncleh.launchd-int-${testId}`,
      OPUNCLEH_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
