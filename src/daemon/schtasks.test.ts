...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when OPUNCLEH_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".opuncleh", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when OPUNCLEH_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", OPUNCLEH_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".opuncleh-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers OPUNCLEH_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        OPUNCLEH_PROFILE: "rescue",
        OPUNCLEH_STATE_DIR: "C:\\State\\opuncleh",
      },
      expected: path.join("C:\\State\\opuncleh", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", OPUNCLEH_PROFILE: "default" },
      expected: path.join("/home/test", ".opuncleh", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "opuncleh-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem Opuncleh Gateway",
          "cd /d C:\\Projects\\opuncleh",
          "set NODE_ENV=production",
          "set OPUNCLEH_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\opuncleh",
          environment: {
            NODE_ENV: "production",
            OPUNCLEH_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\opuncleh\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\opuncleh\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\Opuncleh Share\\node.exe" "\\\\fileserver\\Opuncleh Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\Opuncleh Share\\node.exe",
            "\\\\fileserver\\Opuncleh Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from OPUNCLEH_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ OPUNCLEH_STATE_DIR: path.join(tmpDir, "custom-state") }),
