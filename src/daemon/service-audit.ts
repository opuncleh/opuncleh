...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds OPUNCLEH_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `opuncleh gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service OPUNCLEH_GATEWAY_TOKEN does not match gateway.auth.token in opuncleh.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.OPUNCLEH_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return command.environment?.OPUNCLEH_GATEWAY_TOKEN?.trim() || undefined;
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `opuncleh gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
