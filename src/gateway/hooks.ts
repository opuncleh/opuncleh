...
export function resolveHooksConfig(cfg: OpunclehConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: OpunclehConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-opuncleh-token"] === "string"
      ? req.headers["x-opuncleh-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-opuncleh-idempotency-key"]) ||
