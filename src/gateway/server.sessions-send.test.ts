...
const { createOpunclehTools } = await import("../agents/opuncleh-tools.js");
...
type SessionSendTool = ReturnType<typeof createOpunclehTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createOpunclehTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["OPUNCLEH_GATEWAY_PORT", "OPUNCLEH_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.OPUNCLEH_GATEWAY_PORT = String(gatewayPort);
  process.env.OPUNCLEH_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("OPUNCLEH_CONFIG_PATH missing in gateway test environment");
      }
      ...
      const tool = createOpunclehTools({
        config: {
          tools: {
            sessions: {
              visibility: "all",
