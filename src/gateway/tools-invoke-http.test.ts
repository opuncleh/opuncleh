...
let lastCreateOpunclehToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/opuncleh-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateOpunclehToolsContext?.agentTo,
          agentThreadId: lastCreateOpunclehToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createOpunclehTools: (ctx: Record<string, unknown>) => {
      lastCreateOpunclehToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateOpunclehToolsContext = undefined;
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateOpunclehToolsContext?.allowMediaInvokeCommands).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateOpunclehToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-opuncleh-message-to": "channel:24514",
        "x-opuncleh-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
