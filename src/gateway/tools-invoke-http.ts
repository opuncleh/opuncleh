...
import { createOpunclehTools } from "../agents/opuncleh-tools.js";
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-opuncleh-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-opuncleh-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-opuncleh-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-opuncleh-thread-id")?.trim() || undefined;
  ...
  const allTools = createOpunclehTools({
