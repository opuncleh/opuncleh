...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("opuncleh doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with OPUNCLEH_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with OPUNCLEH_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.OPUNCLEH_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://opuncleh-gateway.ai:18789",
      onConnectError,
    });
