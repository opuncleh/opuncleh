...
function cfg(input: Partial<OpunclehConfig>): OpunclehConfig {
  return input as OpunclehConfig;
}
...
async function expectFailClosedOnUnresolvedLocalAuth(config: OpunclehConfig, path: string) {
  await expect(
    resolveGatewayConnectionAuth({
      config,
...
const DEFAULT_ENV = {
  OPUNCLEH_GATEWAY_TOKEN: "env-token",
  OPUNCLEH_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves config-first token SecretRef even when OPUNCLEH env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when OPUNCLEH env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
