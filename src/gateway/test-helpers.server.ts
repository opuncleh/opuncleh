...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "OPUNCLEH_SKIP_BROWSER_CONTROL_SERVER",
  "OPUNCLEH_SKIP_GMAIL_WATCHER",
  "OPUNCLEH_SKIP_CANVAS_HOST",
  "OPUNCLEH_BUNDLED_PLUGINS_DIR",
  "OPUNCLEH_SKIP_CHANNELS",
  "OPUNCLEH_SKIP_PROVIDERS",
  "OPUNCLEH_SKIP_CRON",
  "OPUNCLEH_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "opuncleh-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.OPUNCLEH_STATE_DIR = path.join(tempHome, ".opuncleh");
  delete process.env.OPUNCLEH_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.OPUNCLEH_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.OPUNCLEH_SKIP_GMAIL_WATCHER = "1";
  process.env.OPUNCLEH_SKIP_CANVAS_HOST = "1";
  process.env.OPUNCLEH_SKIP_CHANNELS = "1";
  process.env.OPUNCLEH_SKIP_PROVIDERS = "1";
  process.env.OPUNCLEH_SKIP_CRON = "1";
  process.env.OPUNCLEH_TEST_MINIMAL_GATEWAY = "1";
  process.env.OPUNCLEH_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "opuncleh-test-no-bundled-extensions")
    : "opuncleh-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".opuncleh-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".opuncleh-test");
    await fs.rm(tempConfigRoot, { recursive: true, force: true });
    await fs.mkdir(tempConfigRoot, { recursive: true });
  }
...
const CONNECT_CHALLENGE_NONCE_KEY = "__opunclehTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__opunclehTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["OPUNCLEH_GATEWAY_TOKEN"]);
  const prev = process.env.OPUNCLEH_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.OPUNCLEH_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.OPUNCLEH_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
