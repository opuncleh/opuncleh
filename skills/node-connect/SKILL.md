---
name: node-connect
description: Diagnose Opuncleh node connection and pairing failures for Android, iOS, and macOS companion apps. Use when QR/setup code/manual connect fails, local Wi-Fi works but VPS/tailnet does not, or errors mention pairing required, unauthorized, bootstrap token invalid or expired, gateway.bind, gateway.remote.url, Tailscale, or plugins.entries.device-pair.config.publicUrl.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify Opuncleh is advertising that route, then fix pairing/auth.
...
- whether `opuncleh devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `opuncleh qr --json`. It uses the same setup-code payload Android scans.

```bash
opuncleh config get gateway.mode
opuncleh config get gateway.bind
opuncleh config get gateway.tailscale.mode
opuncleh config get gateway.remote.url
opuncleh config get gateway.auth.mode
opuncleh config get gateway.auth.allowTailscale
opuncleh config get plugins.entries.device-pair.config.publicUrl
opuncleh qr --json
opuncleh devices list
opuncleh nodes status
```

If this Opuncleh instance is pointed at a remote gateway, also run:

```bash
opuncleh qr --remote --json
```
...
`opuncleh qr --json` success means:
...
If `opuncleh qr --json` says `Gateway is only bound to loopback`:
...
```bash
opuncleh devices list
opuncleh devices approve --latest
```
...
- `opuncleh devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run opuncleh qr again, rescan, then approve the pending device pairing.`
