---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Opuncleh deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Opuncleh cron scheduling for periodic checks, or version status checks on a machine running Opuncleh (laptop, workstation, Pi, VPS).
---

# Opuncleh Host Hardening

## Overview

Assess and harden the host running Opuncleh, then align it to a user-defined risk tolerance without breaking access. Use Opuncleh security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Opuncleh changes the host firewall, SSH, or OS updates; it does not.
...
5. Opuncleh gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Opuncleh can access sensitive data.
...
### 2) Run Opuncleh security audits (read-only)

As part of the default read-only checks, run `opuncleh security audit --deep`. Only offer alternatives if the user requests them:

1. `opuncleh security audit` (faster, non-probing)
2. `opuncleh security audit --json` (structured output)

Offer to apply Opuncleh safe defaults (numbered):

1. `opuncleh security audit --fix`

Be explicit that `--fix` only tightens Opuncleh defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Opuncleh version/update status (read-only)

As part of the default read-only checks, run `opuncleh update status`.
...
- Credential hygiene notes (location of Opuncleh creds, prefer disk encryption)
...
- Opuncleh security audit (re-run)
...
After Opuncleh install or first hardening pass, run at least one baseline audit and version check:

- `opuncleh security audit`
- `opuncleh security audit --deep`
- `opuncleh update status`

Ongoing monitoring is recommended. Use the Opuncleh cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `opuncleh cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `opuncleh update status`
...
Before creating, `opuncleh cron list` and match on exact `name`. If found, `opuncleh cron edit <id> ...`.
If not found, `opuncleh cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `opuncleh update status` (preferred for source checkouts and channels)
2. `npm view opuncleh version` (published npm version)

## Opuncleh command accuracy

Use only supported commands and flags:

- `opuncleh security audit [--deep] [--fix] [--json]`
- `opuncleh status` / `opuncleh status --deep`
- `opuncleh health --json`
- `opuncleh update status`
- `opuncleh cron add|list|runs|run`

Do not invent CLI flags or imply Opuncleh enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Opuncleh compaction:
