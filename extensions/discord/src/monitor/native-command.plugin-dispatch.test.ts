...
function createConfig(): OpunclehConfig {
  return {
    channels: {
      discord: {
...
function createNativeCommand(cfg: OpunclehConfig, commandSpec: NativeCommandSpec) {
...
function createPluginCommand(params: { cfg: OpunclehConfig; name: string }) {
  return createDiscordNativeCommand({
    command: {
      name: params.name,
      description: "Pair",
...
function createStatusCommand(cfg: OpunclehConfig) {
