...
describe("agent components", () => {
  const createCfg = (): OpunclehConfig => ({}) as OpunclehConfig;
...
describe("discord component interactions", () => {
  const createCfg = (): OpunclehConfig =>
    ({
      channels: {
        discord: {
          replyToMode: "first",
  ...
  beforeEach(() => {
    ...
    resolveStorePathMock.mockClear().mockReturnValue("/tmp/opuncleh-sessions-test.json");
    ...
    resolvePluginConversationBindingApprovalMock.mockReset().mockResolvedValue({
      ...
      binding: {
        ...
        pluginName: "Opuncleh App Server",
