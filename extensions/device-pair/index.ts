...
import type { OpunclehPluginApi } from "opuncleh/plugin-sdk/device-pair";
import {
...
} from "opuncleh/plugin-sdk/device-pair";
...
function resolveGatewayPort(cfg: OpunclehPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.OPUNCLEH_GATEWAY_PORT?.trim()) ??
...
function resolveAuthLabel(cfg: OpunclehPluginApi["config"]): ResolveAuthLabelResult {
...
async function resolveGatewayUrl(api: OpunclehPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the Opuncleh iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the Opuncleh iOS app:",
            "",
            "```",
            qrAscii,
            "```",
