import type { SlackSlashCommandConfig } from "opuncleh/plugin-sdk/config-runtime";
...
export function resolveSlackSlashCommandConfig(
  ...
  const normalizedName = normalizeSlackSlashCommandName(raw?.name?.trim() || "opuncleh");
  const name = normalizedName || "opuncleh";
  return {
