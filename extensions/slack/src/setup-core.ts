...
import { formatDocsLink } from "opuncleh/plugin-sdk/setup-tools";
...
function enableSlackAccount(cfg: OpunclehConfig, accountId: string): OpunclehConfig {
...
function createSlackTokenCredential(params: {
  ...
  return {
    ...
    inspect: ({ cfg, accountId }: { cfg: OpunclehConfig; accountId: string }) => {
    ...
    applyUseEnv: ({ cfg, accountId }: { cfg: OpunclehConfig; accountId: string }) =>
...
export function createSlackSetupWizardBase(handlers: {
  ...
  const slackDmPolicy: ChannelSetupDmPolicy = {
    ...
    getCurrent: (cfg: OpunclehConfig) =>
      cfg.channels?.slack?.dmPolicy ?? cfg.channels?.slack?.dm?.policy ?? "pairing",
    setPolicy: (cfg: OpunclehConfig, policy) =>
      setLegacyChannelDmPolicyWithAllowFrom({
        cfg,
        channel,
        dmPolicy: policy,
  ...
  return {
    ...
    groupAccess: {
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: OpunclehConfig; accountId: string }) =>
        resolveSlackAccount({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: OpunclehConfig; accountId: string }) =>
        Object.entries(resolveSlackAccount({ cfg, accountId }).config.channels ?? {})
          .filter(([, value]) => value?.allow !== false && value?.enabled !== false)
          .map(([key]) => key),
      updatePrompt: ({ cfg, accountId }: { cfg: OpunclehConfig; accountId: string }) =>
    ...
    disable: (cfg: OpunclehConfig) => setSetupChannelEnabled(cfg, channel, false),
