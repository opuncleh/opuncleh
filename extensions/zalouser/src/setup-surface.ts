import {
  ...
  type DmPolicy,
  type OpunclehConfig,
} from "opuncleh/plugin-sdk/setup";
import {
...
function setZalouserDmPolicy(cfg: OpunclehConfig, dmPolicy: DmPolicy): OpunclehConfig {
...
function ensureZalouserPluginEnabled(cfg: OpunclehConfig): OpunclehConfig {
  const next: OpunclehConfig = {
    ...cfg,
    plugins: {
      ...cfg.plugins,
      entries: {
    ...
    if (parts.length === 0) {
      await prompter.note(
        [
          ...
          `Tip: use \`${formatCliCommand("opuncleh directory peers list --channel zalouser")}\` to look up people after onboarding.`,
        ].join("\n"),
...
const zalouserDmPolicy: ChannelSetupDmPolicy = {
  ...
  setPolicy: (cfg, policy) => setZalouserDmPolicy(cfg as OpunclehConfig, policy),
  promptAllowFrom: async ({ cfg, prompter, accountId }) => {
    const id =
      accountId && normalizeAccountId(accountId)
        ? (normalizeAccountId(accountId) ?? DEFAULT_ACCOUNT_ID)
        : resolveDefaultZalouserAccountId(cfg as OpunclehConfig);
    return await promptZalouserAllowFrom({
      cfg: cfg as OpunclehConfig,
      prompter,
      accountId: id,
    });
  },
...
export const zalouserSetupWizard: ChannelSetupWizard = {
  ...
  groupAccess: {
    ...
    setPolicy: ({ cfg, accountId, policy }) =>
      setZalouserGroupPolicy(cfg as OpunclehConfig, accountId, policy),
    resolveAllowlist: async ({ cfg, accountId, entries, prompter }) => {
      if (entries.length === 0) {
        await prompter.note(
          [
            ...
            `Tip: use \`${formatCliCommand("opuncleh directory groups list --channel zalouser")}\` after onboarding to find group IDs.`,
      ...
      const updatedAccount = resolveZalouserAccountSync({ cfg: cfg as OpunclehConfig, accountId });
      try {
    ...
    applyAllowlist: ({ cfg, accountId, resolved }) =>
      setZalouserGroupAllowlist(cfg as OpunclehConfig, accountId, resolved as string[]),
  },
