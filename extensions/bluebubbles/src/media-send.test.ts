...
import type { OpunclehConfig, PluginRuntime } from "opuncleh/plugin-sdk/bluebubbles";
...
function createConfig(overrides?: Record<string, unknown>): OpunclehConfig {
  return {
    channels: {
      bluebubbles: {
        ...overrides,
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "opuncleh-bb-media-"));
  tempDirs.push(dir);
  return dir;
}
