...
Read-only diff viewer plugin for **Opuncleh** agents.
...
Set plugin-wide defaults in `~/.opuncleh/opuncleh.json`:
...
Before:
Opuncleh supports plugins.

After:
Opuncleh supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/opuncleh-diffs`).
