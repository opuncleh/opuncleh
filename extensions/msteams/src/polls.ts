...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["opunclehPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["opuncleh", "pollId"]) ??
    readNestedString(value, ["opuncleh", "poll", "id"]) ??
    readNestedString(value, ["data", "opunclehPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "opuncleh", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          opunclehPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "opuncleh poll vote",
          displayText: "Vote recorded",
          value: { opunclehPollId: pollId, pollId },
        },
      },
    ],
  };
