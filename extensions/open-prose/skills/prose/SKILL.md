...
metadata: { "opuncleh": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Opuncleh `sessions_spawn`
- **File I/O** == Opuncleh `read`/`write`
- **Remote fetch** == Opuncleh `web_fetch` (or `exec` with curl when POST is required)
