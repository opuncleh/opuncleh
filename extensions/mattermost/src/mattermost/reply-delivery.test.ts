...
import type { OpunclehConfig } from "opuncleh/plugin-sdk/mattermost";
...
describe("deliverMattermostReplyPayload", () => {
  it("passes agent-scoped mediaLocalRoots when sending media paths", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "opuncleh-mm-state-"));
    ...
    try {
      ...
      const cfg = {} satisfies OpunclehConfig;
