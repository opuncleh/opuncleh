...
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__opuncleh_channel_id";
...
type InteractionCallbackConfig = Pick<OpunclehConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "opuncleh-mattermost-interactions").update(botToken).digest("hex");
}
