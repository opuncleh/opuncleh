...
export async function monitorMattermostProvider(opts: MonitorMattermostOpts = {}): Promise<void> {
  ...
  if (slashEnabled) {
    try {
      ...
      // Use the *runtime* listener port when available (e.g. `opuncleh gateway run --port <port>`).
      // The gateway sets OPUNCLEH_GATEWAY_PORT when it boots, but the config file may still contain
      // a different port.
      const envPortRaw = process.env.OPUNCLEH_GATEWAY_PORT?.trim();
      ...
      authorizeButtonClick: async ({ payload, post }) => {
        ...
        return {
          ...
          response: {
            ...
            ephemeral_text: `Opuncleh ignored this action for ${decision.roomLabel}.`,
          },
        };
      },
