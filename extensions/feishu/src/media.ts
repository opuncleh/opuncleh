...
import { withTempDownloadPath, type ClawdbotConfig } from "opuncleh/plugin-sdk/feishu";
import { mediaKindFromMime } from "opuncleh/plugin-sdk/media-runtime";
...
export async function downloadImageFeishu(params: {
  ...
  const buffer = await readFeishuResponseBuffer({
    response,
    tmpDirPrefix: "opuncleh-feishu-img-",
...
export async function downloadMessageResourceFeishu(params: {
  ...
  const buffer = await readFeishuResponseBuffer({
    response,
    tmpDirPrefix: "opuncleh-feishu-resource-",
