import type { AnyAgentTool, OpunclehPluginApi } from "opuncleh/plugin-sdk/feishu";
...
export function createToolFactoryHarness(cfg: OpunclehPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<OpunclehPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
