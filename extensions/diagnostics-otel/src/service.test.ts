...
vi.mock("opuncleh/plugin-sdk/diagnostics-otel", async () => {
  const actual = await vi.importActual<typeof import("opuncleh/plugin-sdk/diagnostics-otel")>(
    "opuncleh/plugin-sdk/diagnostics-otel",
  );
  return {
    ...actual,
...
import type { OpunclehPluginServiceContext } from "opuncleh/plugin-sdk/diagnostics-otel";
import { emitDiagnosticEvent } from "opuncleh/plugin-sdk/diagnostics-otel";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/opuncleh-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): OpunclehPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("opuncleh.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("opuncleh.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("opuncleh.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("opuncleh.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("opuncleh.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("opuncleh.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("opuncleh.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("opuncleh.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("opuncleh.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("opuncleh.webhook.processed");
    expect(spanNames).toContain("opuncleh.message.processed");
    expect(spanNames).toContain("opuncleh.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["opuncleh.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("opuncleh.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "opuncleh.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["opuncleh.reason"]).toBe("string");
    expect(String(attrs?.["opuncleh.reason"])).not.toContain(
