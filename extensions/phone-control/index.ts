...
import type { OpunclehPluginApi, OpunclehPluginService } from "opuncleh/plugin-sdk/phone-control";
...
function normalizeDenyList(cfg: OpunclehPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: OpunclehPluginApi["config"]): string[] {
...
export default function register(api: OpunclehPluginApi) {
  ...
  const timerService: OpunclehPluginService = {
