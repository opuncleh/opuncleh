...
import type { OpunclehPluginApi, OpunclehPluginToolContext } from "opuncleh/plugin-sdk/lobster";
...
function fakeApi(overrides: Partial<OpunclehPluginApi> = {}): OpunclehPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<OpunclehPluginToolContext> = {}): OpunclehPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "opuncleh-lobster-plugin-"));
  });
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: OpunclehPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
