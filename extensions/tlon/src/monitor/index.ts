import type { RuntimeEnv, ReplyPayload, OpunclehConfig } from "opuncleh/plugin-sdk/tlon";
import { createLoggerBackedRuntime, createReplyPrefixOptions } from "opuncleh/plugin-sdk/tlon";
...
export async function monitorTlonProvider(opts: MonitorTlonOpts = {}): Promise<void> {
  ...
  const cfg = core.config.loadConfig() as OpunclehConfig;
      ...
      if (senders.size > 0 && !senders.has(senderShip)) {
        ...
        runtime.log?.(
          `[tlon] ⚠️ SECURITY: Multiple users sharing DM session. ` +
            `Configure "session.dmScope: per-channel-peer" in Opuncleh config.`,
        );
        ...
        if (!sharedSessionWarningSent && effectiveOwnerShip) {
          ...
          const warningMsg =
            ...
            `Fix: Add to your Opuncleh config:\n` +
            `session:\n  dmScope: "per-channel-peer"\n\n` +
            `Docs: https://www.opuncleh.com/docs.html/concepts/session#secure-dm-mode`;
