...
import { resetInboundDedupe } from "opuncleh/plugin-sdk/reply-runtime";
import type { MsgContext } from "opuncleh/plugin-sdk/reply-runtime";
import type { GetReplyOptions, ReplyPayload } from "opuncleh/plugin-sdk/reply-runtime";
import type { MockFn } from "opuncleh/plugin-sdk/testing";
...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/opuncleh-telegram-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}.json`,
}));
...
vi.mock("opuncleh/plugin-sdk/web-media", () => ({
  loadWebMedia,
}));
...
vi.mock("opuncleh/plugin-sdk/config-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("opuncleh/plugin-sdk/config-runtime")>();
  return {
    ...actual,
    loadConfig,
  };
...
vi.mock("opuncleh/plugin-sdk/conversation-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("opuncleh/plugin-sdk/conversation-runtime")>();
  return {
    ...actual,
...
vi.mock("opuncleh/plugin-sdk/reply-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("opuncleh/plugin-sdk/reply-runtime")>();
  return {
    ...actual,
...
vi.mock("opuncleh/plugin-sdk/infra-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("opuncleh/plugin-sdk/infra-runtime")>();
  return {
    ...actual,
...
const DEFAULT_TELEGRAM_TEST_CONFIG: OpunclehConfig = {
  agents: {
    defaults: {
