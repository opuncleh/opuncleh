/**
 * Twitch channel plugin for Opuncleh.
 *
...
import { buildChannelConfigSchema } from "opuncleh/plugin-sdk/twitch";
 ...
 * for Opuncleh. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: OpunclehConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: OpunclehConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: OpunclehConfig): boolean => {
  ...
  resolver: {
    resolveTargets: async ({
      ...
      runtime: import("opuncleh/plugin-sdk/runtime-env").RuntimeEnv;
