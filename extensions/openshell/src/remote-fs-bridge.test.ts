...
describe("openshell remote fs bridge", () => {
  it("writes, reads, renames, and removes files without local host paths", async () => {
    const workspaceDir = await makeTempDir("opuncleh-openshell-remote-local-");
    const remoteWorkspaceDir = await makeTempDir("opuncleh-openshell-remote-workspace-");
    const remoteAgentDir = await makeTempDir("opuncleh-openshell-remote-agent-");
