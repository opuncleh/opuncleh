...
function getChannelConfig(cfg: OpunclehConfig): SynologyChatChannelConfig {
  return (cfg.channels?.[channel] as SynologyChatChannelConfig | undefined) ?? {};
}

function getRawAccountConfig(cfg: OpunclehConfig, accountId: string): SynologyChatAccountRaw {
...
function isSynologyChatConfigured(cfg: OpunclehConfig, accountId: string): boolean {
...
function resolveExistingAllowedUserIds(cfg: OpunclehConfig, accountId: string): string[] {
  ...
  textInputs: [
    {
      ...
      helpLines: [
        ...
        "This is the URL Opuncleh uses to send replies back to Chat.",
      ],
