# Google Gemini CLI Auth (Opuncleh plugin)
...
```bash
opuncleh plugins enable google-gemini-cli-auth
```
...
```bash
opuncleh models auth login --provider google-gemini-cli --set-default
```
...
- `OPUNCLEH_GEMINI_OAUTH_CLIENT_ID` / `GEMINI_CLI_OAUTH_CLIENT_ID`
- `OPUNCLEH_GEMINI_OAUTH_CLIENT_SECRET` / `GEMINI_CLI_OAUTH_CLIENT_SECRET`
