...
`opuncleh.invoke --each`) without adding new Opuncleh code per workflow.
...
This extension depends on Opuncleh internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Opuncleh extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.opuncleh/extensions` as a standalone plugin directory.
