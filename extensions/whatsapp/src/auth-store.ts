...
import { info, success } from "opuncleh/plugin-sdk/runtime-env";
...
import { resolveOAuthDir } from "opuncleh/plugin-sdk/state-paths";
import type { WebChannel } from "opuncleh/plugin-sdk/text-runtime";
import { jidToE164, resolveUserPath } from "opuncleh/plugin-sdk/text-runtime";
...
export async function pickWebChannel(
  ...
  if (!hasWeb) {
    throw new Error(
      `No WhatsApp Web session found. Run \`${formatCliCommand("opuncleh channels login --channel whatsapp --verbose")}\` to link.`,
    );
  }
  return choice;
}
