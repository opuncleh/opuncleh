...
import * as ssrf from "opuncleh/plugin-sdk/infra-runtime";
...
vi.mock("opuncleh/plugin-sdk/agent-runtime", () => ({
...
export function installWebAutoReplyTestHomeHooks() {
  beforeAll(async () => {
    tempHomeRoot = await fs.mkdtemp(path.join(os.tmpdir(), "opuncleh-web-home-suite-"));
  });
...
export async function makeSessionStore(
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "opuncleh-session-"));
