...
import { generateSecureUuid } from "opuncleh/plugin-sdk/infra-runtime";
...
import { getChildLogger } from "opuncleh/plugin-sdk/text-runtime";
...
import { markdownToWhatsApp } from "opuncleh/plugin-sdk/text-runtime";
import { toWhatsappJid } from "opuncleh/plugin-sdk/text-runtime";
...
export async function sendPollWhatsApp(
  to: string,
  poll: PollInput,
  options: { verbose: boolean; accountId?: string; cfg?: OpunclehConfig },
): Promise<{ messageId: string; toJid: string }> {
  const correlationId = generateSecureUuid();
  const startedAt = Date.now();
  const { listener: active } = requireActiveWebListener(options.accountId);
