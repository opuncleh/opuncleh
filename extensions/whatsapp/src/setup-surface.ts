...
import type { ChannelSetupWizard } from "opuncleh/plugin-sdk/setup";
import { formatCliCommand, formatDocsLink } from "opuncleh/plugin-sdk/setup-tools";
...
function mergeWhatsAppConfig(
  cfg: OpunclehConfig,
  patch: Partial<NonNullable<NonNullable<OpunclehConfig["channels"]>["whatsapp"]>>,
...
function setWhatsAppDmPolicy(cfg: OpunclehConfig, dmPolicy: DmPolicy): OpunclehConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: OpunclehConfig, allowFrom?: string[]): OpunclehConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: OpunclehConfig, selfChatMode: boolean): OpunclehConfig {
  return mergeWhatsAppConfig(cfg, { selfChatMode });
}

async function detectWhatsAppLinked(cfg: OpunclehConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppOwnerAllowFrom(params: {
  ...
  await prompter.note(
    "We need the sender/owner number so Opuncleh can allowlist you.",
    "WhatsApp number",
  );
...
async function promptWhatsAppDmAccess(params: {
  ...
  const phoneMode = await params.prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for Opuncleh" },
    ],
  });
...
export const whatsappSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, accountId, forceAllowFrom, prompter, runtime }) => {
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("opuncleh channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
