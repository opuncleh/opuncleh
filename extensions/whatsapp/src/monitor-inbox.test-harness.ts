...
import { resetLogger, setLoggerOverride } from "opuncleh/plugin-sdk/runtime-env";
...
vi.mock("opuncleh/plugin-sdk/conversation-runtime", () => getPairingStoreMocks());
...
export function installWebMonitorInboxUnitTestHooks(opts?: { authDir?: boolean }) {
  ...
  beforeEach(async () => {
    ...
    if (createAuthDir) {
      authDir = fsSync.mkdtempSync(path.join(os.tmpdir(), "opuncleh-auth-"));
    } else {
      authDir = undefined;
    }
  });
