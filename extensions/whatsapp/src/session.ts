...
import { VERSION } from "opuncleh/plugin-sdk/cli-runtime";
import { danger, success } from "opuncleh/plugin-sdk/runtime-env";
import { getChildLogger, toPinoLikeLogger } from "opuncleh/plugin-sdk/runtime-env";
import { ensureDir, resolveUserPath } from "opuncleh/plugin-sdk/text-runtime";
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === DisconnectReason.loggedOut) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("opuncleh channels login")}`,
              ),
            );
          }
        }
