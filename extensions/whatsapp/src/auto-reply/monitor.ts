import { formatCliCommand } from "opuncleh/plugin-sdk/cli-runtime";
import { waitForever } from "opuncleh/plugin-sdk/cli-runtime";
import { loadConfig } from "opuncleh/plugin-sdk/config-runtime";
import { createConnectedChannelStatusPatch } from "opuncleh/plugin-sdk/gateway-runtime";
import { formatDurationPrecise } from "opuncleh/plugin-sdk/infra-runtime";
...
import { resolveInboundDebounceMs } from "opuncleh/plugin-sdk/reply-runtime";
import { getReplyFromConfig } from "opuncleh/plugin-sdk/reply-runtime";
import { DEFAULT_GROUP_HISTORY_LIMIT } from "opuncleh/plugin-sdk/reply-runtime";
import { resolveAgentRoute } from "opuncleh/plugin-sdk/routing";
import { logVerbose } from "opuncleh/plugin-sdk/runtime-env";
import { registerUnhandledRejectionHandler } from "opuncleh/plugin-sdk/runtime-env";
import { getChildLogger } from "opuncleh/plugin-sdk/runtime-env";
import { defaultRuntime, type RuntimeEnv } from "opuncleh/plugin-sdk/runtime-env";
...
export async function monitorWebChannel(
  ...
  while (true) {
    ...
    if (loggedOut) {
      runtime.error(
        `WhatsApp session logged out. Run \`${formatCliCommand("opuncleh channels login --channel web")}\` to relink.`,
      );
      await closeListener();
      break;
    }
    ...
    if (isNonRetryableWebCloseStatus(statusCode)) {
      ...
      runtime.error(
        `WhatsApp Web connection closed (status ${statusCode}: session conflict). Resolve conflicting WhatsApp Web sessions, then relink with \`${formatCliCommand("opuncleh channels login --channel web")}\`. Stopping web monitoring.`,
      );
      await closeListener();
      break;
    }
