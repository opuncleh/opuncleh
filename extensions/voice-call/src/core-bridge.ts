...
function resolveOpunclehRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.OPUNCLEH_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set OPUNCLEH_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveOpunclehRoot(), "dist", "extensionAPI.js");
