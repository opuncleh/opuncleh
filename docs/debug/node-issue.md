...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Opuncleh
...
Running Opuncleh via Node with `tsx` fails at startup with:

```
[opuncleh] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `opuncleh status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch opuncleh.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node opuncleh.mjs status` works on Node 25.
