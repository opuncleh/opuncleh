---
summary: "CLI reference for `opuncleh plugins` (list, install, marketplace, uninstall, enable/disable, doctor)"
read_when:
...
opuncleh plugins uninstall <id>
opuncleh plugins doctor
opuncleh plugins update <id>
opuncleh plugins update --all
opuncleh plugins marketplace list <marketplace>
```

`info` is an alias for `inspect`.

Bundled plugins ship with Opuncleh but start disabled. Use `plugins enable` to
activate them.

Native Opuncleh plugins must ship `opuncleh.plugin.json` with an inline JSON
Schema (`configSchema`, even if empty). Compatible bundles use their own bundle
manifests instead.

`plugins list` shows `Format: opuncleh` or `Format: bundle`. Verbose list/info
...
```bash
opuncleh plugins install <path-or-spec>
opuncleh plugins install <npm-spec> --pin
opuncleh plugins install <plugin>@<marketplace>
opuncleh plugins install <plugin> --marketplace <marketplace>
```
...
If a bare install spec matches a bundled plugin id (for example `diffs`), Opuncleh
...
```bash
opuncleh plugins install <plugin-name> --marketplace <marketplace-name>
opuncleh plugins install <plugin-name> --marketplace <owner/repo>
opuncleh plugins install <plugin-name> --marketplace ./my-marketplace
```
...
For local paths and archives, Opuncleh auto-detects:

- native Opuncleh plugins (`opuncleh.plugin.json`)
...
```bash
opuncleh plugins install -l ./my-plugin
```
...
```bash
opuncleh plugins uninstall <id>
opuncleh plugins uninstall <id> --dry-run
opuncleh plugins uninstall <id> --keep-files
```
...
state dir extensions root (`$OPUNCLEH_STATE_DIR/extensions/<id>`). Use
...
opuncleh plugins update <id>
opuncleh plugins update --all
opuncleh plugins update <id> --dry-run
```
...
```bash
opuncleh plugins inspect <id>
opuncleh plugins inspect <id> --json
```
