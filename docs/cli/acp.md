...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to a Opuncleh Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`opuncleh acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Opuncleh gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
opuncleh acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
opuncleh acp --url wss://gateway-host:18789 --token-file ~/.opuncleh/gateway.token

# Attach to an existing session key
opuncleh acp --session agent:main:main

# Attach by label (must already exist)
opuncleh acp --session-label "support inbox"

# Reset the session key before the first prompt
opuncleh acp --session agent:main:main --reset-session
```
...
opuncleh acp client --server-args --url wss://gateway-host:18789 --token-file ~/.opuncleh/gateway.token

# Override the server command (default: opuncleh)
opuncleh acp client --server "node" --server-args opuncleh.mjs acp --url ws://127.0.0.1:19001
```
...
it to drive a Opuncleh Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `opuncleh acp` over stdio.

Example config (persisted):

```bash
opuncleh config set gateway.remote.url wss://gateway-host:18789
opuncleh config set gateway.remote.token <token>
```
...
```bash
opuncleh acp --session agent:main:main
opuncleh acp --session agent:design:main
opuncleh acp --session agent:qa:bug-123
```
...
Opuncleh bot over ACP, use `acpx` with its built-in `opuncleh` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx opuncleh` at `opuncleh acp`.
3. Target the Opuncleh session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Opuncleh ACP session
acpx opuncleh exec "Summarize the active Opuncleh session state."

# Persistent named session for follow-up turns
acpx opuncleh sessions ensure --name codex-bridge
acpx opuncleh -s codex-bridge --cwd /path/to/repo \
  "Ask my Opuncleh work agent for recent context relevant to this repo."
```

If you want `acpx opuncleh` to target a specific Gateway and session key every
time, override the `opuncleh` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "opuncleh": {
      "command": "env OPUNCLEH_HIDE_BANNER=1 OPUNCLEH_SUPPRESS_NOTES=1 opuncleh acp --url ws://127.0.0.1:18789 --token-file ~/.opuncleh/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Opuncleh checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env OPUNCLEH_HIDE_BANNER=1 OPUNCLEH_SUPPRESS_NOTES=1 node opuncleh.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an Opuncleh agent without scraping a terminal.
...
In Zed, open the Agent panel and select “Opuncleh ACP” to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`OPUNCLEH_GATEWAY_TOKEN`, `OPUNCLEH_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`OPUNCLEH_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `OPUNCLEH_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `opuncleh acp client` sets `OPUNCLEH_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `opuncleh`).
