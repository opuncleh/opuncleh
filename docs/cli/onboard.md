---
summary: "CLI reference for `opuncleh onboard` (interactive onboarding)"
read_when:
...
```bash
opuncleh onboard
opuncleh onboard --flow quickstart
opuncleh onboard --flow manual
opuncleh onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`OPUNCLEH_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `opuncleh gateway run`.
...
- Fastest first chat: `opuncleh dashboard` (Control UI, no channel setup).
