...
Config helpers for non-interactive edits in `opuncleh.json`: get/set/unset/validate
values by path and print the active config file. Run without a subcommand to
open the configure wizard (same as `opuncleh configure`).
...
opuncleh config set secrets.providers.vaultfile --provider-source file --provider-path /etc/opuncleh/secrets.json --provider-mode json
...
`opuncleh config set` supports four assignment styles:

1. Value mode: `opuncleh config set <path> <value>`
2. SecretRef builder mode:

```bash
opuncleh config set channels.discord.token \
...
```bash
opuncleh config set secrets.providers.vault \
  --provider-source exec \
  --provider-command /usr/local/bin/opuncleh-vault \
  --provider-arg read \
...
```bash
opuncleh config set --batch-json '[
  {
...
opuncleh config set secrets.providers.vaultfile \
  '{"source":"file","path":"/etc/opuncleh/secrets.json","mode":"json"}' \
  --strict-json
```
...
Use `--dry-run` to validate changes without writing `opuncleh.json`.
...
```json
{
  "ok": true,
  "operations": 1,
  "configPath": "~/.opuncleh/opuncleh.json",
  "inputModes": ["builder"],
  "checks": {
    "schema": false,
    "resolvability": true,
