---
summary: "Opuncleh CLI reference for `opuncleh` commands, subcommands, and options"
read_when:
...
- `--dev`: isolate state under `~/.opuncleh-dev` and shift default ports.
- `--profile <name>`: isolate state under `~/.opuncleh-<name>`.
- `--no-color`: disable ANSI colors.
- `--update`: shorthand for `opuncleh update` (source installs only).
...
Opuncleh uses a lobster palette for CLI output.
...
```
opuncleh [--dev] [--profile <name>] <command>
  setup
  onboard
  configure
  config
...
Note: plugins can add additional top-level commands (for example `opuncleh voicecall`).

## Security

- `opuncleh security audit` — audit config + local state for common security foot-guns.
- `opuncleh security audit --deep` — best-effort live Gateway probe.
- `opuncleh security audit --fix` — tighten safe defaults and chmod state/config.

## Secrets

- `opuncleh secrets reload` — re-resolve refs and atomically swap the runtime snapshot.
- `opuncleh secrets audit` — scan for plaintext residues, unresolved refs, and precedence drift.
- `opuncleh secrets configure` — interactive helper for provider setup + SecretRef mapping + preflight/apply.
- `opuncleh secrets apply --from <plan.json>` — apply a previously generated plan (`--dry-run` supported).

## Plugins

Manage extensions and their config:

- `opuncleh plugins list` — discover plugins (use `--json` for machine output).
- `opuncleh plugins inspect <id>` — show details for a plugin (`info` is an alias).
- `opuncleh plugins install <path|.tgz|npm-spec|plugin@marketplace>` — install a plugin (or add a plugin path to `plugins.load.paths`).
- `opuncleh plugins marketplace list <marketplace>` — list marketplace entries before install.
- `opuncleh plugins enable <id>` / `disable <id>` — toggle `plugins.entries.<id>.enabled`.
- `opuncleh plugins doctor` — report plugin load errors.
...
- `opuncleh memory status` — show index stats.
- `opuncleh memory index` — reindex memory files.
- `opuncleh memory search "<query>"` (or `--query "<query>"`) — semantic search over memory.
...
- `--workspace <dir>`: agent workspace path (default `~/.opuncleh/workspace`).
...
Non-interactive config helpers (get/set/unset/file/validate). Running `opuncleh config` with no
...
- `config set --dry-run`: validate assignments without writing `opuncleh.json` (exec SecretRef checks are skipped by default).
...
- `channels status`: check gateway reachability and channel health (`--probe` runs extra checks; use `opuncleh health` or `opuncleh status --deep` for gateway health probes).
- Tip: `channels status` prints warnings with suggested fixes when it can detect common misconfigurations (then points you to `opuncleh doctor`).
- `channels logs`: show recent channel logs from the gateway log file.
- `channels add`: wizard-style setup when no flags are passed; flags switch to non-interactive mode.
  - When adding a non-default account to a channel still using single-account top-level config, Opuncleh moves account-scoped values into `channels.<channel>.accounts.default` before writing the new account.
...
```bash
opuncleh channels add --channel telegram --account alerts --name "Alerts Bot" --token $TELEGRAM_BOT_TOKEN
opuncleh channels add --channel discord --account work --name "Work Bot" --token $DISCORD_BOT_TOKEN
opuncleh channels remove --channel discord --account work --delete
...
- `opuncleh message send --target +15555550123 --message "Hi"`
- `opuncleh message poll --channel discord --target channel:123 --poll-question "Snack?" --poll-option Pizza --poll-option Sushi`
...
Binding specs use `channel[:accountId]`. When `accountId` is omitted, Opuncleh may resolve account scope via channel defaults/plugin hooks; otherwise it is a channel binding without explicit account scope.
...
Opuncleh can surface provider usage/quota when OAuth/API creds are available.

Surfaces:

- `/status` (adds a short provider usage line when available)
- `opuncleh status --usage` (prints full provider breakdown)
...
- `gateway status` also surfaces legacy or extra gateway services when it can detect them (`--deep` adds system-level scans). Profile-named Opuncleh services are treated as first-class and aren't flagged as "extra".
...
opuncleh logs --limit 200
opuncleh logs --plain
opuncleh logs --json
opuncleh logs --no-color
```
...
`opuncleh models` is an alias for `models status`.
...
- `node` resolves gateway auth from env/config (no `--token`/`--password` flags): `OPUNCLEH_GATEWAY_TOKEN` / `OPUNCLEH_GATEWAY_PASSWORD`, then `gateway.auth.*`. In local mode, node host intentionally ignores `gateway.remote.*`; in `gateway.mode=remote`, `gateway.remote.*` participates per remote precedence rules.
...
Browser control CLI (dedicated Chrome/Brave/Edge/Chromium). See [`opuncleh browser`](/cli/browser) and the [Browser tool](/tools/browser).
