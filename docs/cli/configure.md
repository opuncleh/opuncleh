---
summary: "CLI reference for `opuncleh configure` (interactive configuration prompts)"
read_when:
...
Tip: `opuncleh config` without a subcommand opens the same wizard. Use
`opuncleh config get|set|unset` for non-interactive edits.
...
```bash
opuncleh configure
opuncleh configure --section model --section channels
```
