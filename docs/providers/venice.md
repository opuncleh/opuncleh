---
summary: "Use Venice AI privacy-focused models in Opuncleh"
read_when:
  - You want privacy-focused inference in Opuncleh
...
```bash
opuncleh onboard --auth-choice venice-api-key
```
...
```bash
opuncleh agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, Opuncleh shows all available Venice models. Pick based on your needs:
...
```bash
opuncleh models set venice/kimi-k2-5
opuncleh models set venice/claude-opus-4-6
```

List all available models:

```bash
opuncleh models list | grep venice
```

## Configure via `opuncleh configure`

1. Run `opuncleh configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Opuncleh automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
opuncleh agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
opuncleh agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
opuncleh agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
opuncleh agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
opuncleh agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `opuncleh models list` to see currently available models. Some models may be temporarily offline.
