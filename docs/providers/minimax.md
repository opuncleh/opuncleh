---
summary: "Use MiniMax M2.5 in Opuncleh"
read_when:
  - You want MiniMax models in Opuncleh
...
```bash
opuncleh plugins enable minimax-portal-auth  # skip if already loaded.
opuncleh gateway restart  # restart if gateway is already running
opuncleh onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/opuncleh/opuncleh/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `opuncleh.json`:
...
- Use `opuncleh models list` and `opuncleh models set minimax/MiniMax-M2.5` to switch.
...
- Running `opuncleh configure` and selecting **MiniMax M2.5**, or
