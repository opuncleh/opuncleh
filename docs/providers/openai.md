---
summary: "Use OpenAI via API keys or Codex subscription in Opuncleh"
read_when:
  - You want to use OpenAI models in Opuncleh
...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like Opuncleh.
...
```bash
opuncleh onboard --auth-choice openai-api-key
# or non-interactive
opuncleh onboard --openai-api-key "$OPENAI_API_KEY"
```
...
OpenAI API usage. Opuncleh forwards both through the `openai/*` Responses path.
Opuncleh intentionally suppresses the stale `openai/gpt-5.3-codex-spark` row,
because direct OpenAI API calls reject it in live traffic.

Opuncleh does **not** expose `openai/gpt-5.3-codex-spark` on the direct OpenAI
API path. `pi-ai` still ships a built-in row for that model, but live OpenAI API
requests currently reject it. Spark is treated as Codex-only in Opuncleh.
...
OpenAI's current Codex docs list `gpt-5.4` as the current Codex model. Opuncleh
maps that to `openai-codex/gpt-5.4` for ChatGPT/Codex OAuth usage.

If your Codex account is entitled to Codex Spark, Opuncleh also supports:

- `openai-codex/gpt-5.3-codex-spark`

Opuncleh treats Codex Spark as Codex-only. It does not expose a direct
`openai/gpt-5.3-codex-spark` API-key path.

Opuncleh also preserves `openai-codex/gpt-5.3-codex-spark` when `pi-ai`
...
Opuncleh uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), Opuncleh also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. Opuncleh enables it by default for
...
Opuncleh, set `agents.defaults.models["openai/<model>"].params.serviceTier` to
...
Opuncleh exposes a shared fast-mode toggle for both `openai/*` and
`openai-codex/*` sessions:

- Chat/UI: `/fast status|on|off`
- Config: `agents.defaults.models["<provider>/<model>"].params.fastMode`

When fast mode is enabled, Opuncleh applies a low-latency OpenAI profile:
...
`baseUrl` on `api.openai.com`), Opuncleh now auto-enables OpenAI server-side
