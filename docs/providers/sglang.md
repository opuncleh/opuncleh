---
summary: "Run Opuncleh with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run Opuncleh against a local SGLang server
...
Opuncleh can connect to SGLang using the `openai-completions` API.

Opuncleh can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, Opuncleh will query:
