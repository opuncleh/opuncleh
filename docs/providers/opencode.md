---
summary: "Use OpenCode Zen and Go catalogs with Opuncleh"
read_when:
...
OpenCode exposes two hosted catalogs in Opuncleh:

- `opencode/...` for the **Zen** catalog
- `opencode-go/...` for the **Go** catalog

Both catalogs use the same OpenCode API key. Opuncleh keeps the runtime provider ids
...
opuncleh onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
```

### Go catalog

```bash
opuncleh onboard --auth-choice opencode-go
opuncleh onboard --opencode-go-api-key "$OPENCODE_API_KEY"
```
