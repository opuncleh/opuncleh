...
- `/usage cost` in chats: local cost summary aggregated from Opuncleh session logs.
- CLI: `opuncleh status --usage` prints a full per-provider breakdown.
- CLI: `opuncleh channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
