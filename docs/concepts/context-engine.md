...
read_when:
  - You want to understand how Opuncleh assembles model context
...
A **context engine** controls how Opuncleh builds model context for each run.
It decides which messages to include, how to summarize older history, and how
to manage context across subagent boundaries.

Opuncleh ships with a built-in `legacy` engine. Plugins can register
...
cat ~/.opuncleh/opuncleh.json | jq '.plugins.slots.contextEngine'
```

### Installing a context engine plugin

Context engine plugins are installed like any other Opuncleh plugin. Install
first, then select the engine in the slot:

```bash
# Install from npm
opuncleh plugins install @martian-engineering/lossless-claw

# Or install from a local path (for development)
opuncleh plugins install -l ./my-context-engine
```
...
Every time Opuncleh runs a model prompt, the context engine participates at
...
Opuncleh currently calls one subagent lifecycle hook:
...
The `assemble` method can return a `systemPromptAddition` string. Opuncleh
...
The built-in `legacy` engine preserves Opuncleh's original behavior:
...
- `estimatedTokens` (required, `number`) — the engine's estimate of total
  tokens in the assembled context. Opuncleh uses this for compaction threshold
...
lifecycle. Opuncleh will not trigger the built-in auto-compaction; instead it
delegates entirely to the engine's `compact()` method. The engine may also
run compaction proactively in `afterTurn()`.

When `false` or unset, Opuncleh's built-in auto-compaction logic runs
...
Opuncleh resolves when it needs a context engine.

## Relationship to compaction and memory

- **Compaction** is one responsibility of the context engine. The legacy engine
  delegates to Opuncleh's built-in summarization. Plugin engines can implement
...
- Use `opuncleh doctor` to verify your engine is loading correctly.
...
- Engine errors are logged and surfaced in diagnostics. If a plugin engine
  fails to register or the selected engine id cannot be resolved, Opuncleh
  does not fall back automatically; runs fail until you fix the plugin or
  switch `plugins.slots.contextEngine` back to `"legacy"`.
- For development, use `opuncleh plugins install -l ./my-engine` to link a
