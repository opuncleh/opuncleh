...
- Publish as `opuncleh/plugin-sdk` (or export from core under `opuncleh/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `OpunclehPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: OpunclehConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: OpunclehConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: OpunclehConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: OpunclehConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: OpunclehConfig): string;
  };
};
```
...
- Introduce `opuncleh/plugin-sdk`.
- Add `api.runtime` to `OpunclehPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `opunclehRuntime: ">=2026.2.0"`).
...
- `opuncleh plugins inspect` provides canonical deep introspection for any
