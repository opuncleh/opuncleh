...
- [Quick start and first-run setup]
  ...
  - [I can't access docs.opuncleh.io (SSL error). What now?](#i-cant-access-docsopunclehai-ssl-error-what-now)
...
```bash
curl -fsSL https://opuncleh.io/install.sh | bash -s -- --install-method git
```
...
```bash
curl -fsSL https://opuncleh.io/install.sh | bash
...
### I can't access docs.opuncleh.io SSL error What now

Some Comcast/Xfinity connections incorrectly block `docs.opuncleh.io` via Xfinity
Advanced Security. Disable it or allowlist `docs.opuncleh.io`, then retry. More
...
```bash
curl -fsSL --proto '=https' --tlsv1.2 https://opuncleh.io/install.sh | bash -s -- --beta
```

```bash
curl -fsSL --proto '=https' --tlsv1.2 https://opuncleh.io/install.sh | bash -s -- --install-method git
```

Windows installer (PowerShell):
[https://opuncleh.io/install.ps1](https://opuncleh.io/install.ps1)
...
```bash
curl -fsSL https://opuncleh.io/install.sh | bash -s -- --verbose
```

Beta install with verbose:

```bash
curl -fsSL https://opuncleh.io/install.sh | bash -s -- --beta --verbose
```

For a hackable (git) install:

```bash
curl -fsSL https://opuncleh.io/install.sh | bash -s -- --install-method git --verbose
```
...
Set-PSDebug -Trace 1
& ([scriptblock]::Create((iwr -useb https://opuncleh.io/install.ps1))) -NoOnboard
Set-PSDebug -Trace 0
```
...
`claude setup-token` generates a **token string** via the Claude Code CLI (it is not available in the web console). You can run it on **any machine**. Choose **Anthropic token (paste setup-token)** in the wizard or paste it with `opuncleh models auth paste-token --provider anthropic`. The token is stored as an auth profile for the **anthropic** provider and used like an API key (no auto-refresh). More detail: [OAuth](/concepts/oauth).
...
Opuncleh supports **OpenAI Code (Codex)** via OAuth (ChatGPT sign-in). The wizard can run the OAuth flow and will set the default model to `openai-codex/gpt-5.4` when appropriate. See [Model providers](/concepts/model-providers) and [Wizard](/start/wizard).
...
like Opuncleh. The onboarding wizard can run the OAuth flow for you.
...
1. Enable the plugin: `opuncleh plugins enable google-gemini-cli-auth`
...
The onboarding wizard accepts `@username` input and resolves it to a numeric ID, but Opuncleh authorization uses numeric IDs only.
...
Showcase: [https://opuncleh.io/showcase](https://opuncleh.io/showcase)
