...
opuncleh system heartbeat last
opuncleh logs --follow
opuncleh config get agents.defaults.heartbeat
opuncleh channels status --probe
```
...
```bash
opuncleh config get agents.defaults.heartbeat.activeHours
opuncleh config get agents.defaults.heartbeat.activeHours.timezone
opuncleh config get agents.defaults.userTimezone || echo "agents.defaults.userTimezone not set"
opuncleh cron list
opuncleh logs --follow
```
