...
read_when:
  ...
  - Wiring external systems into Opuncleh
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your Opuncleh config directory (typically `~/.opuncleh/hooks/transforms`).
...
- `opuncleh webhooks gmail setup` writes `hooks.gmail` config for `opuncleh webhooks gmail run`.
