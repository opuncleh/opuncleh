...
Hooks provide an extensible event-driven system for automating actions in response to agent commands and events. Hooks are automatically discovered from directories and can be managed via CLI commands, similar to how skills work in Opuncleh.
...
- **Webhooks**: external HTTP webhooks that let other systems trigger work in Opuncleh. See [Webhook Hooks](/automation/webhook) or use `opuncleh webhooks` for Gmail helper commands.
...
- Extend Opuncleh's behavior without modifying core code

## Getting Started

### Bundled Hooks

Opuncleh ships with four bundled hooks that are automatically discovered:

- **💾 session-memory**: Saves session context to your agent workspace (default `~/.opuncleh/workspace/memory/`) when you issue `/new`
- **📎 bootstrap-extra-files**: Injects additional workspace bootstrap files from configured glob/path patterns during `agent:bootstrap`
- **📝 command-logger**: Logs all command events to `~/.opuncleh/logs/commands.log`
...
During onboarding (`opuncleh onboard`), you'll be prompted to enable recommended hooks. The wizard automatically discovers eligible hooks and presents them for selection.
...
2. **Managed hooks**: `~/.opuncleh/hooks/` (user-installed, shared across workspaces)
3. **Bundled hooks**: `<opuncleh>/dist/hooks/bundled/` (shipped with Opuncleh)
...
Hook packs are standard npm packages that export one or more hooks via `opuncleh.hooks` in
...
Hook packs can ship dependencies; they will be installed under `~/.opuncleh/hooks/<id>`.
Each `opuncleh.hooks` entry must stay inside the package directory after symlink
resolution; entries that escape are rejected.

Security note: `opuncleh hooks install` installs dependencies with `npm install --ignore-scripts`
...
homepage: https://docs.opuncleh.io/automation/hooks#my-hook
metadata:
...
The `metadata.opuncleh` object supports:
...
These hooks are not event-stream listeners; they let plugins synchronously adjust tool results before Opuncleh persists them.
...
- **Managed hooks** (`~/.opuncleh/hooks/`): Shared across workspaces

### 2. Create Directory Structure

```bash
mkdir -p ~/.opuncleh/hooks/my-hook
cd ~/.opuncleh/hooks/my-hook
```
...
metadata: { "opuncleh": { "emoji": "🎯", "events": ["command:new"] } }
---
...
# JSON output
opuncleh hooks info session-memory --json
```
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.opuncleh/workspace`)
...
```yaml
metadata: { "opuncleh": { "events": ["command:new"] } } # Specific
```

Rather than:

```yaml
metadata: { "opuncleh": { "events": ["command"] } } # General - more overhead
```
...
2. Verify HOOK.md format:
   ...
   ```bash
   cat ~/.opuncleh/hooks/my-hook/HOOK.md
...
1. Create hook directory:
   ...
   ```bash
   mkdir -p ~/.opuncleh/hooks/my-hook
   mv ./hooks/handlers/my-handler.ts ~/.opuncleh/hooks/my-hook/handler.ts
   ```
...
- [Bundled Hooks README](https://github.com/opuncleh/opuncleh/tree/main/src/hooks/bundled)
