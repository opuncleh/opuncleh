---
summary: "Opuncleh on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up Opuncleh on DigitalOcean
  - Looking for cheap VPS hosting for Opuncleh
title: "DigitalOcean"
---

# Opuncleh on DigitalOcean

## Goal

Run a persistent Opuncleh Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status opuncleh-gateway.service

# View logs
journalctl --user -u opuncleh-gateway.service -f
```
...
```bash
opuncleh config set gateway.bind tailnet
opuncleh gateway restart
```
...
```bash
opuncleh channels login whatsapp
# Scan QR code
```
...
- `~/.opuncleh/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u opuncleh --no-pager -n 50
```
