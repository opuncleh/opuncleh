...
- Same path: run the app from a fixed location (for Opuncleh, `dist/Opuncleh.app`).
...
```bash
sudo tccutil reset Accessibility ai.opuncleh.mac
sudo tccutil reset ScreenCapture ai.opuncleh.mac
...
Workaround: move files into the Opuncleh workspace (`~/.opuncleh/workspace`) if you want to avoid per-folder grants.
