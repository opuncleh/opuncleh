...
service via the external `opuncleh` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.opuncleh.gateway`
  (or `ai.opuncleh.<profile>` when using `--profile`/`OPUNCLEH_PROFILE`; legacy `com.opuncleh.*` is supported).
