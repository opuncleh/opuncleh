---
summary: "Opuncleh on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Opuncleh on a Raspberry Pi
  - Running Opuncleh on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# Opuncleh on Raspberry Pi

## Goal

Run a persistent, always-on Opuncleh Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# View logs
journalctl -u opuncleh -f
```

## 9) Access the Opuncleh Dashboard
...
```bash
ssh user@gateway-host 'opuncleh dashboard --no-open'
```
...
(or `OPUNCLEH_GATEWAY_TOKEN`) into Control UI settings.
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/opuncleh-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/opuncleh-compile-cache
mkdir -p /var/tmp/opuncleh-compile-cache
export OPUNCLEH_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `OPUNCLEH_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running Opuncleh, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit opuncleh
```

```ini
[Service]
Environment=OPUNCLEH_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/opuncleh-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep Opuncleh state/cache on SSD-backed storage to avoid SD-card
...
Most Opuncleh features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled opuncleh

# Enable if not
sudo systemctl enable opuncleh

# Start on boot
sudo systemctl start opuncleh
```
...
```bash
# Check logs
journalctl -u opuncleh --no-pager -n 100

# Common fix: rebuild
cd ~/opuncleh  # if using hackable install
npm run build
sudo systemctl restart opuncleh
```
