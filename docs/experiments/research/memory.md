---
summary: "Research notes: offline memory system for Lobstr workspaces (Markdown source-of-truth + derived index)"
read_when:
  - Designing workspace memory (~/.opuncleh/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Opuncleh integration
...
Target: Lobstr-style workspace (`agents.defaults.workspace`, default `~/.opuncleh/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.opuncleh/workspace` as canonical human-readable memory.
...
- `memory.md` remains “small + core-ish”: the things you want Lobstr to see every session.
...
```
~/.opuncleh/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Opuncleh**, but keep a separable core library.

### Why integrate into Opuncleh?

- Opuncleh already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `opuncleh memory recall "…" --k 25 --since 30d`
  - `opuncleh memory reflect --since 7d`
...
Pragmatic take for `~/.opuncleh/workspace`:
