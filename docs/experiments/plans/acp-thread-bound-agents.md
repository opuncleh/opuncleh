...
This plan defines how Opuncleh should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- Opuncleh core owns ACP control plane concerns
...
Opuncleh should not reimplement ACP transport internals in core.
Opuncleh should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in Opuncleh, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under Opuncleh state dir
...
- ACP event stream is normalized to Opuncleh reply chunks
...
For the first production backend (`extensions/acpx`), Opuncleh and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses Opuncleh `sessionKey`)
...
- Opuncleh consumes ndjson events from `acpx --format json --json-strict`
