...
Opuncleh features that can generate provider usage or paid API calls.
...
- `opuncleh status --usage` and `opuncleh channels list` show provider **usage windows**
...
Opuncleh can pick up credentials from:
...
- `opuncleh status --usage`
- `opuncleh models status --json`
...
`opuncleh models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
