...
This is the full reference for the `opuncleh onboard` CLI wizard.
...
<Steps>
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.opuncleh/credentials/oauth.json`; auth profiles live in `~/.opuncleh/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.opuncleh/credentials/oauth.json` (or `$OPUNCLEH_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.opuncleh/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `opuncleh pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `opuncleh configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `opuncleh health`.
    - Tip: `opuncleh status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
```bash
export OPUNCLEH_GATEWAY_TOKEN="your-token"
opuncleh onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env OPUNCLEH_GATEWAY_TOKEN
```
...
- Stores it under `~/.opuncleh/tools/signal-cli/<version>/`.
...
Typical fields in `~/.opuncleh/opuncleh.json`:
...
`opuncleh agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.opuncleh/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.opuncleh/agents/<agentId>/sessions/`.
