...
Opuncleh integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Opuncleh owns lifecycle when `channels.signal.autoStart=true`.
...
- Opuncleh spawns `imsg rpc` as a child process (legacy iMessage integration).
