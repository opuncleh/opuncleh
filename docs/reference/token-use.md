---
summary: "How Opuncleh builds prompt context and reports token usage + costs"
read_when:
...
Opuncleh tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Opuncleh assembles its own system prompt on every run. It includes:
...
For images, Opuncleh downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Opuncleh session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `opuncleh status --usage` and `opuncleh channels list` show
...
`cacheWrite`. If pricing is missing, Opuncleh shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Opuncleh can
...
Anthropic's 1M context window is currently beta-gated. Opuncleh can inject the
...
Opuncleh skips the `context-1m-*` beta header because Anthropic currently
