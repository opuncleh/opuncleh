...
- With Anthropic API-key auth profiles, Opuncleh seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, Opuncleh injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
Opuncleh exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.opuncleh/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$OPUNCLEH_STATE_DIR/logs/cache-trace.jsonl`
...
- `OPUNCLEH_CACHE_TRACE=1` enables cache tracing.
- `OPUNCLEH_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `OPUNCLEH_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `OPUNCLEH_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `OPUNCLEH_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
