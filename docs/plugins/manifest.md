...
read_when:
  - You are building a Opuncleh plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin Manifest"
---

# Plugin manifest (opuncleh.plugin.json)

This page is for the **native Opuncleh plugin manifest** only.
...
Opuncleh auto-detects those bundle layouts too, but they are not validated
against the `opuncleh.plugin.json` schema described here.

For compatible bundles, Opuncleh currently reads bundle metadata plus declared
skill roots, Claude command roots, Claude bundle `settings.json` defaults, and
supported hook packs when the layout matches Opuncleh runtime expectations.

Every native Opuncleh plugin **must** ship a `opuncleh.plugin.json` file in the
**plugin root**. Opuncleh uses this manifest to validate configuration
...
- `providerAuthEnvVars` (object): auth env vars keyed by provider id. Use this
  when Opuncleh should resolve provider credentials from env without loading
  plugin runtime first.
- `providerAuthChoices` (array): cheap onboarding/auth-choice metadata keyed by
  provider + auth method. Use this when Opuncleh should show a provider in
...
- The manifest is **required for native Opuncleh plugins**, including local filesystem loads.
