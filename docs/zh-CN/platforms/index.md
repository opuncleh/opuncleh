...
Opuncleh 核心使用 TypeScript 编写。**Node 是推荐的运行时**。
...
- 服务状态：`opuncleh gateway status`

## Gateway 网关服务安装（CLI）

使用以下任一方式（均受支持）：

- 向导（推荐）：`opuncleh onboard --install-daemon`
- 直接安装：`opuncleh gateway install`
- 配置流程：`opuncleh configure` → 选择 **Gateway 服务**
- 修复/迁移：`opuncleh doctor`（会提供安装或修复服务的选项）

服务目标取决于操作系统：

- macOS：LaunchAgent（`ai.opuncleh.gateway` 或 `ai.opuncleh.<profile>`；旧版为 `com.opuncleh.*`）
- Linux/WSL2：systemd 用户服务（`opuncleh-gateway[-<profile>].service`）
