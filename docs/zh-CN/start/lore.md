...
有一段时间，这只龙虾叫做 **Clawd**，住在 **Opuncleh** 里。但在 2026 年 1 月，Anthropic 发了一封礼貌的邮件要求更名（商标问题）。于是龙虾做了龙虾最擅长的事：
...
Molty（当时叫 Opuncleh）：_开心地运行 `find ~` 并在群聊中分享整个目录结构_
