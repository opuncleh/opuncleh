...
你可以在 `~/.opuncleh/opuncleh.json` 中覆盖此设置：
...
所有日志配置都在 `~/.opuncleh/opuncleh.json` 的 `logging` 下。
...
- Opuncleh 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `opuncleh plugins enable diagnostics-otel` 启用插件。
...
- `opuncleh.tokens`（计数器，属性：`opuncleh.token`、`opuncleh.channel`、`opuncleh.provider`、`opuncleh.model`）
- `opuncleh.cost.usd`（计数器，属性：`opuncleh.channel`、`opuncleh.provider`、`opuncleh.model`）
- `opuncleh.run.duration_ms`（直方图，属性：`opuncleh.channel`、`opuncleh.provider`、`opuncleh.model`）
- `opuncleh.context.tokens`（直方图，属性：`opuncleh.context`、`opuncleh.channel`、`opuncleh.provider`、`opuncleh.model`）

消息流：

- `opuncleh.webhook.received`（计数器，属性：`opuncleh.channel`、`opuncleh.webhook`）
- `opuncleh.webhook.error`（计数器，属性：`opuncleh.channel`、`opuncleh.webhook`）
- `opuncleh.webhook.duration_ms`（直方图，属性：`opuncleh.channel`、`opuncleh.webhook`）
- `opuncleh.message.queued`（计数器，属性：`opuncleh.channel`、`opuncleh.source`）
- `opuncleh.message.processed`（计数器，属性：`opuncleh.channel`、`opuncleh.outcome`）
- `opuncleh.message.duration_ms`（直方图，属性：`opuncleh.channel`、`opuncleh.outcome`）

队列 + 会话：

- `opuncleh.queue.lane.enqueue`（计数器，属性：`opuncleh.lane`）
- `opuncleh.queue.lane.dequeue`（计数器，属性：`opuncleh.lane`）
- `opuncleh.queue.depth`（直方图，属性：`opuncleh.lane` 或 `opuncleh.channel=heartbeat`）
- `opuncleh.queue.wait_ms`（直方图，属性：`opuncleh.lane`）
- `opuncleh.session.state`（计数器，属性：`opuncleh.state`、`opuncleh.reason`）
- `opuncleh.session.stuck`（计数器，属性：`opuncleh.state`）
- `opuncleh.session.stuck_age_ms`（直方图，属性：`opuncleh.state`）
- `opuncleh.run.attempt`（计数器，属性：`opuncleh.attempt`）
...
- `opuncleh.webhook.error`
  - `opuncleh.channel`、`opuncleh.webhook`、`opuncleh.chatId`、`opuncleh.error`
- `opuncleh.message.processed`
  - `opuncleh.channel`、`opuncleh.outcome`、`opuncleh.chatId`、`opuncleh.messageId`、`opuncleh.sessionKey`、`opuncleh.sessionId`、`opuncleh.reason`
- `opuncleh.session.stuck`
  - `opuncleh.state`、`opuncleh.ageMs`、`opuncleh.queueDepth`、`opuncleh.sessionKey`、`opuncleh.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `opuncleh doctor`。
