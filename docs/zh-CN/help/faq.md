...
- [快速开始与首次运行设置](#quick-start-and-firstrun-setup)
  ...
  - [无法访问 docs.opuncleh.com（SSL 错误），怎么办？](#i-cant-access-docsopunclehai-ssl-error-what-now)
...
### 无法访问 docs.opuncleh.com（SSL 错误），怎么办

一些 Comcast/Xfinity 连接通过 Xfinity Advanced Security 错误地拦截了 `docs.opuncleh.com`。禁用该功能或将 `docs.opuncleh.com` 加入白名单，然后重试。更多详情：[故障排除](/help/troubleshooting#docsopunclehai-shows-an-ssl-error-comcastxfinity)。
...
https://opuncleh.com/install.ps1
...
`claude setup-token` 通过 Claude Code CLI 生成一个**令牌字符串**（在 Web 控制台中不可用）。你可以在**任何机器**上运行它。在新手引导中选择 **Anthropic token (paste setup-token)** 或使用 `opuncleh models auth paste-token --provider anthropic` 粘贴。令牌作为 **anthropic** 提供商的认证配置文件存储，像 API 密钥一样使用（无自动刷新）。更多详情：[OAuth](/concepts/oauth)。
...
复制它打印的令牌，然后在新手引导中选择 **Anthropic token (paste setup-token)**。如果你想在 Gateway 网关主机上运行，使用 `opuncleh models auth setup-token --provider anthropic`。如果你在其他地方运行了 `claude setup-token`，在 Gateway 网关主机上使用 `opuncleh models auth paste-token --provider anthropic` 粘贴。参阅 [Anthropic](/providers/anthropic)。
...
Opuncleh 通过 OAuth（ChatGPT 登录）支持 **OpenAI Code (Codex)**。新手引导可以运行 OAuth 流程，并在适当时将默认模型设置为 `openai-codex/gpt-5.2`。参阅[模型提供商](/concepts/model-providers)和[CLI 新手引导](/start/wizard)。

### 是否支持 OpenAI 订阅认证（Codex OAuth）

是的。Opuncleh 完全支持 **OpenAI Code (Codex) 订阅 OAuth**。新手引导可以为你运行 OAuth 流程。
...
1. 启用插件：`opuncleh plugins enable google`
...
展示：https://opuncleh.com/showcase
