...
Opuncleh supports **camera capture** for agent workflows:
...
```bash
opuncleh nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
opuncleh nodes camera snap --node <id> --facing front
opuncleh nodes camera clip --node <id> --duration 3000
opuncleh nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`opuncleh.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `opuncleh` CLI to invoke camera commands on the macOS node.

Examples:

```bash
opuncleh nodes camera list --node <id>            # list camera ids
opuncleh nodes camera snap --node <id>            # prints MEDIA:<path>
opuncleh nodes camera snap --node <id> --max-width 1280
opuncleh nodes camera snap --node <id> --delay-ms 2000
opuncleh nodes camera snap --node <id> --device-id <id>
opuncleh nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
opuncleh nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
opuncleh nodes camera clip --node <id> --device-id <id>
opuncleh nodes camera clip --node <id> --no-audio
```

Notes:

- `opuncleh nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
opuncleh nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
