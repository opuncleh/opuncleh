---
summary: "Deploy Opuncleh Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run Opuncleh on a Kubernetes cluster
  - You want to test Opuncleh in a Kubernetes environment
title: "Kubernetes"
---

# Opuncleh on Kubernetes

A minimal starting point for running Opuncleh on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

Opuncleh is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/opuncleh 18789:18789 -n opuncleh
...
```bash
kubectl get secret opuncleh-secrets -n opuncleh -o jsonpath='{.data.OPUNCLEH_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: opuncleh (configurable via OPUNCLEH_NAMESPACE)
├── Deployment/opuncleh        # Single pod, init container + gateway
├── Service/opuncleh           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/opuncleh-config  # opuncleh.json + AGENTS.md
└── Secret/opuncleh-secrets    # Gateway token + API keys
```
...
Edit `opuncleh.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret opuncleh-secrets -n opuncleh \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/opuncleh -n opuncleh
```

### Custom namespace

```bash
OPUNCLEH_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/opuncleh/opuncleh:2026.3.1
```
...
└── manifests/
    ...
    ├── configmap.yaml          # opuncleh.json + AGENTS.md
