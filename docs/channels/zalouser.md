...
read_when:
  - Setting up Zalo Personal for Opuncleh
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Opuncleh.
...
- Install via CLI: `opuncleh plugins install @opuncleh/zalouser`
- Or from a source checkout: `opuncleh plugins install ./extensions/zalouser`
...
- `opuncleh pairing list zalouser`
- `opuncleh pairing approve zalouser <code>`
...
- On startup, Opuncleh resolves group/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
...
- When a group message is skipped because mention is required, Opuncleh stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in Opuncleh state. Example:
...
- Opuncleh sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Opuncleh sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `opuncleh channels logout --channel zalouser && opuncleh channels login --channel zalouser`
...
- The channel now runs fully in Opuncleh without external CLI binaries.
