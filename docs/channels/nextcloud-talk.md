...
```bash
opuncleh plugins install @opuncleh/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
opuncleh plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Opuncleh" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `opuncleh pairing list nextcloud-talk`
  - `opuncleh pairing approve nextcloud-talk <CODE>`
