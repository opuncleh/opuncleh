...
read_when:
  - Setting up Twitch chat integration for Opuncleh
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Opuncleh connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
opuncleh plugins install @opuncleh/twitch
```

Local checkout (when running from a git repo):

```bash
opuncleh plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `OPUNCLEH_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "opuncleh", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use OPUNCLEH_TWITCH_ACCESS_TOKEN env var)
...
```bash
OPUNCLEH_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
