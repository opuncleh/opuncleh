---
summary: "Mattermost bot setup and Opuncleh config"
read_when:
...
```bash
opuncleh plugins install @opuncleh/mattermost
```

Local checkout (when running from a git repo):

```bash
opuncleh plugins install ./extensions/mattermost
```
...
4. Configure Opuncleh and start the gateway.
...
Native slash commands are opt-in. When enabled, Opuncleh registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, Opuncleh derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as Opuncleh.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to Opuncleh.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from Opuncleh, not `404`.
...
- Approve via:
  - `opuncleh pairing list mattermost`
  - `opuncleh pairing approve mattermost <CODE>`
...
Use these target formats with `opuncleh message send` or cron/webhooks:
...
Opuncleh resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), Opuncleh sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When Opuncleh sends to a Mattermost DM target and needs to resolve the direct channel first, it
...
- If `interactions.callbackBaseUrl` is omitted, Opuncleh derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and Opuncleh run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="opuncleh-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"opuncleh-mattermost-interactions",
...
`opuncleh message send` and cron/webhook deliveries.
