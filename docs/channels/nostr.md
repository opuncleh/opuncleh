...
read_when:
  - You want Opuncleh to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Opuncleh to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- Onboarding (`opuncleh onboard`) and `opuncleh channels add` list optional channel plugins.
...
```bash
opuncleh plugins install @opuncleh/nostr
```

Use a local checkout (dev workflows):

```bash
opuncleh plugins install --link <path-to-opuncleh>/extensions/nostr
```
...
opuncleh channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
...
```json
{
  "channels": {
    "nostr": {
      ...
      "profile": {
        "name": "opuncleh",
        "displayName": "Opuncleh",
        "about": "Personal assistant DM bot",
        "picture": "https://example.com/avatar.png",
        "banner": "https://example.com/banner.png",
        "website": "https://example.com",
        "nip05": "opuncleh@example.com",
        "lud16": "opuncleh@example.com"
      }
    }
  }
}
