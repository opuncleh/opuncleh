...
Matrix is an open, decentralized messaging protocol. Opuncleh connects as a Matrix **user**
...
```bash
opuncleh plugins install @opuncleh/matrix
```

Local checkout (when running from a git repo):

```bash
opuncleh plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `opuncleh plugins install @opuncleh/matrix`
   - From a local checkout: `opuncleh plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Opuncleh calls the same
     login endpoint, stores the access token in `~/.opuncleh/credentials/matrix/credentials.json`,
...
- On first connection, Opuncleh requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Opuncleh logs a warning.
...
`~/.opuncleh/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `opuncleh pairing list matrix`
  - `opuncleh pairing approve matrix <CODE>`
...
- On startup, Opuncleh resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
