...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Opuncleh to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current Opuncleh releases, so no separate plugin install
is required.
...
```bash
opuncleh plugins install @opuncleh/feishu
```
...
If you just installed Opuncleh, run onboarding:
...
2. Configuring app credentials in Opuncleh
...
1. You already ran `opuncleh channels add` for Feishu
2. The gateway is running (`opuncleh gateway status`)
...
```bash
opuncleh pairing approve feishu <CODE>
```
...
2. Run `opuncleh logs --follow` and look for `chat_id`
...
2. Run `opuncleh logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `opuncleh gateway status`  | Show gateway status           |
| `opuncleh gateway install` | Install/start gateway service |
| `opuncleh gateway stop`    | Stop gateway service          |
| `opuncleh gateway restart` | Restart gateway service       |
| `opuncleh logs --follow`   | Tail gateway logs             |
...
4. Check logs: `opuncleh logs --follow`
...
5. Ensure the gateway is running: `opuncleh gateway status`
6. Check logs: `opuncleh logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.opuncleh/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.opuncleh/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
