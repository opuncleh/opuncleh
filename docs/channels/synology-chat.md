---
summary: "Synology Chat webhook setup and Opuncleh config"
read_when:
  - Setting up Synology Chat with Opuncleh
...
```bash
opuncleh plugins install ./extensions/synology-chat
```
...
1. Install and enable the Synology Chat plugin.
   - `opuncleh onboard` now shows Synology Chat in the same channel setup list as `opuncleh channels add`.
   - Non-interactive setup: `opuncleh channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your Opuncleh gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in Opuncleh.
   - Guided: `opuncleh onboard`
   - Direct: `opuncleh channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
...
- Pairing approvals work with:
  - `opuncleh pairing list synology-chat`
  - `opuncleh pairing approve synology-chat <CODE>`
...
```bash
opuncleh message send --channel synology-chat --target 123456 --text "Hello from Opuncleh"
opuncleh message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
