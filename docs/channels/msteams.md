...
```bash
opuncleh plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
Opuncleh will offer the local install path automatically.
...
3. Configure Opuncleh with those credentials.
...
- Talk to Opuncleh via Teams DMs, group chats, or channels.
...
- On startup, Opuncleh resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.opuncleh/opuncleh.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Opuncleh, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `opuncleh-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `opuncleh plugins install @opuncleh/msteams`
   - From a local checkout: `opuncleh plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "Opuncleh in Teams", "full": "Opuncleh in Teams" },
...
Opuncleh handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Opuncleh only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/OpunclehShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

Opuncleh sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `opuncleh message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.opuncleh/msteams-polls.json`.
...
```bash
opuncleh message send --channel msteams \
...
opuncleh message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
opuncleh message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
opuncleh message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
opuncleh message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
