...
```
  ...
  <Tab title="HTTP Events API mode">
    <Steps>
      ...
      <Step title="Configure Opuncleh HTTP mode">
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `opuncleh pairing approve slack <code>`.
...
- The interactive callback values are Opuncleh-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, Opuncleh falls back to the original text reply instead of sending an invalid blocks payload.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while Opuncleh is processing a reply, then removes it when the run finishes. This is a useful fallback when Slack native assistant typing is unavailable, especially in DMs.
...
```json
{
  "display_information": {
    "name": "Opuncleh",
    "description": "Slack connector for Opuncleh"
  },
  "features": {
    "bot_user": {
      "display_name": "Opuncleh",
      "always_online": false
    },
    "app_home": {
    ...
    "slash_commands": [
      {
        "command": "/opuncleh",
        "description": "Send a message to Opuncleh",
        "should_escape": false
      }
    ]
  },
...
Opuncleh supports Slack native text streaming via the Agents and AI Apps API.
...
- If streaming fails mid-reply, Opuncleh falls back to normal delivery for remaining payloads.
