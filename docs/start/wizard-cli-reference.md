...
read_when:
  - You need detailed behavior for opuncleh onboard
...
This page is the full reference for `opuncleh onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.opuncleh/opuncleh.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - CLI `--reset` defaults to `config+creds+sessions`; use `--reset-scope full` to also remove workspace.
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `opuncleh doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `opuncleh pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `opuncleh health`.
    - `opuncleh status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
- OAuth credentials: `~/.opuncleh/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.opuncleh/agents/<agentId>/agent/auth-profiles.json`
...
`~/.opuncleh/credentials/oauth.json` (or `$OPUNCLEH_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`opuncleh onboard`](/cli/onboard)
