...
The onboarding wizard is the **recommended** way to set up Opuncleh on macOS,
...
`opuncleh dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`opuncleh configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.opuncleh/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, the wizard asks you to run `opuncleh doctor` first.
</Note>
...
Use `opuncleh agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.opuncleh/workspace-<agentId>`.
...
- CLI command reference: [`opuncleh onboard`](/cli/onboard)
