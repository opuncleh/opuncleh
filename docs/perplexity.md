...
Opuncleh supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Opuncleh also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `opuncleh configure --section web`. It stores the key in
`~/.opuncleh/opuncleh.json` under `tools.web.search.perplexity.apiKey`.
...
`~/.opuncleh/.env` (or your service environment). See [Env vars](/help/faq#how-does-opuncleh-load-environment-variables).
