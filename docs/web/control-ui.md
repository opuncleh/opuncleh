...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/opuncleh`)
...
If the page fails to load, start the Gateway first: `opuncleh gateway`.
...
you revoke it with `opuncleh devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.opuncleh/opuncleh.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop opuncleh`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Opuncleh
...
Opuncleh **blocks** Control UI connections without device identity.
