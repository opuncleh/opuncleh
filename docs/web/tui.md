...
```bash
opuncleh tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `opuncleh tui --deliver`
...
- Local shell commands receive `OPUNCLEH_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `OPUNCLEH_THEME=light` before launching `opuncleh tui`.
- To force the original dark palette instead, set `OPUNCLEH_THEME=dark`.
...
- Check the Gateway logs: `opuncleh logs --follow`.
- Confirm the agent can run: `opuncleh status` and `opuncleh models status`.
...
- No agents in picker: check `opuncleh agents list` and your routing config.
