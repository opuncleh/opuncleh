...
A plugin is just a **small code module** that extends Opuncleh with extra
...
1. **Manifest + discovery**
   ...
   `opuncleh.plugin.json` plus package metadata first.
...
Opuncleh plugins are **TypeScript modules** loaded at runtime via jiti. **Config
...
2. read `opuncleh.plugin.json` and package metadata
...
- `opuncleh/plugin-sdk/core` for generic plugin APIs, provider auth types, and shared helpers.
- `opuncleh/plugin-sdk/compat` for bundled/internal plugin code that needs broader shared runtime helpers than `core`.
- `opuncleh/plugin-sdk/telegram` for Telegram channel plugins.
- `opuncleh/plugin-sdk/discord` for Discord channel plugins.
- `opuncleh/plugin-sdk/slack` for Slack channel plugins.
- `opuncleh/plugin-sdk/signal` for Signal channel plugins.
- `opuncleh/plugin-sdk/imessage` for iMessage channel plugins.
- `opuncleh/plugin-sdk/whatsapp` for WhatsApp channel plugins.
...
- Bundled extension-specific subpaths are also available:
  ...
  `opuncleh/plugin-sdk/feishu`,
  `opuncleh/plugin-sdk/google-gemini-cli-auth`, `opuncleh/plugin-sdk/googlechat`,
...
4. Bundled extensions (shipped with Opuncleh, mostly disabled by default)

- `<opuncleh>/extensions/*`

Most bundled plugins must be enabled explicitly via
`plugins.entries.<id>.enabled` or `opuncleh plugins enable <id>`.
...
Each plugin must include a `opuncleh.plugin.json` file in its root. If a path
...
Channel plugins can advertise onboarding metadata via `opuncleh.channel` and
...
2. **Wizard integration**
   `wizard.onboarding` adds an entry to `opuncleh onboard`.
...
- `wizard.onboarding` adds a provider choice to `opuncleh onboard`.
