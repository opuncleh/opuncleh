---
summary: "Opuncleh plugins/extensions: discovery, config, and safety"
read_when:
...
- a native **Opuncleh plugin** (`opuncleh.plugin.json` + runtime module), or
- a compatible **bundle** (`.codex-plugin/plugin.json` or `.claude-plugin/plugin.json`)

Both show up under `opuncleh plugins`, but only native Opuncleh plugins execute
runtime code in-process.

Most of the time, you’ll use plugins when you want a feature that’s not built
into core Opuncleh yet (or you want to keep optional features out of your main
install).
...
```bash
opuncleh plugins install @opuncleh/voice-call
```
...
```bash
opuncleh plugins install ./my-bundle
...
Opuncleh resolves known Claude marketplace names from
...
Capabilities are the public plugin model. Every native Opuncleh plugin
...
Opuncleh classifies every loaded plugin into a shape based on its actual
...
Use `opuncleh plugins inspect <id>` to see a plugin's shape and capability
...
Opuncleh's plugin system has four layers:

1. **Manifest + discovery**
   Opuncleh finds candidate plugins from configured paths, workspace roots,
   global extension roots, and bundled extensions. Discovery reads native
   `opuncleh.plugin.json` manifests plus supported bundle manifests first.
2. **Enablement + validation**
   Core decides whether a discovered plugin is enabled, disabled, blocked, or
   selected for an exclusive slot such as memory.
3. **Runtime loading**
   Native Opuncleh plugins are loaded in-process via jiti and register
...
4. **Surface consumption**
   The rest of Opuncleh reads the registry to expose tools, channels, provider
...
That split lets Opuncleh validate config, explain missing/disabled plugins, and
...
normal chat actions. Opuncleh keeps one shared `message` tool in core, and
...
Opuncleh treats a native plugin as the ownership boundary for a **company** or a
**feature**, not as a grab bag of unrelated integrations.

That means:

- a company plugin should usually own all of that company's Opuncleh-facing
  surfaces
...
So if Opuncleh adds a new domain such as video, the first question is not
...
A company plugin should feel cohesive from the outside. If Opuncleh has shared
contracts for models, speech, media understanding, and web search, a vendor can
own all of its surfaces in one place:

```ts
import type { OpunclehPluginDefinition } from "opuncleh/plugin-sdk";
import {
  buildOpenAISpeechProvider,
  createPluginBackedWebSearchProvider,
  describeImageWithModel,
  transcribeOpenAiCompatibleAudio,
} from "opuncleh/plugin-sdk";

const plugin: OpunclehPluginDefinition = {
  id: "exampleai",
  name: "ExampleAI",
  register(api) {
...
Opuncleh already treats image/audio/video understanding as one shared
...
If Opuncleh adds a new domain later, such as video generation, use the same
...
Opuncleh also recognizes two compatible external bundle layouts:
...
native Opuncleh plugin sources. Opuncleh resolves the marketplace entry first,
...
- supported now: Cursor `.cursor/commands/*.md` roots, mapped into the normal
  Opuncleh skill loader
- supported now: Codex bundle hook directories that use the Opuncleh hook-pack
...
Bundle hook support is limited to the normal Opuncleh hook directory format
...
Native Opuncleh plugins run **in-process** with the Gateway. They are not
...
- a malicious native plugin is equivalent to arbitrary code execution inside
  the Opuncleh process

Compatible bundles are safer by default because Opuncleh currently treats them
...
- Microsoft Teams is plugin-only as of 2026.1.15; install `@opuncleh/msteams` if you use Teams.
- Memory (Core) — bundled memory search plugin (enabled by default via `plugins.slots.memory`)
- Memory (LanceDB) — bundled long-term memory plugin (auto-recall/capture; set `plugins.slots.memory = "memory-lancedb"`)
- [Voice Call](/plugins/voice-call) — `@opuncleh/voice-call`
- [Zalo Personal](/plugins/zalouser) — `@opuncleh/zalouser`
- [Matrix](/channels/matrix) — `@opuncleh/matrix`
- [Nostr](/channels/nostr) — `@opuncleh/nostr`
- [Zalo](/channels/zalo) — `@opuncleh/zalo`
- [Microsoft Teams](/channels/msteams) — `@opuncleh/msteams`
...
Native Opuncleh plugins are **TypeScript modules** loaded at runtime via jiti.
...
Native Opuncleh plugins can register capabilities and surfaces:
...
Native Opuncleh plugins run **in‑process** with the Gateway, so treat them as trusted code.
...
against explicit surfaces that Opuncleh understands, validates, and can expose
...
`OpunclehPluginApi`. That contract defines the supported registration points and
...
2. **contract tests**
   ...
   Opuncleh can assert ownership explicitly. Today this is used for model
   providers, speech providers, web search providers, and bundled registration
   ownership.

The practical effect is that Opuncleh knows, up front, which plugin owns which
...
- ad hoc runtime objects that are not part of `OpunclehPluginApi` or
  `api.runtime`
...
Opuncleh exports capabilities, not implementation convenience.
...
Use `opuncleh plugins inspect <id>` for deep plugin introspection. This is the
canonical command for understanding a plugin's shape and registration behavior.

```bash
opuncleh plugins inspect openai
opuncleh plugins inspect openai --json
```
...
Opuncleh still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, Opuncleh uses hooks in this rough order:

1. `catalog`
   Publish provider config into `models.providers` during `models.json`
   generation.
2. built-in/discovered model lookup
   Opuncleh tries the normal registry/catalog path first.
...
that still runs on Opuncleh's normal inference loop.
...
- OpenRouter uses `catalog` plus `resolveDynamicModel` and
  ...
  model ids before Opuncleh's static catalog updates.
...
At startup, Opuncleh does roughly this:
...
The manifest is the control-plane source of truth. Opuncleh uses it to:
...
Opuncleh keeps short in-process caches for:
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as Opuncleh adds those
...
- If Opuncleh adds a new capability such as video generation later, define the
...
- Opuncleh only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "Opuncleh plugin runtime helpers",
    count: 5,
  },
});
```
...
Use SDK subpaths instead of the monolithic `opuncleh/plugin-sdk` import when
authoring plugins:

- `opuncleh/plugin-sdk/core` for the smallest generic plugin-facing contract.
- Domain subpaths such as `opuncleh/plugin-sdk/channel-config-helpers`,
  `opuncleh/plugin-sdk/channel-config-schema`,
  `opuncleh/plugin-sdk/channel-policy`,
  `opuncleh/plugin-sdk/lazy-runtime`,
  `opuncleh/plugin-sdk/reply-history`,
  `opuncleh/plugin-sdk/routing`,
  `opuncleh/plugin-sdk/runtime-store`, and
  `opuncleh/plugin-sdk/directory-runtime` for shared runtime/config helpers.
- `opuncleh/plugin-sdk/compat` remains as a legacy migration surface for older
...
- Bundled extension internals remain private. External plugins should use only
  `opuncleh/plugin-sdk/*` subpaths. Opuncleh core/test code may use the repo
...
- `opuncleh/plugin-sdk/telegram` for Telegram channel plugin types and shared channel-facing helpers. Built-in Telegram implementation internals stay private to the bundled extension.
- `opuncleh/plugin-sdk/discord` for Discord channel plugin types and shared channel-facing helpers. Built-in Discord implementation internals stay private to the bundled extension.
- `opuncleh/plugin-sdk/slack` for Slack channel plugin types and shared channel-facing helpers. Built-in Slack implementation internals stay private to the bundled extension.
- `opuncleh/plugin-sdk/signal` for Signal channel plugin types and shared channel-facing helpers. Built-in Signal implementation internals stay private to the bundled extension.
- `opuncleh/plugin-sdk/imessage` for iMessage channel plugin types and shared channel-facing helpers. Built-in iMessage implementation internals stay private to the bundled extension.
- `opuncleh/plugin-sdk/whatsapp` for WhatsApp channel plugin types and shared channel-facing helpers. Built-in WhatsApp implementation internals stay private to the bundled extension.
- `opuncleh/plugin-sdk/line` for LINE channel plugins.
- `opuncleh/plugin-sdk/msteams` for the bundled Microsoft Teams plugin surface.
- Bundled extension-specific subpaths are also available:
  `opuncleh/plugin-sdk/acpx`, `opuncleh/plugin-sdk/bluebubbles`,
  `opuncleh/plugin-sdk/copilot-proxy`, `opuncleh/plugin-sdk/device-pair`,
  `opuncleh/plugin-sdk/diagnostics-otel`, `opuncleh/plugin-sdk/diffs`,
  `opuncleh/plugin-sdk/feishu`, `opuncleh/plugin-sdk/googlechat`,
  `opuncleh/plugin-sdk/irc`, `opuncleh/plugin-sdk/llm-task`,
  `opuncleh/plugin-sdk/lobster`, `opuncleh/plugin-sdk/matrix`,
  `opuncleh/plugin-sdk/mattermost`, `opuncleh/plugin-sdk/memory-core`,
  `opuncleh/plugin-sdk/memory-lancedb`,
  `opuncleh/plugin-sdk/minimax-portal-auth`,
  `opuncleh/plugin-sdk/nextcloud-talk`, `opuncleh/plugin-sdk/nostr`,
  `opuncleh/plugin-sdk/open-prose`, `opuncleh/plugin-sdk/phone-control`,
  `opuncleh/plugin-sdk/qwen-portal-auth`, `opuncleh/plugin-sdk/synology-chat`,
  `opuncleh/plugin-sdk/talk-voice`, `opuncleh/plugin-sdk/test-utils`,
  `opuncleh/plugin-sdk/thread-ownership`, `opuncleh/plugin-sdk/tlon`,
  `opuncleh/plugin-sdk/twitch`, `opuncleh/plugin-sdk/voice-call`,
  `opuncleh/plugin-sdk/zalo`, and `opuncleh/plugin-sdk/zalouser`.
...
`catalog.run(...)` returns the same shape Opuncleh writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to Opuncleh's
...
- if both `catalog` and `discovery` are registered, Opuncleh uses `catalog`

Compatibility note:

- `opuncleh/plugin-sdk` remains supported for existing external plugins.
...
- Read-only command paths such as `opuncleh status`, `opuncleh status --all`,
  `opuncleh channels status`, `opuncleh channels resolve`, and doctor/config
...
- Set `OPUNCLEH_DISABLE_PLUGIN_DISCOVERY_CACHE=1` or
  `OPUNCLEH_DISABLE_PLUGIN_MANIFEST_CACHE=1` to disable these caches.
- Tune cache windows with `OPUNCLEH_PLUGIN_DISCOVERY_CACHE_MS` and
  `OPUNCLEH_PLUGIN_MANIFEST_CACHE_MS`.

## Discovery & precedence

Opuncleh scans, in order:
...
- `<workspace>/.opuncleh/extensions/*.ts`
- `<workspace>/.opuncleh/extensions/*/index.ts`

3. Global extensions

- `~/.opuncleh/extensions/*.ts`
- `~/.opuncleh/extensions/*/index.ts`

4. Bundled extensions (shipped with Opuncleh; mixed default-on/default-off)
...
- If `plugins.allow` is empty and non-bundled plugins are discoverable, Opuncleh logs a startup warning with plugin ids and sources.
- Candidate paths are safety-checked before discovery admission. Opuncleh blocks candidates when:
...
Each native Opuncleh plugin must include a `opuncleh.plugin.json` file in its
...
A plugin directory may include a `package.json` with `opuncleh.extensions`:
...
Security guardrail: every `opuncleh.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `opuncleh plugins install` installs plugin dependencies with
`npm install --ignore-scripts` (no lifecycle scripts). Keep plugin dependency
trees "pure JS/TS" and avoid packages that require `postinstall` builds.

Optional: `opuncleh.setupEntry` can point at a lightweight setup-only module.
When Opuncleh needs setup surfaces for a disabled channel plugin, or
...
Optional: `opuncleh.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let Opuncleh load the
...
Channel plugins can advertise setup/discovery metadata via `opuncleh.channel` and
install hints via `opuncleh.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@opuncleh/nextcloud-talk",
  "opuncleh": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@opuncleh/nextcloud-talk",
...
Opuncleh can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.opuncleh/mpm/plugins.json`
- `~/.opuncleh/mpm/catalog.json`
- `~/.opuncleh/plugins/catalog.json`

Or point `OPUNCLEH_PLUGIN_CATALOG_PATHS` (or `OPUNCLEH_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "opuncleh": { "channel": {...}, "install": {...} } } ] }`.
...
If a plugin exports `id`, Opuncleh uses it but warns when it doesn’t match the
configured id.
...
- Native plugin config is validated using the JSON Schema embedded in
  `opuncleh.plugin.json` (`configSchema`).
- Compatible bundles currently do not expose native Opuncleh config schemas.
...
Opuncleh preserves config for disabled plugins so toggling them back on is not
destructive.
...
Opuncleh augments `uiHints` at runtime based on discovered plugins:
...
```bash
opuncleh plugins list
opuncleh plugins inspect <id>
opuncleh plugins install <path>                 # copy a local file/dir into ~/.opuncleh/extensions/<id>
opuncleh plugins install ./extensions/voice-call # relative path ok
opuncleh plugins install ./plugin.tgz           # install from a local tarball
opuncleh plugins install ./plugin.zip           # install from a local zip
opuncleh plugins install -l ./extensions/voice-call # link (no copy) for dev
opuncleh plugins install @opuncleh/voice-call # install from npm
opuncleh plugins install @opuncleh/voice-call --pin # store exact resolved name@version
opuncleh plugins update <id>
opuncleh plugins update --all
opuncleh plugins enable <id>
opuncleh plugins disable <id>
opuncleh plugins doctor
```

`opuncleh plugins list` shows the top-level format as `opuncleh` or `bundle`.
Verbose list/info output also shows bundle subtype (`codex` or `claude`) plus
detected bundle capabilities.

`plugins update` only works for npm installs tracked under `plugins.installs`.
If stored integrity metadata changes between updates, Opuncleh warns and asks for confirmation (use global `--yes` to bypass prompts).

Plugins may also register their own top‑level commands (example: `opuncleh voicecall`).
...
2. add typed plugin registration/runtime surfaces
   Extend `OpunclehPluginApi` and/or `api.runtime` with the smallest useful
...
This is how Opuncleh stays opinionated without becoming hardcoded to one
...
- Plugin-managed hooks show up in `opuncleh hooks list` with `plugin:<id>`.
- You cannot enable/disable plugin-managed hooks via `opuncleh hooks`; enable/disable the plugin instead.
...
- When disabled, Opuncleh blocks `before_prompt_build` and ignores prompt-mutating fields returned from legacy `before_agent_start` while preserving legacy `modelOverride` and `providerOverride`.
...
setup inside Opuncleh, surface provider setup in onboarding/model-pickers, and
...
- `configPatch`: optional `opuncleh.json` changes
...
explicit method or let Opuncleh synthesize per-method choices.

Opuncleh validates provider wizard metadata when the plugin registers:
...
- `opuncleh models auth login --provider <id> [--method <id>]`
- `opuncleh onboard`
...
- `run` receives a `ProviderAuthContext` with `prompter`, `runtime`,
  ...
  capture, while `opuncleh models auth` keeps a tighter prompt surface.
...
- `wizard.setup` adds a provider choice to onboarding surfaces such as
  `opuncleh onboard` / `opuncleh setup --wizard`.
...
- `deprecatedProfileIds` lets the provider own `opuncleh doctor` cleanup for
...
- `config`: The current Opuncleh config
...
- Main package: `opuncleh` (this repo)
- Plugins: separate npm packages under `@opuncleh/*` (example: `@opuncleh/voice-call`)

Publishing contract:

- Plugin `package.json` must include `opuncleh.extensions` with one or more entry files.
- Optional: `opuncleh.setupEntry` may point at a lightweight setup-only entry for disabled or still-unconfigured channel setup.
- Optional: `opuncleh.startup.deferConfiguredChannelFullLoadUntilAfterListen` may opt a channel plugin into using `setupEntry` during pre-listen gateway startup, but only when that setup entry completely covers the plugin's startup-critical surface.
- Entry files can be `.js` or `.ts` (jiti loads TS at runtime).
- `opuncleh plugins install <npm-spec>` uses `npm pack`, extracts into `~/.opuncleh/extensions/<id>/`, and enables it in config.
...
- CLI: `opuncleh voicecall start|status`
- Tool: `voice_call`
...
- Separately published plugins should run their own CI (lint/build/test) and validate `opuncleh.extensions` points at the built entrypoint (`dist/index.js`).
