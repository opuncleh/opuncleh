---
summary: "Agent tool surface for Opuncleh (browser, canvas, nodes, message, cron) replacing legacy `opuncleh-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `opuncleh-*` skills
title: "Tools"
---

# Tools (Opuncleh)

Opuncleh exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `opuncleh-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `opuncleh.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Opuncleh logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:opuncleh`: all built-in Opuncleh tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`opuncleh node run`).
...
Opuncleh tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires an API key for the chosen provider (recommended: `opuncleh configure --section web`).
...
Control the dedicated Opuncleh-managed browser.
...
- Omit `profile` for the safe default: isolated Opuncleh-managed browser (`opuncleh`).
...
- When `profile` is omitted, uses `browser.defaultProfile` (defaults to `opuncleh`).
...
- Quick smoke: `opuncleh nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- Available when `agents.defaults.imageGenerationModel` is configured, or when Opuncleh can infer a compatible image-generation default from your enabled providers plus available auth.
...
- When editing and `resolution` is omitted, Opuncleh infers a draft/final resolution from the input image size.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `opuncleh gateway` restart in-place)
...
- `sessions_spawn` starts a sub-agent run and posts an announce reply back to the requester chat.
  ...
  - If `runTimeoutSeconds` is omitted, Opuncleh uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise timeout defaults to `0` (no timeout).
...
- `sessions_spawn` supports inline file attachments for subagent runtime only (ACP rejects them). Each attachment has `name`, `content`, and optional `encoding` (`utf8` or `base64`) and `mimeType`. Files are materialized into the child workspace at `.opuncleh/attachments/<uuid>/` with a `.manifest.json` metadata file. The tool returns a receipt with `count`, `totalBytes`, per file `sha256`, and `relDir`. Attachment content is automatically redacted from transcript persistence.
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, Opuncleh clamps `tools.sessions.visibility` to `tree`.
