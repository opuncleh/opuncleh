---
summary: "Chrome extension: let Opuncleh drive your existing Chrome tab"
read_when:
...
The Opuncleh Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate opuncleh-managed Chrome profile.
...
Opuncleh then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Opuncleh release (npm package) as static files. There is no separate “build” step.

After upgrading Opuncleh:

- Re-run `opuncleh browser extension install` to refresh the installed files under your Opuncleh state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

Opuncleh ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `OPUNCLEH_GATEWAY_TOKEN`)
...
- Open the tab you want Opuncleh to control.
...
- `ON`: attached; Opuncleh can drive that tab.
...
Debugging: `opuncleh sandbox explain`
...
`opuncleh browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `opuncleh browser extension install` first to copy the extension to a stable location under your Opuncleh state directory.
...
- **This is not isolated** like the dedicated opuncleh-managed profile.
