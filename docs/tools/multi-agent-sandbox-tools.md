...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `opuncleh sandbox explain`.
...
Legacy `agent.*` configs are migrated by `opuncleh doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=opuncleh-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${OPUNCLEH_STATE_DIR:-$HOME/.opuncleh}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
