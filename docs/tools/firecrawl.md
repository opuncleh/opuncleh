...
Opuncleh can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `opuncleh configure --section web` enables the bundled Firecrawl plugin automatically.
...
Opuncleh always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
