...
read_when:
  - Running Opuncleh Gateway in WSL2 while Chrome lives on Windows
...
- Opuncleh Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/opuncleh/opuncleh/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
Use the built-in `chrome` profile plus the Opuncleh Chrome extension.
...
- Opuncleh points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, Opuncleh is not the problem yet.
...
Fix that before touching Opuncleh config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point Opuncleh at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting Opuncleh to succeed
...
```bash
opuncleh browser open https://example.com --browser-profile remote
opuncleh browser tabs --browser-profile remote
```

For the extension relay:

```bash
opuncleh browser tabs --browser-profile chrome
```

Good result:

- the tab opens in Windows Chrome
- `opuncleh browser tabs` returns the target
...
3. Opuncleh config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug Opuncleh config or Control UI auth
