...
- **gateway host** → `opuncleh` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, Opuncleh also tries to bind
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.opuncleh/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.opuncleh/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `opuncleh approvals allowlist ...`).
- `opuncleh security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `opuncleh doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.opuncleh/exec-approvals.json` directly.

CLI: `opuncleh approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
These clients are opt-in. If a channel does not have exec approvals enabled, Opuncleh does not treat
...
topics, Opuncleh preserves the topic for the approval prompt and the post-approval follow-up.
