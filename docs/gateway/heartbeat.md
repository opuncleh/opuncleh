...
- During heartbeat runs, Opuncleh treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `opuncleh sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Opuncleh skips the heartbeat run entirely (no model call).
...
```bash
opuncleh system event --text "Check for urgent follow-ups" --mode now
```
