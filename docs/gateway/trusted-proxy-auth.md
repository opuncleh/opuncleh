...
read_when:
  - Running Opuncleh behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Opuncleh
...
- You run Opuncleh behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Opuncleh checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Opuncleh extracts the user identity from the configured header
...
- Opuncleh can stay on loopback HTTP behind the proxy.
...
If Opuncleh itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://opuncleh.example.com
    to: http://opuncleh-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
opuncleh.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy opuncleh:18789 {
...
location / {
    ...
    proxy_pass http://opuncleh:18789;
...
`opuncleh security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Opuncleh config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `opuncleh security audit` and review findings
