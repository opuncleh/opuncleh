...
Opuncleh supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
- `gateway.auth.token` SecretRef is inactive for startup auth resolution when `OPUNCLEH_GATEWAY_TOKEN` (or `CLAWDBOT_GATEWAY_TOKEN`) is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, Opuncleh runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). Opuncleh validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/Opuncleh QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/opuncleh"],
...
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `opuncleh.json` refs)
...
When reload-time activation fails after a healthy state, Opuncleh enters degraded secrets state.
...
- Strict command paths (for example `opuncleh memory` remote-memory paths and `opuncleh qr --remote`) read from the active snapshot and fail fast when a required SecretRef is unavailable.
- Read-only command paths (for example `opuncleh status`, `opuncleh status --all`, `opuncleh channels status`, `opuncleh channels resolve`, and read-only doctor/config repair flows) also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `opuncleh secrets reload`.
...
```bash
opuncleh secrets audit --check
opuncleh secrets configure
opuncleh secrets audit --check
```
...
- plaintext values at rest (`opuncleh.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
- plaintext sensitive provider header residues in generated `models.json` entries
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `opuncleh.json` refs)
...
- lets you select supported secret-bearing fields in `opuncleh.json` plus `auth-profiles.json` for one agent scope
...
- `opuncleh secrets configure --providers-only`
- `opuncleh secrets configure --skip-provider-setup`
- `opuncleh secrets configure --agent <id>`
...
```bash
opuncleh secrets apply --from /tmp/opuncleh-secrets-plan.json
opuncleh secrets apply --from /tmp/opuncleh-secrets-plan.json --dry-run
```
...
Opuncleh intentionally does not write rollback backups containing historical plaintext secret values.
