...
- `opuncleh gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `opuncleh doctor` reports no blocking config/service issues.
- `opuncleh channels status --probe` shows connected/ready channels.
...
opuncleh config get agents.defaults.models
```
...
opuncleh status
opuncleh channels status --probe
opuncleh pairing list --channel <channel> [--account <id>]
opuncleh config get channels
opuncleh logs --follow
```
...
| ---------------------------- | -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.             | Paste/set token in the client and retry. For dashboard paths: `opuncleh config get gateway.auth.token` then paste into Control UI settings.                          |
...
| `PAIRING_REQUIRED`           | Device identity is known but not approved for this role. | Approve pending request: `opuncleh devices list` then `opuncleh devices approve <requestId>`.                                                                        |
...
- `Gateway start blocked: set gateway.mode=local` → local gateway mode is not enabled. Fix: set `gateway.mode="local"` in your config (or run `opuncleh configure`). If you are running Opuncleh via Podman using the dedicated `opuncleh` user, the config lives at `~opuncleh/.opuncleh/opuncleh.json`.
...
```bash
opuncleh cron status
opuncleh cron list
opuncleh cron runs --id <jobId> --limit 20
...
opuncleh approvals get --node <idOrNameOrIp>
opuncleh logs --follow
opuncleh status
```
...
opuncleh browser start --browser-profile opuncleh
...
```bash
opuncleh gateway status
opuncleh config get gateway.mode
opuncleh config get gateway.remote.url
opuncleh config get gateway.auth.mode
```
...
```bash
opuncleh config get gateway.bind
opuncleh config get gateway.auth.token
opuncleh gateway status
opuncleh logs --follow
```
