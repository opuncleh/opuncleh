...
export async function handleWhatsAppStart(host: OpunclehApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: OpunclehApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: OpunclehApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: OpunclehApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: OpunclehApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: OpunclehApp): string {
...
function resolveGatewayHttpAuthHeader(host: OpunclehApp): string | null {
...
function buildGatewayHttpHeaders(host: OpunclehApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: OpunclehApp) {
...
export function handleNostrProfileToggleAdvanced(host: OpunclehApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: OpunclehApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: OpunclehApp) {
  const state = host.nostrProfileFormState;
