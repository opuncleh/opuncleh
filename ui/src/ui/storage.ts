const SETTINGS_KEY_PREFIX = "opuncleh.control.settings.v1:";
const LEGACY_TOKEN_SESSION_KEY = "opuncleh.control.token.v1";
const TOKEN_SESSION_KEY_PREFIX = "opuncleh.control.token.v1:";
...
function deriveDefaultGatewayUrl(): { pageUrl: string; effectiveUrl: string } {
  ...
  const configured =
    ...
    typeof window.__OPUNCLEH_CONTROL_UI_BASE_PATH__ === "string" &&
    window.__OPUNCLEH_CONTROL_UI_BASE_PATH__.trim();
...
export function loadSettings(): UiSettings {
  ...
  try {
    ...
    const raw =
      ...
      storage?.getItem("opuncleh.control.settings.v1");
    if (!raw) {
      return defaults;
    }
