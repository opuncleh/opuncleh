...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") {
    await loadPresence(host as unknown as OpunclehApp);
  }
  if (host.tab === "usage") {
    await loadUsage(host as unknown as OpunclehApp);
  }
  if (host.tab === "sessions") {
    await loadSessions(host as unknown as OpunclehApp);
  }
  if (host.tab === "cron") {
    await loadCron(host);
  }
  if (host.tab === "skills") {
    await loadSkills(host as unknown as OpunclehApp);
  }
  if (host.tab === "agents") {
    await loadAgents(host as unknown as OpunclehApp);
    await loadConfig(host as unknown as OpunclehApp);
    const agentIds = host.agentsList?.agents?.map((entry) => entry.id) ?? [];
    if (agentIds.length > 0) {
      void loadAgentIdentities(host as unknown as OpunclehApp, agentIds);
    }
    const agentId =
      host.agentsSelectedId ?? host.agentsList?.defaultId ?? host.agentsList?.agents?.[0]?.id;
    if (agentId) {
      void loadAgentIdentity(host as unknown as OpunclehApp, agentId);
      if (host.agentsPanel === "skills") {
        void loadAgentSkills(host as unknown as OpunclehApp, agentId);
      }
      if (host.agentsPanel === "channels") {
        void loadChannels(host as unknown as OpunclehApp, false);
      }
  ...
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as OpunclehApp);
    await loadDevices(host as unknown as OpunclehApp);
    await loadConfig(host as unknown as OpunclehApp);
    await loadExecApprovals(host as unknown as OpunclehApp);
  }
  if (host.tab === "chat") {
  ...
  if (
    ...
    await loadConfigSchema(host as unknown as OpunclehApp);
    await loadConfig(host as unknown as OpunclehApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as OpunclehApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as OpunclehApp, { reset: true });
...
export function inferBasePath() {
  ...
  const configured = window.__OPUNCLEH_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  const app = host as unknown as OpunclehApp;
...
async function loadOverviewLogs(host: OpunclehApp) {
...
function buildAttentionItems(host: OpunclehApp) {
...
export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as OpunclehApp, true),
    loadConfigSchema(host as unknown as OpunclehApp),
    loadConfig(host as unknown as OpunclehApp),
  ]);
}
