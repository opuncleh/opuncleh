...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/opuncleh/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/opuncleh");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/opuncleh/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__OPUNCLEH_CONTROL_UI_BASE_PATH__ = "/opuncleh";
    const app = mountApp("/opuncleh/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/opuncleh");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/opuncleh/sessions");
  });
  ...
  it("strips query token params without importing them", async () => {
    ...
    expect(JSON.parse(localStorage.getItem("opuncleh.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("hydrates token from URL hash when settings already set", async () => {
    localStorage.setItem(
      ...
      JSON.stringify({ token: "existing-token", gatewayUrl: "wss://gateway.example/opuncleh" }),
    );
    const app = mountApp("/ui/overview#token=abc123");
    await app.updateComplete;
    ...
    expect(JSON.parse(localStorage.getItem("opuncleh.control.settings.v1") ?? "{}")).toMatchObject({
      gatewayUrl: "wss://gateway.example/opuncleh",
    });
    expect(JSON.parse(localStorage.getItem("opuncleh.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("clears the current token when the gateway URL changes", async () => {
    ...
    gatewayUrlInput!.value = "wss://other-gateway.example/opuncleh";
    gatewayUrlInput!.dispatchEvent(new Event("input", { bubbles: true }));
    await app.updateComplete;

    expect(app.settings.gatewayUrl).toBe("wss://other-gateway.example/opuncleh");
    expect(app.settings.token).toBe("");
  });

  it("keeps a hash token pending until the gateway URL change is confirmed", async () => {
    const app = mountApp(
      "/ui/overview?gatewayUrl=wss://other-gateway.example/opuncleh#token=abc123",
    );
    await app.updateComplete;

    expect(app.settings.gatewayUrl).not.toBe("wss://other-gateway.example/opuncleh");
    expect(app.settings.token).toBe("");
