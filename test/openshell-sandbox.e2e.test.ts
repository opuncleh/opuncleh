...
const OPUNCLEH_OPENSHELL_E2E = process.env.OPUNCLEH_E2E_OPENSHELL === "1";
const OPUNCLEH_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const OPUNCLEH_OPENSHELL_COMMAND =
  process.env.OPUNCLEH_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "opuncleh-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && OPUNCLEH_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: OPUNCLEH_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(OPUNCLEH_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "opuncleh-openshell-e2e-"));
      ...
      const gatewayName = `opuncleh-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `opuncleh-policy-allow-${scopeSuffix}`;
      ...
      const sandboxCfg = {
        ...
        browser: {
          enabled: false,
          image: "opuncleh-browser",
          containerPrefix: "opuncleh-browser-",
          network: "bridge",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: OPUNCLEH_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("opuncleh-openshell-e2e");
