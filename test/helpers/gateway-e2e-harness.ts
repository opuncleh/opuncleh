...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `opuncleh-e2e-${name}-`));
  const configDir = path.join(homeDir, ".opuncleh");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          OPUNCLEH_SKIP_PROVIDERS: "1",
          OPUNCLEH_SKIP_GMAIL_WATCHER: "1",
          OPUNCLEH_SKIP_CRON: "1",
          OPUNCLEH_SKIP_BROWSER_CONTROL_SERVER: "1",
          OPUNCLEH_SKIP_CANVAS_HOST: "1",
          OPUNCLEH_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
