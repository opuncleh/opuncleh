...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked OPUNCLEH_HOME.
  delete process.env.OPUNCLEH_HOME;
  process.env.OPUNCLEH_STATE_DIR = path.join(base, ".opuncleh");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "opuncleh-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".opuncleh", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
