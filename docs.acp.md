...
This document describes how the Opuncleh ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`opuncleh acp` exposes an ACP agent over stdio and forwards prompts to a running
Opuncleh Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing Opuncleh Gateway
...
- ACP client spawns `opuncleh acp` and speaks ACP messages over stdio.
...
`opuncleh acp` resolves the Gateway URL and auth from CLI flags or config:
