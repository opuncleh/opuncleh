...
MINGIT_ZIP_NAME=""
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/opuncleh-parallels-windows.XXXXXX)"
BUILD_LOCK_DIR="${TMPDIR:-/tmp}/opuncleh-parallels-build.lock"
...
Options:
  ...
  --snapshot-hint <name>     Snapshot name substring/fuzzy match.
                             Default: "pre-opuncleh-native-e2e-2026-03-12"
  ...
  --target-package-spec <npm-spec>
                             ...
                             Example: opuncleh@2026.3.13-beta.1
...
guest_run_opuncleh() {
  ...
  stdout_name="opuncleh-stdout-$RANDOM-$RANDOM.log"
  stderr_name="opuncleh-stderr-$RANDOM-$RANDOM.log"
...
try {
  ...
  \$proc = Start-Process -FilePath (Join-Path \$env:APPDATA 'npm\opuncleh.cmd') -ArgumentList $args_literal -NoNewWindow -PassThru -RedirectStandardOutput \$stdout -RedirectStandardError \$stderr
  \$proc.WaitForExit()
  if (Test-Path \$stdout) {
    Get-Content \$stdout
  }
...
resolve_latest_version() {
  ...
  npm view opuncleh version --userconfig "$(mktemp)"
}
...
req = urllib.request.Request(
    ...
    headers={
        "User-Agent": "opuncleh-parallels-smoke",
...
ensure_guest_git() {
  ...
  guest_exec cmd.exe /d /s /c "if exist \"%LOCALAPPDATA%\\Opuncleh\\deps\\portable-git\" rmdir /s /q \"%LOCALAPPDATA%\\Opuncleh\\deps\\portable-git\""
  guest_exec cmd.exe /d /s /c "if not exist \"%LOCALAPPDATA%\\Opuncleh\\deps\" mkdir \"%LOCALAPPDATA%\\Opuncleh\\deps\""
  guest_exec cmd.exe /d /s /c "mkdir \"%LOCALAPPDATA%\\Opuncleh\\deps\\portable-git\""
  guest_exec cmd.exe /d /s /c "curl.exe -fsSL \"$mingit_url\" -o \"%TEMP%\\$MINGIT_ZIP_NAME\""
  guest_exec cmd.exe /d /s /c "tar.exe -xf \"%TEMP%\\$MINGIT_ZIP_NAME\" -C \"%LOCALAPPDATA%\\Opuncleh\\deps\\portable-git\""
  guest_exec cmd.exe /d /s /c "del /q \"%TEMP%\\$MINGIT_ZIP_NAME\" & set \"PATH=%LOCALAPPDATA%\\Opuncleh\\deps\\portable-git\\cmd;%LOCALAPPDATA%\\Opuncleh\\deps\\portable-git\\mingw64\\bin;%LOCALAPPDATA%\\Opuncleh\\deps\\portable-git\\usr\\bin;%PATH%\" && git.exe --version"
}
...
pack_main_tgz() {
  ...
  MAIN_TGZ_PATH="$MAIN_TGZ_DIR/opuncleh-main-$short_head.tgz"
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/opuncleh-parallels-windows-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
& (Join-Path \$env:APPDATA 'npm\opuncleh.cmd') --version
EOF
)"
}
...
install_main_tgz() {
  ...
  guest_exec cmd.exe /d /s /c "set \"PATH=%LOCALAPPDATA%\\Opuncleh\\deps\\portable-git\\cmd;%LOCALAPPDATA%\\Opuncleh\\deps\\portable-git\\mingw64\\bin;%LOCALAPPDATA%\\Opuncleh\\deps\\portable-git\\usr\\bin;%PATH%\" && curl.exe -fsSL \"$tgz_url\" -o \"%TEMP%\\$temp_name\" && npm.cmd install -g \"%TEMP%\\$temp_name\" --no-fund --no-audit && \"%APPDATA%\\npm\\opuncleh.cmd\" --version"
}

verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_run_opuncleh "" "" "--version")"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
run_ref_onboard() {
  ...
  runner_name="opuncleh-onboard-$RANDOM-$RANDOM.ps1"
  log_name="opuncleh-onboard-$RANDOM-$RANDOM.log"
  done_name="opuncleh-onboard-$RANDOM-$RANDOM.done"
...
try {
  \$opuncleh = Join-Path \$env:APPDATA 'npm\opuncleh.cmd'
  \$cmdLine = ('"{0}" onboard --non-interactive --mode local --auth-choice openai-api-key --secret-input-mode ref --gateway-port 18789 --gateway-bind loopback --install-daemon --skip-skills --accept-risk --json > "{1}" 2>&1' -f \$opuncleh, \$log)
...
verify_gateway() {
  guest_run_opuncleh "" "" gateway status --deep --require-rpc
}

show_gateway_status_compat() {
  if guest_run_opuncleh "" "" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_run_opuncleh "" "" gateway status --deep --require-rpc
    return
  fi
  guest_run_opuncleh "" "" gateway status --deep
}

verify_turn() {
  guest_run_opuncleh "" "" agent --agent main --message "Reply with exact ASCII text OK only." --json
}
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "opuncleh-main-fresh.tgz" || return $?
...
run_upgrade_lane() {
  ...
  phase_run "upgrade.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "opuncleh-main-upgrade.tgz" || return $?
