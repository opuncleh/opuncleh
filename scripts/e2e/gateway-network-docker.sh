...
IMAGE_NAME="opuncleh-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="opuncleh-net-e2e-$$"
GW_NAME="opuncleh-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "OPUNCLEH_GATEWAY_TOKEN=$TOKEN" \
  -e "OPUNCLEH_SKIP_CHANNELS=1" \
  -e "OPUNCLEH_SKIP_GMAIL_WATCHER=1" \
  -e "OPUNCLEH_SKIP_CRON=1" \
  -e "OPUNCLEH_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
