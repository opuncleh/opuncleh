...
SNAPSHOT_STATE=""
SNAPSHOT_NAME=""
GUEST_OPUNCLEH_BIN="/opt/homebrew/bin/opuncleh"
GUEST_OPUNCLEH_ENTRY="/opt/homebrew/lib/node_modules/opuncleh/opuncleh.mjs"
...
MAIN_TGZ_PATH=""
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/opuncleh-parallels-smoke.XXXXXX)"
...
spawn {*}$cmd
send -- "printf '__OPUNCLEH_READY__\\n'\r"
expect "__OPUNCLEH_READY__"
log_user 0
send -- "export PS1='' PROMPT='' PROMPT2='' RPROMPT=''\r"
send -- "stty -echo\r"

send -- "cat >/tmp/opuncleh-prl.sh <<'__OPUNCLEH_SCRIPT__'\r"
send -- $script
if {![string match "*\n" $script]} {
  send -- "\r"
}
send -- "__OPUNCLEH_SCRIPT__\r"
send -- "/bin/bash /tmp/opuncleh-prl.sh; rc=\$?; rm -f /tmp/opuncleh-prl.sh; printf '__OPUNCLEH_RC__:%s\\n' \"\$rc\"; exit \"\$rc\"\r"
log_user 1

set rc 1
expect {
  -re {__OPUNCLEH_RC__:(-?[0-9]+)} {
...
export OPUNCLEH_NO_ONBOARD=1
curl -fsSL $install_url_q -o /tmp/opuncleh-install.sh
bash /tmp/opuncleh-install.sh${version_arg_q}
$GUEST_OPUNCLEH_BIN --version
EOF
)"
}
...
verify_version_contains() {
  ...
  local version
  version="$(
    guest_current_user_exec "$GUEST_OPUNCLEH_BIN" --version
  )"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
...
start_server() {
  ...
  ) >/tmp/opuncleh-parallels-http.log 2>&1 &
  SERVER_PID=$!
  sleep 1
...
verify_bundle_permissions() {
  ...
  cmd="$(cat <<EOF
root=\$($npm_q root -g); check_path() { local path="\$1"; [ -e "\$path" ] || return 0; local perm perm_oct; perm=\$(/usr/bin/stat -f '%OLp' "\$path"); perm_oct=\$((8#\$perm)); if (( perm_oct & 0002 )); then echo "world-writable install artifact: \$path (\$perm)" >&2; exit 1; fi; }; check_path "\$root/opuncleh"; check_path "\$root/opuncleh/extensions"; if [ -d "\$root/opuncleh/extensions" ]; then while IFS= read -r -d '' extension_dir; do check_path "\$extension_dir"; done < <(/usr/bin/find "\$root/opuncleh/extensions" -mindepth 1 -maxdepth 1 -type d -print0); fi
EOF
)"
...
run_ref_onboard() {
  guest_current_user_exec \
    ...
    "$GUEST_NODE_BIN" "$GUEST_OPUNCLEH_ENTRY" onboard \
    --non-interactive \
    --mode local \
...
verify_gateway() {
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_OPUNCLEH_ENTRY" gateway status --deep --require-rpc
}

show_gateway_status_compat() {
  if guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_OPUNCLEH_ENTRY" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_OPUNCLEH_ENTRY" gateway status --deep --require-rpc
    return
  fi
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_OPUNCLEH_ENTRY" gateway status --deep
}

verify_turn() {
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_OPUNCLEH_ENTRY" agent --agent main --message ping --json
}
...
PY
  ...
  script="$(cat <<EOF
cat >/tmp/opuncleh-discord-token <<'__OPUNCLEH_TOKEN__'
$DISCORD_TOKEN_VALUE
__OPUNCLEH_TOKEN__
cat >/tmp/opuncleh-discord-guilds.json <<'__OPUNCLEH_GUILDS__'
$guilds_json
__OPUNCLEH_GUILDS__
token="\$(tr -d '\n' </tmp/opuncleh-discord-token)"
guilds_json="\$(cat /tmp/opuncleh-discord-guilds.json)"
$GUEST_NODE_BIN $GUEST_OPUNCLEH_ENTRY config set channels.discord.token "\$token"
$GUEST_NODE_BIN $GUEST_OPUNCLEH_ENTRY config set channels.discord.enabled true
$GUEST_NODE_BIN $GUEST_OPUNCLEH_ENTRY config set channels.discord.groupPolicy allowlist
$GUEST_NODE_BIN $GUEST_OPUNCLEH_ENTRY config set channels.discord.guilds "\$guilds_json" --strict-json
$GUEST_NODE_BIN $GUEST_OPUNCLEH_ENTRY gateway restart
for _ in 1 2 3 4 5 6 7 8; do
  if $GUEST_NODE_BIN $GUEST_OPUNCLEH_ENTRY gateway status --deep --require-rpc >/dev/null 2>&1; then
    break
  fi
  sleep 2
done
$GUEST_NODE_BIN $GUEST_OPUNCLEH_ENTRY channels status --probe --json
rm -f /tmp/opuncleh-discord-token /tmp/opuncleh-discord-guilds.json
EOF
)"
  prlctl exec "$VM_NAME" --current-user /usr/bin/env \
    PATH=/opt/homebrew/bin:/opt/homebrew/opt/node/bin:/opt/homebrew/sbin:/usr/bin:/bin:/usr/sbin:/sbin \
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "opuncleh-main-fresh.tgz"
...
run_upgrade_lane() {
  ...
  phase_run "upgrade.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "opuncleh-main-upgrade.tgz"
