...
MAIN_TGZ_PATH=""
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/opuncleh-parallels-linux.XXXXXX)"
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/opuncleh-parallels-linux-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
install_latest_release() {
  ...
  guest_exec curl -fsSL "$INSTALL_URL" -o /tmp/opuncleh-install.sh
  guest_exec /usr/bin/env OPUNCLEH_NO_ONBOARD=1 bash /tmp/opuncleh-install.sh "${version_args[@]}" --no-onboard
  guest_exec opuncleh --version
}
...
verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_exec opuncleh --version)"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
run_ref_onboard() {
  guest_exec /usr/bin/env "OPENAI_API_KEY=$OPENAI_API_KEY_VALUE" opuncleh onboard \
    --non-interactive \
    --mode local \
...
verify_local_turn() {
  guest_exec /usr/bin/env "OPENAI_API_KEY=$OPENAI_API_KEY_VALUE" opuncleh agent \
    --local \
    --agent main \
    --message ping \
    --json
