...
IMAGE_NAME="opuncleh-plugins-e2e"
...
if [ -f dist/index.mjs ]; then
  OPUNCLEH_ENTRY="dist/index.mjs"
elif [ -f dist/index.js ]; then
  OPUNCLEH_ENTRY="dist/index.js"
else
  echo "Missing dist/index.(m)js (build output):"
  ls -la dist || true
  exit 1
fi
export OPUNCLEH_ENTRY

home_dir=$(mktemp -d "/tmp/opuncleh-plugins-e2e.XXXXXX")
...
JS
  cat > "$dir/opuncleh.plugin.json" <<'JSON'
{
  "id": "placeholder",
  "configSchema": {
    "type": "object",
...
JSON
  node - <<'NODE' "$dir/opuncleh.plugin.json" "$id"
...
mkdir -p "$HOME/.opuncleh/extensions/demo-plugin"

cat > "$HOME/.opuncleh/extensions/demo-plugin/index.js" <<'JS'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
JS
cat > "$HOME/.opuncleh/extensions/demo-plugin/opuncleh.plugin.json" <<'JSON'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
node "$OPUNCLEH_ENTRY" plugins list --json > /tmp/plugins.json
...
pack_dir="$(mktemp -d "/tmp/opuncleh-plugin-pack.XXXXXX")"
mkdir -p "$pack_dir/package"
cat > "$pack_dir/package/package.json" <<'JSON'
{
  "name": "@opuncleh/demo-plugin-tgz",
  "version": "0.0.1",
  "opuncleh": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$pack_dir/package/opuncleh.plugin.json" <<'JSON'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
node "$OPUNCLEH_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
node "$OPUNCLEH_ENTRY" plugins list --json > /tmp/plugins2.json
...
dir_plugin="$(mktemp -d "/tmp/opuncleh-plugin-dir.XXXXXX")"
cat > "$dir_plugin/package.json" <<'JSON'
{
  "name": "@opuncleh/demo-plugin-dir",
  "version": "0.0.1",
  "opuncleh": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$dir_plugin/opuncleh.plugin.json" <<'JSON'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
node "$OPUNCLEH_ENTRY" plugins install "$dir_plugin"
node "$OPUNCLEH_ENTRY" plugins list --json > /tmp/plugins3.json
...
file_pack_dir="$(mktemp -d "/tmp/opuncleh-plugin-filepack.XXXXXX")"
mkdir -p "$file_pack_dir/package"
cat > "$file_pack_dir/package/package.json" <<'JSON'
{
  "name": "@opuncleh/demo-plugin-file",
  "version": "0.0.1",
  "opuncleh": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$file_pack_dir/package/opuncleh.plugin.json" <<'JSON'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
node "$OPUNCLEH_ENTRY" plugins install "file:$file_pack_dir/package"
node "$OPUNCLEH_ENTRY" plugins list --json > /tmp/plugins4.json
...
cat > "$HOME/.claude/plugins/known_marketplaces.json" <<JSON
{
  "claude-fixtures": {
    ...
    "source": {
      "type": "github",
      "repo": "opuncleh/fixture-marketplace"
    }
  }
}
JSON

node "$OPUNCLEH_ENTRY" plugins marketplace list claude-fixtures --json > /tmp/marketplace-list.json
...
node "$OPUNCLEH_ENTRY" plugins install marketplace-shortcut@claude-fixtures
node "$OPUNCLEH_ENTRY" plugins install marketplace-direct --marketplace claude-fixtures
node "$OPUNCLEH_ENTRY" plugins list --json > /tmp/plugins-marketplace.json
...
const configPath = path.join(process.env.HOME, ".opuncleh", "opuncleh.json");
...
write_fixture_plugin \
  ...
  "Marketplace Shortcut"
node "$OPUNCLEH_ENTRY" plugins update marketplace-shortcut --dry-run
node "$OPUNCLEH_ENTRY" plugins update marketplace-shortcut
node "$OPUNCLEH_ENTRY" plugins list --json > /tmp/plugins-marketplace-updated.json
