...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const manifestPath = path.join(pluginDir, "opuncleh.plugin.json");
    const distPluginDir = path.join(distExtensionsRoot, dirent.name);
    const distManifestPath = path.join(distPluginDir, "opuncleh.plugin.json");
    ...
    if (packageJson.opuncleh && "extensions" in packageJson.opuncleh) {
      packageJson.opuncleh = {
        ...packageJson.opuncleh,
        extensions: rewritePackageExtensions(packageJson.opuncleh.extensions),
        ...(typeof packageJson.opuncleh.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.opuncleh.setupEntry) }
          : {}),
      };
    }
