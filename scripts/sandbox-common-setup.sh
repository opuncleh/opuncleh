...
BASE_IMAGE="${BASE_IMAGE:-opuncleh-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-opuncleh-sandbox-common:bookworm-slim}"
...
OPUNCLEH_DOCKER_BUILD_USE_BUILDX="${OPUNCLEH_DOCKER_BUILD_USE_BUILDX:-0}"
OPUNCLEH_DOCKER_BUILD_CACHE_FROM="${OPUNCLEH_DOCKER_BUILD_CACHE_FROM:-}"
OPUNCLEH_DOCKER_BUILD_CACHE_TO="${OPUNCLEH_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${OPUNCLEH_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${OPUNCLEH_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${OPUNCLEH_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${OPUNCLEH_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${OPUNCLEH_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=opuncleh.sandbox=1)
NOTE
