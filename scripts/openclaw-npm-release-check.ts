...
const EXPECTED_REPOSITORY_URL = "https://github.com/opuncleh/opuncleh";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "opuncleh") {
    errors.push(`package.json name must be "opuncleh"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.opuncleh !== "opuncleh.mjs") {
    errors.push(
      `package.json bin.opuncleh must be "opuncleh.mjs"; found "${pkg.bin?.opuncleh ?? ""}".`,
    );
  }
...
function main(): number {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`opuncleh-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `opuncleh-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta).`,
  );
  return 0;
}
