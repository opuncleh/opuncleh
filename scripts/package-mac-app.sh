...
# Build and bundle Opuncleh into a minimal .app we can open.
# Outputs to dist/Opuncleh.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Opuncleh.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Opuncleh"
BUNDLE_ID="${BUNDLE_ID:-ai.opuncleh.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/opuncleh/opuncleh/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Opuncleh/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :OpunclehBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :OpunclehGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Opuncleh"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Opuncleh"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Opuncleh"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Opuncleh" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Opuncleh/Resources/Opuncleh.icns" "$APP_ROOT/Contents/Resources/Opuncleh.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Opuncleh/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying OpunclehKit resources"
OPUNCLEHKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/OpunclehKit_OpunclehKit.bundle"
if [ -d "$OPUNCLEHKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/OpunclehKit_OpunclehKit.bundle"
  cp -R "$OPUNCLEHKIT_BUNDLE" "$APP_ROOT/Contents/Resources/OpunclehKit_OpunclehKit.bundle"
else
  echo "WARN: OpunclehKit resource bundle not found at $OPUNCLEHKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Opuncleh"
killall -q Opuncleh 2>/dev/null || true
