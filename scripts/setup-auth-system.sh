#!/bin/bash
# Setup Opuncleh Auth Management System
...
echo "=== Opuncleh Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'opuncleh-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Opuncleh message: Send warning via Opuncleh itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/opuncleh-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/opuncleh-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/opuncleh-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now opuncleh-auth-monitor.timer
...
echo "   scp $SCRIPT_DIR/termux-quick-auth.sh phone:~/.shortcuts/LobstrAuth"
echo "   scp $SCRIPT_DIR/termux-auth-widget.sh phone:~/.shortcuts/LobstrAuth-Full"
echo ""
echo "4. Make them executable on phone:"
echo "   ssh phone 'chmod +x ~/.shortcuts/Lobstr*'"
echo ""
echo "5. Add Termux:Widget to your home screen"
echo "6. Tap the widget to see your auth scripts"
echo ""
echo "The quick widget (LobstrAuth) shows status and opens auth URL if needed."
echo "The full widget (LobstrAuth-Full) provides guided re-auth flow."
echo ""
...
echo "  - Auth monitor: systemctl --user status opuncleh-auth-monitor.timer"
echo ""
echo "Quick commands:"
