...
const { clearPluginCommands, getPluginCommandSpecs, loadOpunclehPlugins, matchPluginCommand } =
  await import(pathToFileURL(smokeEntryPath).href);

assert.equal(typeof loadOpunclehPlugins, "function", "built loader export missing");
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "opuncleh-build-smoke-"));
...
fs.writeFileSync(
  ...
  JSON.stringify(
    {
      name: "@opuncleh/build-smoke-plugin",
      type: "module",
      opuncleh: {
        extensions: ["./index.js"],
      },
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'opuncleh/plugin-sdk';",
...
const registry = loadOpunclehPlugins({
  ...
  env: {
    ...process.env,
    OPUNCLEH_BUNDLED_PLUGINS_DIR: path.join(tempRoot, "dist-runtime", "extensions"),
    OPUNCLEH_DISABLE_PLUGIN_DISCOVERY_CACHE: "1",
  },
  config: {
    plugins: {
      enabled: true,
