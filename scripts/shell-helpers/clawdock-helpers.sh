#!/usr/bin/env bash
# ClawDock - Docker helpers for Opuncleh
# Inspired by Simon Willison's "Running Opuncleh in Docker"
# https://til.simonwillison.net/llms/opuncleh-docker
#
# Installation:
...
# Common paths to check for Opuncleh
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/opuncleh"
  "${HOME}/workspace/opuncleh"
  "${HOME}/projects/opuncleh"
  "${HOME}/dev/opuncleh"
  "${HOME}/code/opuncleh"
  "${HOME}/src/opuncleh"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found Opuncleh at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/opuncleh"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ Opuncleh not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/opuncleh/opuncleh.git ~/opuncleh"
    echo "  cd ~/opuncleh && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^OPUNCLEH_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d opuncleh-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart opuncleh-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f opuncleh-gateway
}
...
clawdock-workspace() {
  cd ~/.opuncleh/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec opuncleh-gateway \
    bash -c 'echo "alias opuncleh=\"./opuncleh.mjs\"" > /tmp/.bashrc_opuncleh && bash --rcfile /tmp/.bashrc_opuncleh'
}
...
clawdock-cli() {
  _clawdock_compose run --rm opuncleh-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build opuncleh-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "OPUNCLEH_GATEWAY_TOKEN=$token" opuncleh-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" opuncleh-gateway \
    bash -c './opuncleh.mjs config set gateway.remote.token "$TOKEN" && ./opuncleh.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec opuncleh-gateway \
    bash -c "./opuncleh.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart opuncleh-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm opuncleh-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec opuncleh-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'opuncleh config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for Opuncleh${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (opuncleh alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to opuncleh project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.opuncleh)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'opuncleh channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'opuncleh status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.opuncleh.com${_CLR_RESET}"
  echo ""
}
