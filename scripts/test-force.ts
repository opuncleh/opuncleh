...
function runTests() {
  const isolatedLock =
    process.env.OPUNCLEH_GATEWAY_LOCK ??
    path.join(os.tmpdir(), `opuncleh-gateway.lock.test.${Date.now()}`);
  const result = spawnSync("pnpm", ["vitest", "run"], {
    stdio: "inherit",
    env: {
      ...process.env,
      OPUNCLEH_GATEWAY_LOCK: isolatedLock,
    },
  });
  if (result.error) {
...
function main() {
  const port = Number.parseInt(process.env.OPUNCLEH_GATEWAY_PORT ?? `${DEFAULT_PORT}`, 10);
