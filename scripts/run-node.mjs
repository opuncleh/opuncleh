...
const extensionRestartMetadataFiles = new Set(["opuncleh.plugin.json", "package.json"]);
...
const shouldBuild = (deps) => {
  if (deps.env.OPUNCLEH_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.OPUNCLEH_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[opuncleh] ${message}\n`);
};

const runOpuncleh = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["opuncleh.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
