...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/opuncleh/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Opuncleh Re-Auth" -c "After getting key, run: ssh $SERVER '~/opuncleh/scripts/mobile-reauth.sh'" --id opuncleh-auth
        ;;
    *)
