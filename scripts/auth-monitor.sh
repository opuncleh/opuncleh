...
#
# Suggested cron: */30 * * * * /home/admin/opuncleh/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Opuncleh notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., opuncleh-alerts)
...
STATE_FILE="$HOME/.opuncleh/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Opuncleh if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use opuncleh
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Opuncleh to $NOTIFY_PHONE..."
            opuncleh send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Opuncleh is down. Run: ssh l36 '~/opuncleh/scripts/mobile-reauth.sh'" "urgent"
    exit 1
