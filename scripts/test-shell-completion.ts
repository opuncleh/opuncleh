 ...
 * `opuncleh update`. Use it to verify the completion installation flow
...
\`opuncleh update\`, \`opuncleh doctor\`, and \`opuncleh onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Opuncleh Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`opuncleh completion --install\` later to enable.`));
    return;
  }
