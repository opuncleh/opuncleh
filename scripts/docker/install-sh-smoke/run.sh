...
INSTALL_URL="${OPUNCLEH_INSTALL_URL:-https://opuncleh.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${OPUNCLEH_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${OPUNCLEH_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="opuncleh"
PACKAGE_NAME="${OPUNCLEH_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (OPUNCLEH_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${OPUNCLEH_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${OPUNCLEH_INSTALL_LATEST_OUT:-}"
fi
