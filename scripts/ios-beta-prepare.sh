...
- regenerates apps/ios/Opuncleh.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${OPUNCLEH_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid OPUNCLEH_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid OPUNCLEH_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid OPUNCLEH_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid OPUNCLEH_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing OPUNCLEH_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(OPUNCLEH_URL_SLASH)$(OPUNCLEH_URL_SLASH)#g'
)"
...
OPUNCLEH_CODE_SIGN_STYLE = Automatic
OPUNCLEH_DEVELOPMENT_TEAM = ${TEAM_ID}
OPUNCLEH_IOS_SELECTED_TEAM = ${TEAM_ID}
OPUNCLEH_APP_BUNDLE_ID = ai.opuncleh.client
OPUNCLEH_SHARE_BUNDLE_ID = ai.opuncleh.client.share
OPUNCLEH_ACTIVITY_WIDGET_BUNDLE_ID = ai.opuncleh.client.activitywidget
OPUNCLEH_WATCH_APP_BUNDLE_ID = ai.opuncleh.client.watchkitapp
OPUNCLEH_WATCH_EXTENSION_BUNDLE_ID = ai.opuncleh.client.watchkitapp.extension
OPUNCLEH_APP_PROFILE =
OPUNCLEH_SHARE_PROFILE =
OPUNCLEH_PUSH_TRANSPORT = relay
OPUNCLEH_PUSH_DISTRIBUTION = official
OPUNCLEH_URL_SLASH = /
OPUNCLEH_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
OPUNCLEH_PUSH_APNS_ENVIRONMENT = production
EOF
