...
function buildInputOptions(options: InputOptionsArg): InputOptionsReturn {
  if (process.env.OPUNCLEH_BUILD_VERBOSE === "1") {
    return undefined;
  }
    ...
    if (fs.existsSync(packageJsonPath)) {
      try {
        const packageJson = JSON.parse(fs.readFileSync(packageJsonPath, "utf8")) as {
          opuncleh?: { extensions?: unknown; setupEntry?: unknown };
        };
        packageEntries = Array.isArray(packageJson.opuncleh?.extensions)
          ? packageJson.opuncleh.extensions.filter(
              (entry): entry is string => typeof entry === "string" && entry.trim().length > 0,
            )
          : [];
        const setupEntry =
          typeof packageJson.opuncleh?.setupEntry === "string" &&
          packageJson.opuncleh.setupEntry.trim().length > 0
            ? packageJson.opuncleh.setupEntry
            : undefined;
        if (setupEntry) {
