...
import "@a2ui/lit/ui";
import { themeContext } from "@opuncleh/a2ui-theme-context";
...
class OpunclehA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  connectedCallback() {
    ...
    for (const eventName of ["opuncleh:a2ui-action-status"]) {
  ...
  #handleA2UIAction(evt) {
    ...
    const handler =
      globalThis.webkit?.messageHandlers?.opunclehCanvasA2UIAction ??
      globalThis.opunclehCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (handler === globalThis.opunclehCanvasA2UIAction) {
...
if (!customElements.get("opuncleh-a2ui-host")) {
  customElements.define("opuncleh-a2ui-host", OpunclehA2UIHost);
}
